...
function runReachabilityAudit(kernel, options = {}) {
  ...
  return {
    ...governFindings(kernel, findings, options),
    unacknowledgedCount: findings.length,
  };
}
...
async function runSourceSimulation(kernel, options = {}) {
  ...
  return {
    ...governFindings(kernel, [simulation.finding], options),
    simulation,
  };
}
...
module.exports = {
  ...
  run(kernel, input = {}) {
    ...
    if (action === 'simulate') {
      return runSourceSimulation(kernel, input).catch(failure);
    }
