...
// Mutable so runtime callers (plugins, config) can extend the disjoint set via
// registerDisjointPair. Kept as a plain array (not Object.freeze) for that reason.
const DISJOINT_TYPE_PAIRS = [
  ['hayvan', 'bitki'],
  ['canlı', 'cansız'],
  ['insan', 'kurum'],
  ['ilaç', 'hastalık'],
...
/**
 * Register an additional disjoint type pair at runtime.
 * Both directions (a↔b) are covered by pairMatchesDisjoint.
 * No-op if an equivalent pair already exists (either order).
 * @param {string} left
 * @param {string} right
 * @returns {boolean} true if a new pair was added, false if it already existed
 */
function registerDisjointPair(left, right) {
  const a = normalizeTypeText(left);
  const b = normalizeTypeText(right);
  if (!a || !b || a === b) return false;
  if (pairMatchesDisjoint(a, b)) return false;
  DISJOINT_TYPE_PAIRS.push([a, b]);
  return true;
}
