...
const SAFE_SHELL = /^(?:pwd|whoami|hostname|dir|ls|Get-ChildItem|Get-Location|git\s+(?:status|diff|log|show|branch|rev-parse|remote)|rg|grep|find|where|which|type|Get-Content)(?:\s|$)/i;
...
function inferRiskCategory(envelope) {
  ...
  if (envelope.kind === EXTERNAL_ACTION_KINDS.SHELL) {
    ...
    if (SAFE_SHELL.test(envelope.command)) return ACTION_CATEGORIES.READ_ONLY;
...
function normalizeExternalActionEnvelope(input, options = {}) {
  ...
  envelope.riskCategory = inferRiskCategory(envelope);
