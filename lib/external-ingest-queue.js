...
function trustedResolverRequest(data, sourceType, identity, requestedAt, expiresAt, options) {
  const base = {
    sourceType,
  ...
  if (sourceType === 'github') {
    return {
      ...base,
      repoUrl: data.repoUrl || data.url || '',
      commitSha: data.commitSha || data.sha || data.oid || '',
      paths: data.paths,
    };
  }
  const trustedRootPath = String(options.markdownRootPath || '').trim();
  if (!trustedRootPath) {
    return failure('EXTERNAL_QUEUE_MARKDOWN_ROOT_REQUIRED', 'trusted markdownRootPath configuration is required');
  }
  return {
    ...base,
    rootPath: trustedRootPath,
    path: data.path || data.targetPath || '',
  };
}

async function resolveForWindow(data, sourceType, identity, requestedAt, expiresAt, options) {
  const request = trustedResolverRequest(data, sourceType, identity, requestedAt, expiresAt, options);
  if (!request.ok && request.code) return request;
  const resolved = await resolveExternalIngestApproval(request, options.resolverOptions || {});
...
async function queueReviewedExternalIngest(store, data = {}, options = {}) {
  ...
  if (sourceType === 'markdown' && !String(options.markdownRootPath || '').trim()) {
    return failure('EXTERNAL_QUEUE_MARKDOWN_ROOT_REQUIRED', 'trusted markdownRootPath configuration is required');
  }
