...
test('GitHub ingest source metadata strips URL credentials, query, and fragments', async () => {
  const data = {
    sourceType: 'github',
  ...
  const sourceRef = buildSourceRef(data, 'github');
  const payload = buildCapabilityPayload(data, 'github', sourceRef, 'idem-github-redaction');
  assert.equal(payload.repoUrl, 'https://github.com/owner/repo');
  assert.equal(sourceRef.includes('secret'), false);
  assert.equal(sourceRef.includes('#fragment'), false);

  const calls = [];
  const result = await handleIngest({
    kernel: { runCapability: async (name, value) => { calls.push({ name, value }); return { ok: true }; } },
    data,
  });
  assert.equal(result.ingestMeta.sourceRef.includes('secret'), false);
  assert.equal(JSON.stringify(calls[0]).includes('secret'), false);
});

test('invalid GitHub repo URLs fail closed across ingest builders', () => {
  assert.doesNotThrow(() => buildSourceRef({ sourceType: 'github' }, 'github'));
  assert.equal(buildSourceRef({ sourceType: 'github' }, 'github'), '');
  assert.equal(buildSourceRef({ sourceType: 'github', repoUrl: 'https://example.com/owner/repo' }, 'github'), '');
  assert.equal(buildCapabilityPayload({ sourceType: 'github' }, 'github', '', 'idem-invalid'), null);
  assert.equal(buildCapabilityPayload({ sourceType: 'github', repoUrl: 'https://example.com/owner/repo' }, 'github', '', 'idem-invalid'), null);
});

test('handleIngest returns structured GitHub URL errors without invoking a capability', async () => {
  const calls = [];
  const kernel = { runCapability: async (...args) => { calls.push(args); return { ok: true }; } };

  const missing = await handleIngest({ kernel, data: { sourceType: 'github' } });
  assert.deepEqual(missing, { ok: false, code: 'REPO_URL_REQUIRED', error: 'repoUrl is required' });

  const invalid = await handleIngest({ kernel, data: { sourceType: 'github', repoUrl: 'https://example.com/owner/repo' } });
  assert.deepEqual(invalid, { ok: false, code: 'REPO_URL_INVALID', error: 'Invalid GitHub repository URL' });
