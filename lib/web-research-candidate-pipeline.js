...
function oppositionLinks(signals, provenance) {
  const links = [];
  const seen = new Set();
  for (const signal of signals) {
    ...
    const targetId = `${from}|${relation}|${to}`;
    ...
    links.push({
      ...
      targetId,
      rule,
      flags: Array.isArray(signal.flags) ? [...new Set(signal.flags)] : [],
      canonicalEdge: {
        from,
        relation,
        to,
...
function buildResearchCandidate({ workspaceId, provider, source, claim, signals }) {
  ...
  const conflict = {
    ...
    signals,
    oppositions: oppositionLinks(signals, built.provenance),
