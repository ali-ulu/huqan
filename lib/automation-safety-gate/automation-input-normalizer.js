...
function normalizeAutomationSafetyInput(input) {
  ...
  // #2024: an upstream gate that saw the caller's full payload can report a
  // secret it detected there. AB5 only ever sees the projected operation, so
  // without this channel a nested secret silently disappears. The signal is a
  // boolean on purpose: the detecting gate must not copy the value forward.
  const secretSignal = raw.__secretSignal === true;
