...
function summarizeOutcomes(subclaimOutcomes = []) {
  ...
  const supported = list.filter(item => item.status === 'verified').length;
  const contradicted = list.filter(item => item.status === 'contradicted').length;
  const unknown = list.filter(item => item.status === 'unknown').length;
...
function aggregateSubclaimVerdicts(subclaims = [], opts = {}) {
  const list = normalizeArray(subclaims).map(item => ({
    ...
    status: ['verified', 'contradicted', 'unknown'].includes(item.status) ? item.status : 'unknown',
  ...
  const contradicted = requiredList.filter(item => item.status === 'contradicted');
  const supported = requiredList.filter(item => item.status === 'verified');
  const unknown = requiredList.filter(item => item.status === 'unknown');
  ...
  if (status !== 'verified' && supported.length > 0 && supported.length === requiredList.length && confidence < floor) {
...
function buildReasoningTrace(input = {}, opts = {}) {
  ...
  const contradictionCount = subclaimOutcomes.filter(item => item.status === 'contradicted').length;
  ...
  return {
    ...
    trustReceiptPreview: {
      ...
      downgradeReasons,
      semanticFlags,
      canonical: aggregate.status === 'verified',
    },
  };
}
