...
const GITHUB_APP_BETA_TO_CANONICAL = Object.freeze({
  beta_observation_only: 'review',
});

const CODE_CHANGE_TO_CANONICAL = Object.freeze({
  allow: 'allow',
  review: 'review',
  block: 'block',
  dry_run_only: 'dry_run_only',
...
function toCanonicalVerdict(sourceLayer, sourceDecisionValue) {
  const table = sourceLayer === 'admission' ? ADMISSION_TO_CANONICAL
    : sourceLayer === 'mcp' ? MCP_TO_CANONICAL
      : sourceLayer === 'github_app_beta' ? GITHUB_APP_BETA_TO_CANONICAL
        : sourceLayer === 'code_change' ? CODE_CHANGE_TO_CANONICAL
          : null;
  if (!table) {
    throw new UnknownVerdictSourceError(sourceLayer, sourceDecisionValue);
  }
  // Own-property, and a string: `Object.freeze` leaves the prototype chain
  // intact, so `table['constructor']` answered with an inherited function --
  // truthy, so the fail-closed branch below never ran and a *function* was
  // returned as the canonical verdict. Downstream that is worse than a throw:
  // the envelope is frozen with a function in `verdict`, `classifyForUi`
  // compares it against strings and loses, and `JSON.stringify` drops the key
  // entirely, so the serialized record carries no verdict at all while still
  // looking well-formed (#1323).
  const canonical = typeof sourceDecisionValue === 'string'
    && Object.hasOwn(table, sourceDecisionValue)
    ? table[sourceDecisionValue]
    : undefined;
  if (!canonical) {
