...
function continuationData(result, request, checkpoint) {
  ...
  return {
    ...
    repairDecision: request.mode === 'repair' ? 'allow' : null,
  };
}
...
function executeMcpAgentContinuation(agent, args = {}) {
  ...
  try {
    ...
    return fail('AGENT_CHECKPOINT_LOOKUP_FAILED', 'Agent checkpoint lookup failed.', {
      operation: 'loadLatestCheckpoint',
  ...
  if (result && typeof result.then === 'function') {
    return result.then((resolved) => ({
      ...resolved,
      data: continuationData(resolved, { workspaceId, mode }, checkpoint),
    }));
  }
  return {
    ...result,
    data: continuationData(result, { workspaceId, mode }, checkpoint),
  };
}
