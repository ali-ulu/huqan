...
function executeMcpAgentContinuation(agent, args = {}) {
  ...
  // H-02: no repair policy exists downstream (agent.v3.js / agent.js never
  // read opts.mode or repairReason, repairDecision stays null), so a repair
  // request must fail closed here instead of silently running a normal
  // resume. agent.run() is never reached on this path.
  if (mode === 'repair') {
    return fail('AGENT_REPAIR_UNSUPPORTED', 'Repair mode is not supported by this continuation surface; no resume was performed.', {
      retrySafe: false,
      workspaceId,
    });
  }
