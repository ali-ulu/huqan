...
function defaultExternalActionPolicyPath(environment = process.env, workspaceId) {
  const override = typeof environment.HUQAN_EXTERNAL_GUARD_POLICY === 'string'
    ? environment.HUQAN_EXTERNAL_GUARD_POLICY.trim()
    : '';
  if (override) return path.resolve(override);
  const root = path.dirname(defaultExternalActionReceiptPath(environment));
  if (workspaceId === undefined) return path.join(root, 'external-action-policy.json');
  const scope = encodeURIComponent(String(workspaceId).trim() || 'default');
  return path.join(root, 'workspaces', scope, 'external-action-policy.json');
}
