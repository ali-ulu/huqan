...
/**
 * #1989 (H-19): the v1 verifier resolves multi-word subjects itself
 * ('kedi vahsi bir hayvan degildir' -> subject 'kedi') and can report a real
 * contradiction with evidence. Returns that base verdict so the v2
 * multi-word guard can keep its verdict, confidence and evidence instead of
 * dropping everything to unknown/0/[], and null otherwise so the guard falls
 * back to unknown.
 */
function contradictedBaseVerdict(kernel, statement, opts) {
  const base = kernel.verify(statement, opts);
  return base?.data?.status === 'contradicted' ? base : null;
}

module.exports = { collectTypeTargetsAsFacts, buildNegationConflict, contradictedBaseVerdict };
