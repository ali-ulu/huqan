 ...
 *
 * The two cost figures above are design-phase measurements. The *run profiles*
 * are now measured for real — see the next section — and the profiles in this
 * paragraph are kept only to explain how the ceiling was sized.
 *
 ...
 *
 * ## Real integrated runs (decision 1 and 3)
 *
 * Measured by driving the production factory — `createAgent` with a real
 * `HuqanStorage` and the journal it builds on it — not by appending synthetic
 * events. Median of three profiles, Node 24.21.0, 2026-09-25:
 *
 *   profile                events/run   ms/event (median)   p99      added wall-clock
 *   1-step                 4            0.122               0.483     5.7%
 *   10-step                22           0.094               0.335    16.5%
 *   10-step + 3 repairs    28           0.082               0.343    27.4%
 *
 * The added wall-clock rises with the number of steps because the run's own
 * work is mostly in-memory while every append is a synchronous commit; the
 * 10-step profiles sit near 1 ms of journal for ~14 ms of run. All three are
 * two orders of magnitude below `MAX_MS_PER_EVENT`, and the 27.4% worst case is
 * on a run short enough that the fixed cost dominates — a longer run amortises
 * it, which is why the percentage is not the figure to guard.
 *
 * `test/experience-write-cost-budget.test.js` drives these same three profiles
 * and pins the event counts, so the numbers above and the runs cannot drift
 * apart again.
 *
