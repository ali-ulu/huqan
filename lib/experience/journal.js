 ...
 *
 * The journal receives a store (DIP #2118 — it never constructs one) and uses
 * `store.db` when present; `storage.js` keeps owning SQLite and its
 * `withTransaction` seam, so no second persistence engine is introduced. Crash
 * reconciliation stays a separate delivery (#2399); this file only guarantees
 * that what was acknowledged is on disk and untorn.
 *
 * In production that store is the journal's own EVIDENCE connection
 * (`./journal-connection.js`), not `storage.db` (#2915 — see that file).
 *
...
function createExperienceJournal({ store } = {}) {
  ...
  const useTx = store && typeof store.withTransaction === 'function'
    ...
    : (fn) => fn();
  // Per use, not once: a reopen after restore (#1848) must not leave it stale.
  const liveDb = () => (store && store.db ? store.db : null);
  if (liveDb()) ensureTable(liveDb());
  ...
  const rebuildDb = liveDb();
  if (rebuildDb) {
    const rows = rebuildDb.prepare(`SELECT run_id, body, hash FROM ${TABLE} ORDER BY run_id, sequence`).all();
