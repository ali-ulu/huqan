...
// Strict variant for enforcement reads: an unreadable ledger must hold the
// action for review, never read as an empty (clean) budget.
function readBudgetRowsStrict(graph) {
  const rows = graph.getCommittedMutationResultsByPrefix(BUDGET_OPERATION_PREFIX);
  if (!Array.isArray(rows)) throw new Error('budget ledger unreadable');
  return rows.filter((row) => row && row.result && row.result.budget === true);
}
...
function readBudgetState(graph, scope) {
  ...
  const rows = readBudgetRowsStrict(graph);
  let reserved = 0;
  let committed = 0;
...
function reserveImpact(graph, { scope, amount: units, idempotencyKey: keyText, actionDigest = null } = {}) {
  ...
  try {
    ...
    // The row is written; only the read-back failed. Report unknown totals
    // rather than throwing away a recorded reservation or inventing zeros.
  }
  return Object.freeze({ replayed: Boolean(outcome.replayed), reservationId, reserved, committed });
}
