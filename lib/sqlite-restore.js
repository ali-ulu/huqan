...
function closeRestoreHandles({ kernel, storage }) {
  ...
  // The journal's own EVIDENCE connection points at the same memory.db (#2915),
  // so it has to be closed here too or Windows refuses the rename (EPERM).
  if (kernel?.experienceJournalConnection) kernel.experienceJournalConnection.close();
...
function reopenRestoreHandles({ kernel, storage, storageOpen }) {
  ...
  // The journal handle was closed with the others; reopen it at the replaced
  // file. Its journal reads the handle per use, so no journal rebuild is needed.
  if (kernel?.experienceJournalConnection) kernel.experienceJournalConnection.reopen();
}
