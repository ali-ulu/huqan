...
function readIngestRunStatus(kernel, args = {}, runtime = {}) {
  ...
  if (!runId || !workspaceId) {
    return kernel.fail(OPERATION, 'INVALID_INPUT', 'runId and workspaceId are required.');
  }
  ...
  if (!store || typeof store.getToolApprovalById !== 'function') {
    return kernel.fail(OPERATION, 'APPROVAL_STORE_UNAVAILABLE', 'Persistent approval store is unavailable.');
  }
  ...
  if (!approval || approval.tool !== 'http.ingest' || approvalWorkspace(approval) !== workspaceId) {
    return kernel.fail(OPERATION, 'INGEST_RUN_NOT_FOUND', 'Ingest run was not found in this workspace.');
  }

  const run = buildIngestWorkflowRun(approval);
  if (!run) {
    return kernel.fail(OPERATION, 'INGEST_RUN_STATE_UNKNOWN', 'Ingest run state cannot be projected safely.');
  }
  return kernel.ok(OPERATION, { ...run, workspaceId });
}
