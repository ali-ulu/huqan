...
const DEFAULT_GITHUB_APP_TOKEN_TIMEOUT_MS = 15_000;
...
async function createInstallationAccessToken({
  ...
  const effectiveTimeoutMs = Number.isSafeInteger(timeoutMs) && timeoutMs > 0 ? timeoutMs : DEFAULT_GITHUB_APP_TOKEN_TIMEOUT_MS;
  let response;
  try {
