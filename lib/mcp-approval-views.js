...
function listPersistentApprovals(approvalStore, limit = 50, workspaceId = 'default') {
  if (!approvalStore) return [];
  const list = typeof approvalStore.listUnresolvedToolApprovals === 'function'
    ? approvalStore.listUnresolvedToolApprovals(limit, workspaceId)
    : typeof approvalStore.listPendingToolApprovals === 'function'
      ? approvalStore.listPendingToolApprovals(limit, workspaceId)
      : [];
  return list
...
function countPersistentApprovals(approvalStore, workspaceId = 'default') {
  if (!approvalStore || typeof approvalStore.countPendingToolApprovals !== 'function') return 0;
  return approvalStore.countPendingToolApprovals(workspaceId);
}

function countUnresolvedApprovals(approvalStore, workspaceId = 'default') {
  if (!approvalStore || typeof approvalStore.countUnresolvedToolApprovals !== 'function') {
    return countPersistentApprovals(approvalStore, workspaceId);
  }
  return approvalStore.countUnresolvedToolApprovals(workspaceId);
}
