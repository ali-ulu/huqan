...
// #1643 follow-up: JS literal leakage. Ingested code/JSON content turns
// runtime values into graph nodes ("undefined", "null", "true"). These pass
// the lexical-content gate below because they are pure-letter words of decent
// length, yet they never name a concept. Block them explicitly (case
// differs across sources, so compare lowercase).
const JS_LITERAL_NOISE = new Set(['undefined', 'null', 'true', 'false', 'nan', 'none']);

function isEligibleHypothesisNode(label) {
  const s = String(label ?? '').trim();
  if (s.length < MIN_LABEL_LENGTH) return false;
  if (JS_LITERAL_NOISE.has(s.toLowerCase())) return false;
