...
/**
 * `admission` is optional and receiver-owned, injected only so tests can pin
 * the seam. Widening the exact-shape check to admit it does not weaken what the
 * check was for: any key other than these two is still refused, and `graph` is
 * still mandatory.
 */
const DEPENDENCY_KEYS = Object.freeze(['graph', 'admission']);
...
function admissionDependency(options) {
  ...
  const injected = descriptor?.enumerable && Object.hasOwn(descriptor, 'value')
    ? descriptor.value
    : null;
  if (injected === null || injected === undefined) return createMutationAdmission();
...
function commitExternalClientCandidateClaim(pkgInput, contextInput, options = {}) {
  ...
  const admission = admissionDependency(options);
