...
function parseCommand(input, kernel) {
  ...
  if (isPrefix('why')) return { command: 'neden', args: prefixPayload };
  if (isPrefix('compare')) return { command: 'karşılaştır', args: normalizeCompareArgs(prefixPayload) };
  ...
  if (['acik dusun', 'surekli dusun', 'otomatik dusun', 'auto think', 'dusunmeye basla', 'think', 'start thinking'].includes(plain)) return { command: 'düşün', args: 'başla' };
  ...
  if (['birlestir', 'konsolide et', 'toparla', 'consolidate'].includes(plain)) return { command: 'konsolide', args: '' };
