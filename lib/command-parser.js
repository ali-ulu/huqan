...
function parseCommand(input, kernel) {
  ...
  if (isPrefix('audit')) return { command: 'audit', args: prefixPayload };
  ...
  if (plain === 'doctor') return { command: 'doctor', args: '' };
  ...
  const approvalListMatch = trimmed.match(/^(onaylar|approvals)\s+--workspace\s+(\S+)$/i);
  if (approvalListMatch) return { command: 'onaylar', args: { workspaceId: approvalListMatch[2] } };
  ...
  const auditMatch = trimmed.match(/^audit\s*(.*)$/i);
  if (auditMatch) return { command: 'audit', args: (auditMatch[1] || '').trim() };

  // `coder <task.json> [flags]`. Bare form only: the task file is a path, and a
  // `coder:` prefix would read as though the payload were prose.
  //
  // Matched against `raw`, not `trimmed`. Every other command here reads prose
  // or flags, where folding case is harmless; this one carries file paths and
  // git refs, where it is not. `--base HEAD` became `--base head` and a path
  // with a capital in it became a different path -- invisible on Windows,
  // where the filesystem is case-insensitive, and broken on CI, where it is
  // not. The command word itself stays case-insensitive via the regex flag.
  const coderMatch = raw.match(/^coder\s+(.+)$/i);
  if (coderMatch) return { command: 'coder', args: coderMatch[1].trim() };
