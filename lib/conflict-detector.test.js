...
function routeKernelCandidate(kernel, claim, opts = {}) {
  ...
  return routeKernelCandidate(kernel, claim, opts, {
...
test('detectClaimConflict flags a contradictory edge even when provenance matches', () => {
  const graph = new Graph({ noLoad: true, useSQLite: false, ...makePaths('same-source-conflict') });
  const provenance = seedCausalEdge(graph);
...
test('routeCandidateClaim enforces strict provenance before writing anything', () => {
  ...
  assert.throws(() => routeKernelCandidate(kernel, {
    claim: 'kedi hayvandir',
    subject: 'kedi',
    relation: 'tür',
    object: 'hayvan',
