...
class VerifyService {
  ...
  verify(statement, opts = {}) {
    ...
    if (directEdge) {
      ...
      if (incomingAbsolute) {
        ...
        return this._verifyResult(statement, opts, { status: 'verified', confidence }, [this.host.edgeEvidence(directEdge, 'direct_edge', confidence)], { ...verifyContext, directEdge });
      }
    }
    ...
    if (target !== subject) {
      ...
      if (pathResult.path && !incomingAbsolute) {
        ...
        return this._verifyResult(statement, opts, { status: 'verified', confidence }, [this.host.pathEvidence(pathResult.path, 'path', confidence)], { ...verifyContext, pathSearch: pathResult });
      }
    }
    ...
    for (const word of parts.slice(1)) {
      ...
      if (match) {
        ...
        return this._verifyResult(statement, opts, { status: 'verified', confidence: 0.35 }, [this.host.edgeEvidence(match, 'partial_match', 0.35)], { ...verifyContext, fuzzy });
      }
    }
  ...
  contradictionEvidence(contradiction) {
    ...
    const edges = Array.isArray(contradiction.edges)
      ? contradiction.edges.map(edge => this.host.edgeRef(edge))
