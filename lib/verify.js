...
class VerifyService {
  ...
  _verifyResult(statement, opts, data, evidence, context = {}) {
    ...
    return this.kernel._ok('verify', nextData, resolvedEvidence, attachRobustnessMeta(this, {
  ...
  detectContradictions(subject = '', workspaceId = 'default') {
    ...
    for (const node of allNodes) {
      ...
      const numeric = findNumericContradictions(edges, node.id, {
        extractNumbers: value => this._extractNumbers(value),
        getTextCore: value => this._getTextCore(value),
      });
