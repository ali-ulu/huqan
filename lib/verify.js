...
const {
  foldTurkishAscii, stripCopulaSuffix, phraseMatches,
  sharedPrefixLength, hasSharedSemanticAnchor,
  isPreventRelation, normalizeNegationTarget, normalizeForVerify,
...
const { enforceEvidencedContradiction, buildContradictionEvidence } = require("./verify-contradiction-evidence");
...
class VerifyService {
  ...
  verify(statement, opts = {}) {
    ...
    if (decomposition.compound && !opts.skipDecomposition) {
      ...
      if (traceDepth >= maxDepth) {
        return this.verifyResult(statement, opts, { status: 'unknown', confidence: 0 }, [], {
          workspaceId,
          decomposition,
      ...
      const subclaimResults = decomposition.subclaims.map((subclaim) => this.host.verifyInternal(subclaim.claim, {
        ...opts,
        workspaceId,
    ...
    const predicateNumericComparison = this.parseNumericComparison(predicate);
    ...
    if (negMatch) {
      ...
      if (positive) {
        ...
        if (posEdge) {
          return this.verifyResult(statement, opts, { status: 'contradicted', confidence: 0.85 }, [{
            kind: 'contradiction',
    ...
    if (directEdge) {
      ...
      const incomingParsed = typeof this.kernel._parsePredicate === 'function'
        ? this.host.parsePredicate(predicate)
        : null;
      ...
      const causalConflict = buildCausalPreventionConflict(subject, directEdge, statement, incomingPrevents); if (causalConflict) return this.verifyResult(statement, opts, causalConflict.data, causalConflict.evidence, { ...verifyContext, directEdge });
      ...
      if (directEdge.relation === 'PREVENTS' && !incomingPrevents) {
        ...
        return this.verifyResult(statement, opts, { status: 'contradicted', confidence: pConfidence }, [{
          kind: 'contradiction',
      ...
      if (incomingAbsolute) {
        ...
        return this.verifyResult(statement, opts, { status: 'verified', confidence }, [this.host.edgeEvidence(directEdge, 'direct_edge', confidence)], { ...verifyContext, directEdge });
      }
    }
    ...
    const typeConflict = target && target !== subject
      ...
      : null;
    if (typeConflict) {
      return this.verifyResult(statement, opts, { status: 'contradicted', confidence: typeConflict.confidence || 0.9 }, typeConflict.evidence || [{
        kind: 'contradiction',
    ...
    const subjCons = cons.filter(c => c.node === subject).filter((c) => {
      ...
      return targets.some((t) => {
        const nt = normalizeForVerify(this.kernel, t);
    ...
    if (subjCons.length > 0) {
      const evidence = subjCons.map(c => this.contradictionEvidence(c));
      return this.verifyResult(statement, opts, { status: 'contradicted', confidence: 0.7 }, evidence, verifyContext);
    }

    if (target !== subject) {
      const pathResult = typeof this.kernel._findPathWithTimeout === 'function'
        ? this.host.findPathWithTimeout(subject, target, opts.pathTimeoutMs ?? 100, workspaceId, 4)
        : { path: this.host.findPath(subject, target, new Set(), [], 4, workspaceId), stoppedReason: null, timeoutMs: opts.pathTimeoutMs ?? 100, maxDepth: 4, workspaceId, visitedCount: 0 };
      if (pathResult.path && !incomingAbsolute) {
        const confidence = pathSupportConfidence(this.kernel.graph, pathResult.path, workspaceId);
        return this.verifyResult(statement, opts, { status: 'verified', confidence }, [this.host.pathEvidence(pathResult.path, 'path', confidence)], { ...verifyContext, pathSearch: pathResult });
      }
    }
      ...
      for (const edge of edges) {
        ...
        if (edgeNums) {
          ...
          if (mismatch) {
            ...
            if (hasTextOverlap) {
              return this.verifyResult(statement, opts, { status: 'contradicted', confidence: 0.75 }, [{
                kind: 'contradiction',
    ...
    for (const word of parts.slice(1)) {
      ...
      if (match) {
        ...
        if (contradictionSignals.length > 0) {
          ...
          return this.verifyResult(statement, opts, { status: 'contradicted', confidence: 0.75 }, evidence, { ...verifyContext, fuzzy });
        }
        if (incomingAbsolute || fuzzy.isWeak) continue;
        return this.verifyResult(statement, opts, { status: 'verified', confidence: 0.35 }, [this.host.edgeEvidence(match, 'partial_match', 0.35)], { ...verifyContext, fuzzy });
      }
    }
  ...
  verifyResult(statement, opts, data, evidence, context = {}) {
