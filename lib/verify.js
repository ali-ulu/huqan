  ...
  verify(statement, opts = {}) {
    ...
    if (directEdge) {
      ...
      const incomingParsed = typeof this.kernel._parsePredicate === 'function'
        ...
        : null;
      const incomingPrevents = incomingParsed && incomingParsed.relation
        ? isPreventRelation(incomingParsed.relation)
        : isPreventRelation(String(predicate).trim().split(/\s+/)[0] || '');
    ...
    for (const node of allNodes) {
      ...
      for (let leftIndex = 0; leftIndex < typeEdges.length; leftIndex += 1) {
        for (let rightIndex = leftIndex + 1; rightIndex < typeEdges.length; rightIndex += 1) {
          ...
          if (!pairMatchesDisjoint(left.to, right.to)) continue;
          contradictions.push({
            type: 'çoklu-tür',
            node: node.id,
