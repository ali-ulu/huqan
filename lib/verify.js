...
class VerifyService {
  ...
  verify(statement, opts = {}) {
    ...
    if (target !== subject) {
      const foundPath = this.kernel._findPath(subject, target, new Set(), [], 4, workspaceId);
      if (foundPath) {
        return this._verifyResult(statement, opts, { status: 'dogrulandi', confidence: 0.5 }, [this.kernel._pathEvidence(foundPath, 'path', 0.5)], { workspaceId, subject, predicate, edges });
      }
    }
    ...
    for (const word of parts.slice(1)) {
      ...
      if (match) {
        return this._verifyResult(statement, opts, { status: 'dogrulandi', confidence: 0.35 }, [this.kernel._edgeEvidence(match, 'partial_match', 0.35)], { workspaceId, subject, predicate, edges });
      }
    }
