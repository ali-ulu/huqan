...
const { fitnessHistoryPath, recordFitnessEntry } = require('./fitness-history');
...
function runCliHypothesesFitness(kernel, args, opts = {}) {
  ...
  let recorded = null;
  if (args.record === true) {
    recorded = recordFitnessEntry(fitnessHistoryPath(kernel), fitness);
  }
  if (opts.json === true || args.json === true) {
    return recorded ? { fitness, recorded } : { fitness };
  }
  const report = formatFitnessReport(fitness);
  return recorded ? `${report}\n  Kaydedildi: ${fitnessHistoryPath(kernel)}` : report;
}
