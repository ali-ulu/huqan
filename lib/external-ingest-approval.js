...
const MAX_EXTERNAL_APPROVAL_WINDOW_MS = 15 * 60 * 1000;
...
function requestMetadata(data = {}) {
  ...
  const windowMs = expiresAt.millis - requestedAt.millis;
  if (windowMs <= 0) {
    return fail('EXTERNAL_APPROVAL_WINDOW_INVALID', 'expiresAt must be later than requestedAt');
  }
  if (windowMs > MAX_EXTERNAL_APPROVAL_WINDOW_MS) {
    return fail('EXTERNAL_APPROVAL_WINDOW_TOO_LONG', 'external approval validity window may not exceed 15 minutes');
  }
  return {
    ok: true,
...
function toReviewedSource(snapshot) {
  ...
  else source.path = snapshot.path;
