...
const DEFAULT_CLI_INGEST_TIMEOUT_MS = 15_000;

async function requestJson(fetchImpl, url, options, timeoutMs = DEFAULT_CLI_INGEST_TIMEOUT_MS) {
  const effectiveTimeoutMs = Number.isSafeInteger(timeoutMs) && timeoutMs > 0 ? timeoutMs : DEFAULT_CLI_INGEST_TIMEOUT_MS;
  const callerSignal = options && options.signal;
  const timeoutSignal = AbortSignal.timeout(effectiveTimeoutMs);
  const signal = callerSignal ? AbortSignal.any([callerSignal, timeoutSignal]) : timeoutSignal;
  const optionsWithSignal = { ...options, signal };
  try {
    const response = await fetchImpl(url, optionsWithSignal);
...
module.exports = { MAX_BATCH_ITEMS, DEFAULT_CLI_INGEST_TIMEOUT_MS, aggregateStatus, previewItem, runIngestBatch };
