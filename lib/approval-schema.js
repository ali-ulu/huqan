...
function isApprovalObject(value) {
...
function makeApprovalId(request) {
  ...
  // a 64-bit id has a ~2^32 birthday bound, reachable at production approval
  // volume; 128 bits keeps collision risk negligible (#385).
  return `apr_${crypto.createHash('sha256').update(basis, 'utf8').digest('hex').slice(0, 32)}`;
}
...
function normalizeApprovalRequest(request = {}, opts = {}) {
  const source = isApprovalObject(request) ? request : {};
