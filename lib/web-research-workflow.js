...
const { openExternalResearchCandidates, verifyResearchSummary } = require('./web-research-candidate-pipeline');
...
async function runWebResearchWorkflow(input, options) {
  ...
  try {
    ...
    const summaryVerification = verifyResearchSummary(opts.kernel, result, { workspaceId: input.workspaceId });
    const candidatePipeline = input.openCandidates === true
      ? openExternalResearchCandidates(opts.kernel, result, { workspaceId: input.workspaceId })
      : null;
    return {
      statusCode: 200,
      body: workflowEnvelope({
        ...
        data: {
          ...
          ...(summaryVerification ? { summaryVerification } : {}),
    ...
    if (error && error.code === 'RESEARCH_CANDIDATE_PIPELINE_UNAVAILABLE') {
      return failure(503, error.code, 'Research candidate pipeline is unavailable.');
    }
    if (error && error.code === 'MUTATION_ADMISSION_REFUSED') {
      return failure(403, 'RESEARCH_CANDIDATE_PIPELINE_BLOCKED', 'Research candidate admission was blocked.');
    }
