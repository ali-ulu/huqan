...
function createGitHubAppStreamingTrustStore({ rootPath }) {
  ...
  function readEvaluation(deliveryId) {
    ...
    if (record.schemaVersion !== STORE_VERSION || !receiptMatchesBinding(record.receipt, binding)) {
  ...
  function readWriteback(deliveryId) {
    ...
    if (complete) return Object.freeze({ state: 'complete', ...complete });
    const started = readJsonFile(fileFor(startedDir, deliveryId));
    if (started) return Object.freeze({ state: 'started', ...started });
