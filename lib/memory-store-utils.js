 ...
 *
 * Appending when there is no extension to replace is what `jsonJournalPath()`
 ...
 *
 * Deriving each role separately is how they drifted apart: `jsonJournalPath()`
...
function resolveDbPath(opts = {}) {
  ...
  // #1579: the same default-path rule as the graph's. Without it MemoryStore
  // derived its own cwd-relative `memory.db`, which under the test runner is
  // the shared repository root -- a second route to the pollution the graph's
  ...
  // The second of the three default-path sources, and the one that produced the
  // stray this guard exists for: the store left in %LOCALAPPDATA%\huqan held
  // checkpoints, goal_memory, agent_runs and tool_approvals -- MemoryStore's
  // tables, with no graph and no audit log. Guarding only the graph's route
  // would have left the one that actually strays wide open.
...
function resolveContainedPath(candidate, allowedRoots = []) {
  ...
  const roots = allowedRoots
    ...
    // A candidate spelled through a root's real path (macOS /var -> /private/var) is inside it.
...
// Keeps withTransaction synchronous (no Promise / setTimeout).
