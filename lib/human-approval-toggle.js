...
/**
 * The agent-loop classification still carries an alpha `dry_run_only`
 * fallback. AB1/AB2 can independently require human review before that
 * fallback is applied; the old merge priority hid that stronger concrete
 * decision and made a reviewed agent call impossible to approve later.
 *
 * Preserve the alpha dry-run only when no concrete gate asked for review.
 * When a gate did ask for review, restore the review decision so the normal
 * MCP durable approval path can persist it. This does not execute anything
 * and does not ignore a block -- blocked calls never arrive here as
 * `dry_run_only`.
 */
function restoreAgentReview(gate) {
  if (!gate
  ...
  const reviewFinding = Array.isArray(gate.findings)
    ? gate.findings.find((finding) => {
      const decision = String(finding?.decision || '').toLowerCase();
      return ['review', 'human_review', 'quarantine'].includes(decision);
    })
    : null;
  if (!reviewFinding) return gate;
  ...
  return {
    ...
    requiredReview: true,
    dryRunOnly: false,
    reason: 'agent_loop_requires_review',
    metadata: {
      ...gate.metadata,
      agentReviewRestored: true,
      reviewGate: reviewFinding.gate || null,
...
function applyHumanApprovalToggle(gate, env = process.env) {
  const runtimeGate = restoreAgentReview(gate);
