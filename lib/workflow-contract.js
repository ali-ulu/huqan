...
const WORKFLOW_CAPABILITIES = Object.freeze([
  ...
  workflow({ workflowId: 'ingest-execute', route: '/api/v2/ingest/execute', method: 'POST', bodyMaxBytes: 1048576, mutation: true, approvalRequired: true, availability: { api: true, cli: true } }),
  workflow({ workflowId: 'ingest-run-detail', route: '/api/v2/ingest/runs/{id}', method: 'GET', bodyMaxBytes: 4096, availability: { api: true, cli: true } }),
  workflow({ workflowId: 'agent-plan', route: '/api/v2/agent/plan', method: 'POST', mcpTool: 'huqan.plan', availability: { api: true, cli: true } }),
  workflow({ workflowId: 'agent-run', route: '/api/v2/agent/runs', method: 'POST', mcpTool: 'huqan.agent', availability: { api: true, cli: true } }),
  ...
  workflow({ workflowId: 'auto-think', mutation: true, capabilityClass: 'operator', approvalRequired: true, availability: { cli: true } }),
  workflow({ workflowId: 'optimize', mutation: true, capabilityClass: 'operator', approvalRequired: true, availability: { cli: true } }),
  workflow({ workflowId: 'consolidate', mutation: true, capabilityClass: 'operator', approvalRequired: true, availability: { cli: true } }),
  workflow({ workflowId: 'evolve', mutation: true, capabilityClass: 'admin', approvalRequired: true, availability: { cli: true } }),
]);
...
const CLI_COMMAND_CAPABILITIES = Object.freeze([
  ...
  ['restore', 'restore', 'restore [--dry-run] [yol]', 'I preview or restore a backup', ['geri yükle', 'geri yukle']],
  ...
  ['receipt', 'trust-receipt-detail', 'receipt <id> [--workspace <id>]', 'I read an already materialized Trust Receipt', ['trust-receipt']],
  ['llm-sor', 'recommendation', 'llm-sor: soru', 'I prepare an LLM recommendation', []],
  ['düşün', 'auto-think', 'düşün', 'operator-gated background thinking', []],
  ['optimize', 'optimize', 'optimize', 'operator-gated memory optimization', []],
  ['konsolide', 'consolidate', 'konsolide', 'operator-gated contradiction cleanup', []],
  ['evolve', 'evolve', 'evolve', 'admin-gated self evolution', []],
