...
const WORKFLOW_CAPABILITIES = Object.freeze([
  workflow({ workflowId: 'ask', route: '/api/v2/workflows/ask', method: 'POST', mcpTool: 'huqan.ask', compatibilityCommand: 'sor', availability: { cli: true } }),
  workflow({ workflowId: 'verify', route: '/v2/verify', method: 'POST', mcpTool: 'huqan.verify', workspaceRequired: false, availability: { api: true, cli: true } }),
  workflow({ workflowId: 'advocate', route: '/api/v2/workflows/advocate', method: 'POST' }),
  ...
  workflow({ workflowId: 'memory-search', route: '/api/v2/workflows/search', method: 'POST' }),
  ...
  workflow({ workflowId: 'trust-receipt', route: '/api/trust-receipt/{id}', method: 'GET', availability: { api: true } }),
]);
