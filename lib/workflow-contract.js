...
const HTTP_SCHEMAS = Object.freeze({
  ...
  'web-research': { type: 'object', required: ['workspaceId', 'provider', 'query'], properties: { workspaceId: { type: 'string', minLength: 1, maxLength: 128 }, provider: { enum: ['brave', 'firecrawl', 'tavily'] }, query: { type: 'string', minLength: 1, maxLength: 1000 }, limit: { type: 'integer', minimum: 1, maximum: 10 }, maxSnippet: { type: 'integer', minimum: 500, maximum: 8000 }, summarize: { type: 'boolean' }, openCandidates: { type: 'boolean' } }, additionalProperties: false },
...
const WORKFLOW_CAPABILITIES = Object.freeze([
  workflow({ workflowId: 'web-research', route: '/api/v2/workflows/research', method: 'POST', mcpTool: 'huqan.web_research', mutation: true, availability: { api: true, ui: true } }),
  ...
  workflow({ workflowId: 'learn-review', route: '/api/v2/workflows/learn', method: 'POST', bodyMaxBytes: 1048576, mcpTool: 'huqan.learn', mutation: true, approvalRequired: true, availability: { api: true, cli: true, ui: true } }),
  ...
  workflow({ workflowId: 'approval-detail', route: '/api/v2/approvals/{id}', method: 'GET', mcpTool: 'huqan.approval_detail', bodyMaxBytes: 4096, availability: { api: true, mcp: true, ui: true } }),
  workflow({ workflowId: 'approval-decision', route: '/api/v2/approvals/{id}/decision', method: 'POST', mcpTool: 'huqan.approve', bodyMaxBytes: 4096, mutation: true, approvalRequired: true, availability: { api: true, cli: true, ui: true } }),
  ...
  workflow({ workflowId: 'ingest-preview', route: '/api/v2/ingest/preview', method: 'POST', mcpTool: 'huqan.ingest_preview', availability: { api: true, cli: true, ui: true } }),
  workflow({ workflowId: 'ingest-execute', route: '/api/v2/ingest/execute', method: 'POST', mcpTool: 'huqan.ingest_execute', bodyMaxBytes: 1048576, mutation: true, approvalRequired: true, availability: { api: true, cli: true, mcp: true, ui: true } }),
  workflow({ workflowId: 'ingest-run-detail', route: '/api/v2/ingest/runs/{id}', method: 'GET', mcpTool: 'huqan.ingest_status', bodyMaxBytes: 4096, availability: { api: true, cli: true, mcp: true, ui: true } }),
  ...
  workflow({ workflowId: 'trust-receipt-detail', route: '/api/v2/trust-receipts/{id}', method: 'GET', mcpTool: 'huqan.trust_receipt_detail', availability: { api: true, cli: true, mcp: true, ui: true } }),
  workflow({ workflowId: 'compliance-audit', mcpTool: 'huqan.audit', mutation: false, availability: { cli: true, mcp: true } }),
  // Writes files, so `mutation: true`. It deliberately does not carry
  // approvalRequired: its authorization is lib/code-change-gate.js, evaluated
  // per patch against the actual paths being touched. A blanket approval
  // prompt in front of that would be a second, weaker gate asking a coarser
  // question than the one that already governs the write.
  workflow({ workflowId: 'deterministic-coder', mutation: true, workspaceRequired: false, availability: { cli: true } }),
  workflow({ workflowId: 'quickstart', workspaceRequired: false, availability: { cli: true } }),
  workflow({ workflowId: 'system-status', mcpTool: 'huqan.status', mutation: false, availability: { cli: true, mcp: true } }),
  ...
  // #2505 F: the operator emergency stop. Its HTTP route is deployment-gated
  // (lib/http/emergency-stop-routes.js) and declared outside this contract,
  // like memory approvals.
  workflow({ workflowId: 'emergency-stop', mcpTool: 'huqan.emergency_stop', mutation: true, capabilityClass: 'operator', availability: { cli: true, mcp: true } }),
  ...
  // The `exposureBlockedBy` marker is load-bearing, not a comment: the
  // foundation contract test requires every capability with no reachable
  // surface to carry one, so a capacity audit reads intention rather than
  // drift (#1880). Surfacing them without the approval workflow would open
  // unaudited operator/admin mutations, so the fix is the marker, not a route.
  workflow({ workflowId: 'auto-think', mutation: true, capabilityClass: 'operator', approvalRequired: true, exposureBlockedBy: 'durable approval/execution workflow' }),
  workflow({ workflowId: 'optimize', mutation: true, capabilityClass: 'operator', approvalRequired: true, exposureBlockedBy: 'durable approval/execution workflow' }),
  workflow({ workflowId: 'consolidate', mutation: true, capabilityClass: 'operator', approvalRequired: true, exposureBlockedBy: 'durable approval/execution workflow' }),
  workflow({ workflowId: 'evolve', mutation: true, capabilityClass: 'admin', approvalRequired: true, exposureBlockedBy: 'durable approval/execution workflow' }),
]);
...
const CLI_COMMAND_CAPABILITIES = Object.freeze([
  ...
  ['doctor', 'system-status', 'doctor [--json]', 'run comprehensive local health checks', []],
  ...
  ['stop', 'emergency-stop', 'stop --scope agent|workspace --workspace <id> [--agent <id>] --reason "<text>"', 'emergency-stop an agent, or every agent and MCP call in a workspace', []],
  ['lift', 'emergency-stop', 'lift --scope agent|workspace --workspace <id> [--agent <id>]', 'lift an emergency stop', []],
  ['integrity', 'emergency-stop', 'integrity [--workspace <id>] [--notify]', 'verify the emergency-stop ledger and optionally sound the siren', []],
  ...
  ['audit', 'compliance-audit', 'audit [--eu-ai-act] [--workspace <id>]', 'EU AI Act compliance report from the receipt chain', ['denetim']],
  ['coder', 'deterministic-coder', 'coder <task.json> [--dry-run] [--root <dir>]', 'run a deterministic code transform through the code-change gate', []],
