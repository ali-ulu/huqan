...
const { text } = require('./external-action-identity-card');
...
function evaluateHookInvocation(profile, payload, options = {}) {
  ...
  const invocation = options.identityCard
    ...
    : normalized;
  const result = evaluateExternalAction(invocation, taskHookOptions(payload, options));
  return { invocation, result, projection: projectHookDecision(profile, result) };
}

/**
 * #2505 C: resolve the task a task-scoped card binds against. The
 * deployment's explicit option owns the binding; the host-reported payload
 * task is only a fallback and never overrides it — a host asserting its own
 * task has the same standing as the session.id it already reports. Bounded
 * by text(), like every other host-reported field. Returns the options to
 * evaluate with: untouched when no task is known anywhere.
 */
function taskHookOptions(payload, options) {
  const explicit = text(options.taskId);
  if (explicit) return explicit === options.taskId ? options : { ...options, taskId: explicit };
  const reported = text(payload.task_id) || text(payload.taskId)
    || text(payload.input && payload.input.task_id) || text(payload.input && payload.input.taskId);
  return reported ? { ...options, taskId: reported } : options;
}
...
function inProcessGuardOptions(base, options) {
  ...
  const policy = require('./external-action-command-policy');
  const target = options.policyPath || policy.defaultExternalActionPolicyPath(process.env, options.workspaceId || 'default');
  return { ...base, allowedCommands: policy.readAllowedCommands(target) };
}
