...
function makeKernel(overrides = {}) {
  ...
  const kernel = {
    verify(text) {
      ...
      return { data: { status: 'unknown', confidence: 0 } };
    },
...
test('normalizeCheck reads envelope data', () => {
  const normalized = normalizeCheck({ data: { status: 'verified', confidence: 0.8 } });
...
test('classifyLlmSor returns graph-backed, llm-assisted, unsupported, contradicted', () => {
  assert.strictEqual(
    classifyLlmSor({ data: { status: 'verified', confidence: 0.9 } }, { data: { status: 'verified', confidence: 0.7 } }),
    'graph-backed',
  );
  assert.strictEqual(
    classifyLlmSor({ data: { status: 'verified', confidence: 0.9 } }, { data: { status: 'unknown', confidence: 0 } }),
    'llm-assisted',
  );
  assert.strictEqual(
    classifyLlmSor({ data: { status: 'unknown', confidence: 0 } }, { data: { status: 'unknown', confidence: 0 } }),
    'unsupported',
  );
  assert.strictEqual(
    classifyLlmSor({ data: { status: 'contradicted', confidence: 0.9 } }, { data: { status: 'verified', confidence: 0.7 } }),
    'contradicted',
  );
});
...
test('evaluateLlmSor keeps autoLearn off by default', () => {
  const { kernel, learnCalls, verifyCalls, getSaveCalls } = makeKernel({
    verify(text) {
      if (text.includes('question')) return { data: { status: 'verified', confidence: 0.9 } };
      return { data: { status: 'verified', confidence: 0.8 } };
    },
  });
  ...
  const result = evaluateLlmSor({
    ...
    question: 'question',
    llmText: 'answer',
    axiomCheck: { data: { status: 'verified', confidence: 0.9 } },
    llmCheck: { data: { status: 'verified', confidence: 0.8 } },
  });
...
test('evaluateLlmSor marks partialVerification when llmText exceeds 300 chars', () => {
  ...
  assert.strictEqual('verifiedPrefix' in result.partialVerification, false, 'stale verifiedPrefix key must not resurface -- see #1296');
});
...
test('evaluateLlmSor does not treat safe prefix as safe when contradiction appears after 300 chars', () => {
  ...
  const { kernel, verifyCalls } = makeKernel({
    verify(text) {
      if (text === 'question') {
        return { data: { status: 'verified', confidence: 0.95 } };
      }
      if (text.includes('celiski var')) {
        return { data: { status: 'contradicted', confidence: 0.95 } };
      }
      return { data: { status: 'verified', confidence: 0.8 } };
    },
  });
  ...
  assert.strictEqual(result.llmCheck.status, 'contradicted');
...
test('evaluateLlmSor learns only when explicitly enabled and never for unsupported or contradicted', () => {
  const graphBacked = makeKernel({
    verify(text) {
      if (text.includes('question')) return { data: { status: 'verified', confidence: 0.95 } };
      return { data: { status: 'verified', confidence: 0.75 } };
    },
  });
  ...
  assert.strictEqual(graphBacked.getSaveCalls(), 0);
  ...
  const unsupportedResult = evaluateLlmSor({
    ...
    question: 'question',
    llmText: 'answer',
    axiomCheck: { data: { status: 'unknown', confidence: 0 } },
    llmCheck: { data: { status: 'unknown', confidence: 0 } },
    autoLearn: true,
  });
  ...
  const contradictedResult = evaluateLlmSor({
    ...
    question: 'question',
    llmText: 'answer',
    axiomCheck: { data: { status: 'contradicted', confidence: 0.9 } },
    llmCheck: { data: { status: 'verified', confidence: 0.8 } },
    autoLearn: true,
  });
