...
function getApprovalRequest(queue = [], approvalId = '', opts = {}) {
  ...
  if (!workspaceId) {
    return {
      ...
      type: 'approval-queue',
      warnings: [],
      errors: [{ code: 'WORKSPACE_ID_REQUIRED', field: 'workspaceId', message: 'workspaceId is required' }],
      approvals: sortQueueItems([]),
      item: null,
    };
  }
  ...
  const item = matches.find((candidate) => trimText(candidate.workspaceId, 'default') === trimText(workspaceId, 'default')) || null;
  ...
  if (!item) {
    return {
      ...
      approvals: sortQueueItems(filterByWorkspace(sourceQueue, workspaceId)),
      item: null,
    };
  }
...
function updateApprovalRequestStatus(queue = [], approvalId = '', status = '', opts = {}) {
  ...
  const currentIndex = sourceQueue.findIndex(
    (item) => trimText(item.approvalId) === trimText(approvalId) && trimText(item.workspaceId, 'default') === trimText(workspaceId, 'default'),
  );
