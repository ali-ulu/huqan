...
const { CANONICAL_RECEIPT_SCHEMA_VERSION } = require('./receipt/canonical-receipt');
const { validateReceiptChain } = require('./receipt/receipt-chain');
...
function hasValidCanonicalHash(receipt) {
  try {
    return validateReceiptChain([receipt]).valid === true;
  } catch (_) {
    return false;
  }
}
...
function receiptMatchesBinding(receipt, binding) {
  if (!isPlainObject(receipt)
      || receipt.schemaVersion !== CANONICAL_RECEIPT_SCHEMA_VERSION
      || receipt.createdAt !== binding.reservedAt
      || !isPlainObject(receipt.metadata)) {
    return false;
  }
  const metadata = receipt.metadata;
  return metadata.deliveryId === binding.deliveryId
    ...
    && metadata.payloadSha256 === binding.payloadSha256;
}
...
function createGitHubAppBetaStore({ rootPath }) {
  ...
  function readReceipt(deliveryId) {
    ...
    if (record.schemaVersion !== STORE_VERSION
        || !isPlainObject(record.receipt)
        || !hasValidCanonicalHash(record.receipt)) {
      fail(ERROR_CODES.INVALID_RECEIPT, 'GitHub App stored receipt is invalid');
    }
  ...
  function reserveDelivery(input) {
    ...
    if (existingReservation) {
      ...
      if (receipt && !receiptMatchesBinding(receipt, existingBinding)) {
        fail(ERROR_CODES.INVALID_RECEIPT, 'GitHub App stored receipt does not match its delivery reservation');
      }
  ...
  function commitReceipt(inputBinding, receipt) {
    ...
    if (!receiptMatchesBinding(receipt, binding) || !hasValidCanonicalHash(receipt)) {
