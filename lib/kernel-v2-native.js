...
const PREDICATE_DETERMINERS = new Set(['bir', 'bu', 'şu', 'o', 'her', 'hiç']);
const NEGATIVE_VERB_SUFFIX = /(?:m[ae]z|[ae]m[ae]z|m[ae]d[ıiuü]|m[ae]y[ae]c[ae]k|m[ae]m[ae]kt[ae]|m[ıiuü]yor)$/i;
const SHORT_NEGATIVE_VERB_STEMS = new Set(['al', 'at', 'et', 'ol']);
function nowIso() {
...
function normalizePredicateWords(words) {
  return words.filter(word => !PREDICATE_DETERMINERS.has(word)).join(' ');
}
function isNegativeVerbToken(token) {
  const normalized = String(token || '').toLowerCase().replace(/[.,!?;:]+$/, '');
  const match = normalized.match(NEGATIVE_VERB_SUFFIX);
  if (!match) return false;
  const stem = normalized.slice(0, -match[0].length);
  return stem.length >= 3 || SHORT_NEGATIVE_VERB_STEMS.has(stem);
}
function predicateIsNegated(predicate) {
  const words = String(predicate || '').split(/\s+/).filter(Boolean);
  return words.length > 0 && isNegativeVerbToken(words.at(-1));
}
function buildFailClosedNegativeClaimResult(kernel, statement, opts, workspaceId, parsed, predicate) {
  const guarded = kernel._verifyService._verifyResult(
    statement, opts,
    { status: 'unknown', confidence: 0, negationReason: 'negative_claim_has_no_known_positive_conflict' },
    [], { workspaceId, subject: parsed.subject, predicate },
  );
  return { ...guarded, meta: { ...guarded.meta, negativeClaimGuard: 'fail_closed' } };
}
function resolveNegativeClaimFallback(kernel, base, statement, opts, workspaceId, parsed, predicate) {
  return parsed.isNegated && base?.data?.status === 'verified'
    ? buildFailClosedNegativeClaimResult(kernel, statement, opts, workspaceId, parsed, predicate)
    : base;
}
function parseSimpleTurkishStatement(statement) {
  ...
  if (negMatch) {
    ...
    if (article > 0 && article < clause.length - 1) {
      const predicate = normalizePredicateWords(clause.slice(article + 1));
      return { subject: subjectBeforeArticle(clause, article), predicate, isNegated: true };
    }
    const predicate = normalizePredicateWords(clause.slice(1));
    return { subject: clause[0], predicate, isNegated: true };
  }
  const words = raw.split(/\s+/).filter(Boolean);
  if (words.length < 2) return null;
  const article = words.indexOf('bir');
  if (article > 0 && article < words.length - 1) {
    const predicate = normalizePredicateWords(words.slice(article + 1));
    return { subject: subjectBeforeArticle(words, article), predicate, isNegated: predicateIsNegated(predicate) };
  }
  const predicate = normalizePredicateWords(words.slice(1));
  return {
    subject: words[0],
    predicate,
    isNegated: predicateIsNegated(predicate),
  };
}
module.exports = {
  TYPE_RELATIONS,
