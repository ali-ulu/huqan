...
// #1300: keys that must never be assigned as a plain `obj[key] = ...`
// property, since JS treats an assignment to '__proto__' as setting the
// object's prototype rather than a data property.
...
function projectEnvelope(value, key) {
  ...
  if (value && typeof value === 'object') {
    ...
    for (const childKey of Object.keys(value)) {
      // #1300: a plain assignment here (projected[childKey] = ...) lets an
      // own-enumerable '__proto__' key in `value` -- e.g. from
      // JSON.parse('{"__proto__":{...}}') -- reach as an assignment to
      // `projected.__proto__` instead of a data property, polluting the
      // freshly created object's prototype. Skip the well-known dangerous
      // keys rather than assigning them.
...
function projectEnvelopeLegacy(value, key) {
  ...
  if (value && typeof value === 'object') {
    const projected = {};
    for (const childKey of Object.keys(value)) {
      // #1300: see projectEnvelope's matching guard above.
      if (DANGEROUS_KEYS.has(childKey)) continue;
