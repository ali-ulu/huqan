...
function createReviewService({ storage, kernel = null, getCurrentSnapshot = null, now = nowMs } = {}) {
  ...
  function list(limit = 50) {
    const rows = typeof storage.listUnresolvedToolApprovals === 'function'
      ? storage.listUnresolvedToolApprovals(Math.min(100, Math.max(1, Number(limit) || 50)))
      : [];
  ...
  function get(id) {
    ...
    const row = storage.getToolApprovalById(id);
  ...
  function decide(id, decision, reason = '') {
    const record = storage.getToolApprovalById(id);
    ...
    const updated = decision === 'approved'
      ? storage.resolveToolApproval(id, 'approved', reason || 'operator_approved')
      : storage.rejectToolApproval(id, reason || 'operator_rejected');
    return {
      ok: true,
      decision,
  ...
  async function execute(id, { action, body, githubClient = null, operatorToken = '' } = {}) {
    const record = storage.getToolApprovalById(id);
    if (!record || record.tool !== TOOL) return { ok: false, status: 404, code: 'PR_APPROVAL_NOT_FOUND' };
    ...
    try {
      ...
      const failed = typeof storage.failToolApproval === 'function'
        ? storage.failToolApproval(id, `github_executor_failed:${error.code || 'unknown'}`)
        : record;
