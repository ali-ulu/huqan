...
function createGitHubRestClient({ token = '', apiBaseUrl = 'https://api.github.com', userAgent = 'huqan-pr-guardian', timeoutMs = DEFAULT_GITHUB_REQUEST_TIMEOUT_MS } = {}) {
  const bearer = text(token);
  if (!bearer) return null;
  const base = text(apiBaseUrl).replace(/\/$/, '');
  const effectiveTimeoutMs = Number.isSafeInteger(timeoutMs) && timeoutMs > 0 ? timeoutMs : DEFAULT_GITHUB_REQUEST_TIMEOUT_MS;
...
module.exports = Object.freeze({ DEFAULT_GITHUB_REQUEST_TIMEOUT_MS, createGitHubRestClient });
