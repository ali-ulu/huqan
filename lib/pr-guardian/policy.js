...
/**
 * Read the derivation signal defensively. An absent signal means "no records
 * were involved", which is the ordinary case; a malformed one means the check
 * could not be trusted to have run, which is `unknown` rather than clean.
 */
function normalizeDerivationSignal(derivations) {
  if (derivations === null || derivations === undefined) return { status: 'none', total: 0, verified: 0, failures: [] };
  if (typeof derivations !== 'object') return { status: 'unknown', total: 0, verified: 0, failures: [] };
  const status = text(derivations.status);
  if (!['none', 'verified', 'failed', 'unknown'].includes(status)) {
    return { status: 'unknown', total: 0, verified: 0, failures: [] };
  }
  return {
    status,
    total: Number(derivations.total) || 0,
    verified: Number(derivations.verified) || 0,
    failures: Array.isArray(derivations.failures) ? derivations.failures : [],
  };
}
...
function evaluatePullRequest(snapshot = {}, options = {}) {
  ...
  // Derivation signal. Computed outside this function -- re-running a transform
  // is I/O and this stays a pure function over a snapshot. Severity mirrors
  // required checks exactly, for the same reasons:
  //
  //   failed  -> review. The change claims some of its files are the mechanical
  //              output of a recorded transform, and re-running that transform
  //              did not reproduce them. Worth a human, not a block: a stale
  //              record is a likely and innocent cause, and blocking would
  //              punish that case as hard as a dishonest one.
  //   unknown -> surfaced, not escalated. Otherwise every pull request in a
  //              repository that does not use derivations, or wherever the
  //              check simply could not run, would land in review.
  //   none    -> silent. Most changes carry no records and must not pay for it.
  const derivations = normalizeDerivationSignal(snapshot.derivations);
  if (derivations.status === 'failed') {
    decision = decision === DECISIONS.BLOCK ? decision : DECISIONS.REVIEW;
    reasons.push('derivation_not_reproducible');
  } else if (derivations.status === 'unknown') {
    reasons.push('derivation_verification_unknown');
  }
