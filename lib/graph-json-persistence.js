...
function loadEmbeddingsLenient(graph) {
  ...
  // Returns the raw sidecar text (undefined when absent/corrupt) so the JSON
  // path can keep snapshotting exactly what it read.
  graph._embeddingsIgnored = false;
  graph._embeddingLoadError = null;
  if (!fs.existsSync(graph._embeddingPath)) return undefined;
  try {
    const text = fs.readFileSync(graph._embeddingPath, 'utf8');
    graph._restoreEmbeddings(JSON.parse(text));
    return text;
  } catch (error) {
