...
/**
 * Get all events for a memory in one workspace.
...
/**
 * Full event history for one memory, including events that reference it as
 * relatedMemoryId (e.g. the supersede update event on the old memory).
 * Moved verbatim from MemoryStore.history by #2129: pure filter over the
 * read-only event array, no store receiver involved.
 * @param {EventReadContext} context
 * @param {string} memoryId
 * @param {object} opts
 * @returns {{ok:boolean,memoryId?:string,workspaceId?:string,events?:object[],total?:number,error?:object}}
 */
function runHistory(context, memoryId, opts = {}) {
  if (!memoryId || typeof memoryId !== 'string') {
    return { ok: false, error: { code: 'INVALID_INPUT', message: 'memoryId is required' } };
  }
  const workspaceId = normalizeWorkspaceId(opts.workspaceId);
...
module.exports = { runEventsForMemory, runTimeline, runGetEvents, runHistory };
