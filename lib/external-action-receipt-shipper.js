...
async function shipExternalActionReceipts(options = {}) {
  ...
  if (!dryRun && !endpoint) throw new Error('shipping requires an endpoint (or --dry-run)');
  ...
  if (typeof fetchImpl !== 'function') throw new Error('no fetch implementation available for shipping');
  const settings = {
