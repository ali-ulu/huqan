...
function buildMcpIngestExecuteResult(kernel, args, gate) {
  ...
  return request.ok
    ? kernel.fail('ingest_execute', 'APPROVAL_REQUIRED', 'Ingest execution must remain behind the approval owner.')
    : kernel.fail('ingest_execute', request.code, request.error);
}
