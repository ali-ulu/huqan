...
const NOT_YET_WIRED = Object.freeze({
  ...
  // --- P1 admission seam ---------------------------------------------------
  // Gate 2 closed with NO_EXISTING_UNIVERSAL_HOOK and required this seam before
  // any caller could be routed through it. It is unreached by construction and
  // by order: the guard in
  // test/mutation-admission-boundary.contract.test.js was landed first so that
  // routing shows up as its ledger falling to zero, and routing is the unit
  // after this one. This entry is expected to be deleted by the first family
  // that lands on it, not to persist.
  'lib/mutation-admission.js': 'P1 mutation admission seam; callers routed in the units after Gate 2',
