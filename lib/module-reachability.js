...
const NOT_YET_WIRED = Object.freeze({
  ...
  // --- self-evolve probe ---------------------------------------------------
  // An investigation tool, not a feature: it measures whether a self-evolution
  // run changes graph content or the thresholds that produce it. Wiring it to
  // a production entry point would mean shipping a command whose only purpose
  // is to trigger and observe self-evolution, which is a product decision this
  // investigation does not make.
  'lib/self-evolve-probe.js': 'read-only investigation probe; deliberately has no production entry point',

  // --- self-evolve adapter -------------------------------------------------
  // Wraps fractal-learn + runSelfEvolve into one callable. Wiring it to an MCP
  // or CLI entry point is a separate product decision; until then it is an
  // acknowledged not-yet-wired module (tested in isolation).
  'lib/self-evolve-adapter.js': 'wraps fractal-learn + self-evolve; no production entry point yet',
