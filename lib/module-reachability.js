...
const NOT_YET_WIRED = Object.freeze({
  ...
  // --- AB6 -----------------------------------------------------------------
  // Was reachable only through a dead reference in lib/mcp-gate-adapter.js:
  // imported, with an input builder written, but no code path ever called it
  // and no tool listed AB6. Removing that reference (#1253) leaves the module
  // genuinely unreached, which this line now records rather than hides.
  //
  // It is not wired to the MCP surface because sandbox isolation is a property
  // of how a runner is launched, not of a tool invocation -- the dead builder
  // had to invent `runner: 'unknown'` and `hasSnapshot: false` to call it at
  // all, i.e. gate on values the caller asserts about its own sandbox. If AB6
  // is enforced it belongs where the sandbox is created.
  'lib/sandbox-isolation.js': 'AB6 is not enforced at the MCP surface; belongs at sandbox creation, no caller yet',
  'lib/sandbox-timeout-policy.js': 'AB6 timeout ceiling rules; reached only from lib/sandbox-isolation.js, which is itself unwired',
