...
const NOT_YET_WIRED = Object.freeze({
  ...
  'lib/self-healer/dryrun-runner.js': 'self-healer is library-only by design (#224)',
  'lib/self-healer/finding-classifier.js': 'self-healer is library-only by design (#224)',
  'lib/self-healer/finding-schema.js': 'self-healer is library-only by design (#224)',
  'lib/self-healer/safety-decision.js': 'self-healer is library-only by design (#224)',
  ...
  'lib/code-change-gate.js': 'gate with no production caller',
  ...
  'sandboxRunner.js': 'node:vm sandbox; no production caller',

  // This checker itself: a CI/test tool, deliberately not part of any runtime
  // path. Listed rather than special-cased so the rule has no exceptions.
  'lib/module-reachability.js': 'reachability checker; consumed by its test and CI, not by the product',
});
...
function analyzeReachability(opts = {}) {
  ...
  const all = collectSourceFiles(root).map((file) => path.relative(root, file));
  ...
  // An acknowledgement for a file that is now reachable (or deleted) is stale
  // and should be removed, so the list keeps meaning something.
