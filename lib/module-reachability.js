...
const NOT_YET_WIRED = Object.freeze({
  ...
  'lib/sqlite-restore.js': 'Windows restore EPERM helper; CLI restore closes handles via this module, not yet statically wired from a declared entry point',

  // --- trust-signals (F2) ----------------------------------------------------
  // Robustness-lite probe: shipped and unit-tested library by design. Nothing
  // calls it yet on purpose — production wiring (policy flag + receipt
  // signals) lands in F3, and wiring it earlier would put an unreviewed
  // stress loop on the verify path.
  'lib/trust-signals/robustness.js': 'F2 trust-signal probe; unit-tested library, production wiring lands in F3',
});
