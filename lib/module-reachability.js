...
/** Helpers used only by a standalone command, not by the canonical runtime entry graph. */
const STANDALONE_SUPPORT_FILES = Object.freeze([
  'lib/observability/demo.js', // npm run demo:observability
]);
...
function isStandalone(relPath) {
  return STANDALONE_FILES.includes(relPath)
    || STANDALONE_SUPPORT_FILES.includes(relPath)
