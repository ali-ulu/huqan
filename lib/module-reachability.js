...
const NOT_YET_WIRED = Object.freeze({
  // --- V5 ------------------------------------------------------------------
  // These entries used to cite ADR-010's V5_IMPLEMENTATION_ENTRY: FAIL as the
  // reason nothing may call them. That decision has been superseded:
  // docs/v5/v5-implementation-entry-successor-audit.md records PASS against
  // b6ea911ef44ad9a762ad2a1b57a85a64281a580c, so entry is no longer what holds
  // them back.
  //
  // They stay listed because the list tracks what is actually unreached, not
  // what is forbidden -- and no production caller exists for any of them yet.
  // A module leaves this list when something calls it, under its own gate;
  // deleting a line here would only make the check fail as a stale
  // acknowledgement.
  'lib/v5/cryptographic-profile-contract.js': 'V5 library implementation; entry authorized, no production caller yet',
  'lib/v5/cryptographic-verification-adapter.js': 'V5 library implementation; entry authorized, no production caller yet',
  ...
  'lib/v5/runtime-writer.js': 'V5 library implementation; entry authorized, no production caller yet',
  'lib/v5/structural-signing-helper.js': 'V5 library implementation; entry authorized, no production caller yet',
  'lib/v5/trusted-key-resolver.js': 'V5 library implementation; entry authorized, no production caller yet',
  'lib/v5/verification-core.js': 'V5 library implementation; entry authorized, no production caller yet',
  ...
  'schemas/v5/shared-trust-package-validator.js': 'V5 schema implementation; entry authorized, no production caller yet',
