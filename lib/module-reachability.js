...
const PRODUCTION_ENTRY_POINTS = Object.freeze([
  ...
  // A coding-agent host executes this on every tool call it is configured for,
  // which makes it a product surface rather than a maintenance script: it is
  // listed in package.json `files`, and the gate behind it only ever runs
  // because this file calls it. Listing it here keeps the analysis honest in
  // both directions -- the adapter has a real caller, and if that call is ever
  // removed the adapter goes back to reading as unreached.
  'scripts/agent-action-gate-hook.js',
]);
