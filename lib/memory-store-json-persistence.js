...
function loadJsonStore(store, report) {
  try {
    ...
    return persistenceError('load', err);
  }
}

function withJsonTransaction(store, fn, withTransaction) {
  if (typeof withTransaction !== 'function') {
    throw new TypeError('withJsonTransaction requires a transaction callback.');
  }
...
function applyJsonMemoryStore(store, state) {
  store._memories = new Map(
    state.memories.map((record) => [store.makeMemoryKey(record.workspaceId, record.memoryId), record]),
  );
