...
function saveToolApprovalWithIdempotencyConflict(store, record = {}, fingerprintValue = '') {
  ...
  if (!approval || (saved.inserted !== true && saved.inserted !== false)) {
    return failure('APPROVAL_STORE_RESULT_INVALID', 'approval store did not return a valid persisted approval result');
  }
  ...
  const exactPersistedMatch = approval.approval_key === approvalKey
  ...
  if (saved.inserted === true) {
    if (!exactPersistedMatch) {
      return failure('APPROVAL_STORE_RESULT_INVALID', 'approval store inserted a record that does not match the reviewed request', {
        inserted: true,
