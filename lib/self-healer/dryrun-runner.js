...
function runSelfHealerDryRun(input = {}, opts = {}) {
  ...
  const findingsToProcess = budgetReviewRequired
    ? findings.slice(0, Math.max(0, Math.floor(budget.remaining)))
    : findings;
  const budgetTruncated = findingsToProcess.length < findings.length;
