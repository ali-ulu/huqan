...
function classifyFindingKind(input) {
  ...
  // Own-property only. `Object.freeze` does not cut the prototype chain, so a
  // bare read answered `constructor` and `__proto__` -- both already lowercase,
  // so they survive the fold -- with inherited values, and this function
  // returned a function or `Object.prototype` where every other branch returns
  // a kind string or null. `classifyConfidence` below already reads this way
  // (#1191).
  if (Object.hasOwn(FINDING_KIND_ALIASES, direct)) {
...
function classifySeverity(input) {
  ...
  if (Object.hasOwn(SEVERITY_SIGNAL_MAP, signal)) {
    return SEVERITY_SIGNAL_MAP[signal];
  }

  // `kind ? ... : 'medium'` skipped the fallback whenever `kind` was truthy but
  // not a real kind, returning `undefined` -- and a downstream
  // `SEVERITY_ORDER.indexOf(undefined)` is -1, which sorts an unclassifiable
  // finding below 'info', the opposite of the intended 'medium' (#1191).
  const kind = classifyFindingKind(input);
  return Object.hasOwn(SEVERITY_BY_KIND, kind) ? SEVERITY_BY_KIND[kind] : 'medium';
}
