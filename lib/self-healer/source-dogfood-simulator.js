...
function simulateInSandbox(dependencyGraph, candidate, evidence = {}) {
  ...
  // AB6 reached a verdict and, before #1891, this line is where it stopped
  // existing: only `result.data` was read and `meta` was dropped, so a
  // quarantine was never written anywhere. Recording is opt-in on a supplied
  // graph and never affects the outcome below -- see lib/sandbox-escape-ledger.js
  // for why this is only the write and not a trust-score signal.
