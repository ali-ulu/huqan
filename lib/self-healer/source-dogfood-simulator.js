    ...
    evidence: [
      {
        type: 'file',
        ref: candidate.from,
        detail: `source dependency graph target; hypothesis=${candidate.hypothesisType}`,
      },
      {
        type: 'manual',
        ref: `sandbox:${candidate.candidateId}`,
        detail: `${cycleText}; edges ${sandbox.beforeEdges}->${sandbox.afterEdges}`,
      },
      {
        type: 'manual',
        ref: `rustGraph:${candidate.candidateId}`,
        detail: `backend=${rustComparison.backend}; edges ${rustComparison.before.edges}->${rustComparison.after.edges}`,
      },
      {
        type: 'manual',
        ref: `code-change-gate:${candidate.candidateId}`,
        detail: `decision=${gate.decision}; reason=${gate.reason}`,
      },
    ],
