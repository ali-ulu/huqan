...
const { createSandboxEscapeProducer } = require('../sandbox-escape-producer');
const { evaluateCodeChange } = require('../code-change-gate');

// The runner is handed in rather than reached for: the producer lives in an
// inner ring and may not require the outer runner (layer check).
...
function simulateInSandbox(dependencyGraph, candidate, evidence = {}) {
  // The producer records AB6 block/quarantine verdicts when it is given a
  // graph (#2505 G), so this call site only forwards the surface and never
  // records twice: one verdict, one row.
  const result = runSandboxedWithEscapeRecording(SANDBOX_SIMULATION_SOURCE, {
    ...
    sourceRef: 'self-healer:source-dogfood-simulator',
  });
...
async function simulateSourceCandidate(input = {}) {
  ...
  const sandbox = simulateInSandbox(dependencyGraph, candidate, { graph: input.graph, workspaceId });
