...
function decideSelfHealerAction(finding) {
  ...
  // Minimum evidence rule: a finding with nothing observable behind it can be
  // recorded for trend purposes but must not become a fix proposal.
...
function buildDecision(decision, reason, riskFlags) {
  return {
    decision,
    reason,
    // `block` is not approvable -- it is refused, not queued for a human.
    // `observe` needs no approval because it authorizes nothing.
