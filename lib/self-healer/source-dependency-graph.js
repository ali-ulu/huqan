...
function resolveInsideRoot(root, relativePath) {
  ...
  const lexical = path.resolve(rootReal, normalized);
  if (!isInsideRoot(rootReal, lexical)) {
  ...
  if (!isInsideRoot(rootReal, real)) {
    throw sourceError('SELF_HEALER_TARGET_OUTSIDE_ROOT', 'targetPath resolves outside the repository root');
  }
  return real;
}
...
function resolveRelativeRequire(root, fromFile, specifier) {
  ...
  for (const candidate of [base, `${base}.js`, path.join(base, 'index.js')]) {
    ...
    try {
      if (!fs.statSync(candidate).isFile() || !candidate.endsWith('.js')) continue;
      const real = fs.realpathSync(candidate);
      if (!isInsideRoot(root, real)) {
        throw sourceError('SELF_HEALER_SOURCE_OUTSIDE_ROOT', `dependency resolves outside repository root: ${specifier}`);
      }
      return real;
    } catch (error) {
      if (error && error.code === 'SELF_HEALER_SOURCE_OUTSIDE_ROOT') throw error;
    }
  }
  return null;
}
...
function readQuotedLiteral(source, index) {
  ...
  while (cursor < source.length) {
    ...
    if (source[cursor] === quote) return { value, nextIndex: cursor + 1 };
    value += source[cursor];
    cursor += 1;
  }
  return null;
...
function extractRelativeRequires(source) {
  ...
  while (cursor < text.length) {
    ...
    if (text[cursor] === "'" || text[cursor] === '"' || text[cursor] === '`') {
      cursor = skipQuotedLiteral(text, cursor);
      continue;
    }
    if (text.startsWith('require', cursor)
      && !isIdentifierCharacter(text[cursor - 1])
      && text[cursor - 1] !== '.'
      && !isIdentifierCharacter(text[cursor + 7])) {
      const openParen = skipTrivia(text, cursor + 7);
      if (text[openParen] === '(') {
        const argument = skipTrivia(text, openParen + 1);
        if (text[argument] === "'" || text[argument] === '"') {
          const literal = readQuotedLiteral(text, argument);
          if (literal && literal.value.startsWith('.')) specifiers.push(literal.value);
        }
      }
    }
    cursor += 1;
...
function buildDependencyGraph(input = {}) {
  ...
  const root = fs.realpathSync(path.resolve(source.root || path.join(__dirname, '..', '..')));
...
module.exports = {
  ...
  // Exported so containment is decided in one place: audit-runner needs the
  // same rule for outputPath, and a second copy of it is a copy that can drift.
  isInsideRoot,
  DEFAULT_MAX_FILES,
