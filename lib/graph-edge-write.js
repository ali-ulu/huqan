...
function addEdge(storeApi, fromId, toId, relation, opts = {}) {
  ...
  if (existing) {
    storeApi.recordEdge?.(existing);
    const oldConfidence = existing.confidence ?? existing.weight ?? 0.5;
    // H-06: kernel-öğrenme yolunda aynı kanıt metni tekrar learn edilince
    // ağırlık şişmemeli. Doğrudan graph.addEdge çağrıları eski +0.1
    // davranışını korur (graph.test.js "tavan 1.0" testi); yalnızca
    // suppressWeightBumpOnReaffirm bayrağı set edilmiş VE gelen kanıtların
    // tamamı zaten kayıtlıysa artırım atlanır.
    const existingEvidence = Array.isArray(existing.evidence) ? existing.evidence : [];
    const hasNewEvidence = nextEvidence.some((item) => !existingEvidence.includes(item));
    const isReaffirm = opts.suppressWeightBumpOnReaffirm === true && !hasNewEvidence;
    const requestedWeight = opts.weight === undefined
      ? (isReaffirm ? (existing.weight ?? 0.5) : (existing.weight ?? 0.5) + 0.1)
      : opts.weight;
    ...
    if (isReaffirm) {
      existing.confidence_history.push({ value: oldConfidence, updated_at: isoNow, event: 'reaffirmed' });
    } else if (existing.confidence !== oldConfidence) {
