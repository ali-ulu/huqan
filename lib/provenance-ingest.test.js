...
test('ingestWithProvenance forwards a caller mutationOperationId without generating one', async () => {
  const calls = [];
  const kernel = {
    learn(text, opts) {
      calls.push({ text, opts });
      return { ok: true, data: { learned: 1, skipped: 0 } };
    },
  };
  const input = {
    text: 'bounded provenance claim',
    provenance: {
      provenanceId: 'prov-operation-id',
      sourceRef: 'docs/operation-id.md#claim',
      sourceTitle: 'Operation ID',
      sourceType: 'document',
      actor: 'test',
      timestamp: '2026-07-31T00:00:00.000Z',
      workspaceId: 'workspace-operation-id',
    },
  };

  await ingestWithProvenance(kernel, input, { mutationOperationId: 'mutation-123' });
  await ingestWithProvenance(kernel, input);

  assert.strictEqual(calls[0].opts.mutationOperationId, 'mutation-123');
  assert.strictEqual(Object.hasOwn(calls[1].opts, 'mutationOperationId'), false);
});

test('ingestWithProvenance keeps skipped admission explicit without a graph-write claim', async () => {
  const result = await ingestWithProvenance({
    learn() {
      return { ok: true, data: { learned: 0, skipped: 1 } };
    },
  }, {
    text: 'skipped provenance claim',
    provenance: {
      provenanceId: 'prov-skipped',
      sourceRef: 'docs/skipped.md#claim',
      sourceTitle: 'Skipped',
      sourceType: 'document',
      actor: 'test',
      timestamp: '2026-07-31T00:00:00.000Z',
      workspaceId: 'workspace-skipped',
    },
  });

  assert.strictEqual(result.admission.outcome, 'skipped');
  assert.strictEqual(result.admission.graphWrite, false);
  assert.strictEqual(result.admission.learned, 0);
  assert.strictEqual(result.admission.skipped, 1);
});
