...
class MemoryStore {
    ...
    try {
      ...
      for (const row of events) {
        const event = {
          ...
          provenance: JSON.parse(row.provenance_json),
          trustPolicyVersion: row.trust_policy_version,
          details: JSON.parse(row.details_json), relatedMemoryId: row.related_memory_id || undefined,
        };
   ...
   * @param {string} memoryId; opts.workspaceId is optional
   * @returns {object[]}
   */
  getEvents(memoryId, opts = {}) {
    return runGetEvents(this._eventReadContext(), memoryId, opts);
  }
  ...
  getLinks(memoryId, opts = {}) {
    return readLinks(this._linkReadContext(), memoryId, opts);
  }
