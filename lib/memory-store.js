...
class MemoryStore {
  ...
  // (#1848). dbPath/config were resolved in the constructor; no-op in JSON mode.
  reopen() {
    if (this._db) {
  ...
  /**
   * Reopens the SQLite connection after restore replaced the backing database
   * file (the same memory.db graph uses unless a separate memoryStoreDbPath was
   * configured). The dbPath and busy/lock config were resolved in the
   * constructor, so the handle can be rebuilt without re-running that
   * resolution. No-op in JSON mode where there is no connection. See #1848.
   */
  reopen() {
    if (this._db) {
      try { this._db.close(); } catch (_) {}
    ...
    applySqliteDurability(this._db, 'EVIDENCE', { busyTimeoutMs: this._busyRetryConfig.busyTimeoutMs });
    this._initDB();
    // #1864: warmup appends to the existing collections, so a reopen after a
    // restore (or any second close/reopen) would otherwise keep serving the
    // pre-reopen records and double the event/link counts on every cycle.
    // Reset the in-memory state first so warmup rebuilds it from the current
    // database contents. On a warmup failure we fail closed to an empty cache
    // rather than leaving a partial old/new mix.
