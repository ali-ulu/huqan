...
class MemoryStore {
  constructor(opts = {}) {
    ...
    if (wantSQLite) {
      ...
      applySqliteDurability(this._db, 'EVIDENCE', { busyTimeoutMs: this._busyRetryConfig.busyTimeoutMs });
      this._initDB();
      this._warmup();
  ...
  /**
   * Reopens the persistence backend after restore replaced the backing files
   * (the same memory.db the graph uses unless a separate memoryStoreDbPath was
   * configured, or the JSON file in JSON mode). The dbPath/busy config were
   * resolved in the constructor, so the handle is rebuilt without re-running
   * that resolution. See #1848 (handle lifecycle) and #1864 (stale cache).
   *
   * The in-memory collections are rebuilt from the fresh backend, not merged
   * into the old ones: previous collections are snapshotted aside, cleared,
   * and reloaded; a reload failure restores the previous collections and
   * rethrows, so a partially loaded cache is never exposed.
   */
  reopen() {
    if (this._db) {
    ...
    if (this._jsonPath) {
      const prevMemories = this._memories;
      const prevEvents = this._events;
      const prevLinks = this._links;
      try {
        jsonPersistence.applyJsonMemoryStore(this, jsonPersistence.loadJsonMemoryStore(this._jsonPath));
      } catch (err) {
        this._memories = prevMemories;
    ...
    if (!this.dbPath || !Database) return;
    const prevMemories = this._memories;
    const prevEvents = this._events;
    const prevLinks = this._links;
    const prevCorruptRows = this.corruptRows;
    ...
    try {
      ...
      this._events = prevEvents;
      this._links = prevLinks;
      this.corruptRows = prevCorruptRows;
      throw err;
    }
  }
