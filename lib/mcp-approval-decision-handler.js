...
function handleMcpApprovalDecision(kernel, args = {}, runtime = {}, failApprovalDecision) {
  ...
  // H-07 (#1980): the MCP agent/learn claim is leaseless, so a crash between
  // claim and finalize leaves the row `executing` with no lease for the
  // lease sweeper to expire. Reap stuck rows best-effort here so a retry
  // reports APPROVAL_RECONCILIATION_REQUIRED instead of hanging on
  // APPROVAL_EXECUTION_IN_PROGRESS forever. The claim UPDATE below stays
  // authoritative: recovery only fails rows, never re-executes them.
  try {
