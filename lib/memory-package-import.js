...
/**
 * Whether this exact record is already present in the target workspace.
 *
 ...
 * @returns {boolean}
 */
function alreadyPresent(records, idField, id, workspaceId) {
  return records.some(record => record[idField] === id && record.workspaceId === workspaceId);
}
...
function importPackageEvents(store, events, ctx) {
  for (const evt of events) {
    ...
    if (alreadyPresent(store._events, 'eventId', normalized.eventId, normalized.workspaceId)) {
      ctx.skipped.events++;
      continue;
    }
...
function importPackageLinks(store, links, ctx) {
  for (const lnk of links) {
    ...
    if (alreadyPresent(store._links, 'linkId', normalized.linkId, normalized.workspaceId)) {
      ctx.skipped.links++;
      continue;
    }
