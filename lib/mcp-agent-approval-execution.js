...
function runApprovedAgent(kernel, cleanArgs, workspaceId) {
  const revalidatedGate = restoreAgentReview(evaluateMcpGate({
    tool: 'huqan.agent',
    args: cleanArgs,
    metadata: {},
  }));
  if (!revalidatedGate || !['allow', 'review'].includes(revalidatedGate.decision)) {
    const error = new Error('approved agent call no longer passes current MCP policy');
    error.code = 'APPROVED_AGENT_REVALIDATION_BLOCKED';
    error.gate = revalidatedGate || null;
    throw error;
  }
...
function executeApprovedMcpAgent({
  ...
  let execution;
  try {
    execution = runApprovedAgent(kernel, cleanArgs, workspaceId);
  } catch (error) {
