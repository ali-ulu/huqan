...
/**
 * What this proves, precisely: the guard's decision path blocks the sentinel
 * *in this process*. It does not load the installed artifact and does not run
 * the host's hook, so it cannot prove the gate is live in the client -- hence
 * `via: 'evaluator'` rather than a bare `live: true` that reads like end-to-end
 * proof. Loading each profile's artifact the way its host would is the
 * remaining half of #1792.
 */
function selfValidate(profile, root) {
  ...
  return { live: true, via: 'evaluator', decision: evaluated.result.decision, reason: evaluated.result.reason };
}
