...
//
// The generated adapter (#2061) is not shipped runtime code: `huqan-gate
// adapter` copies it into the user's own project, where lib/environment-compat.js
// does not exist. It deliberately depends on nothing outside `node:` builtins,
// so it cannot delegate, and the AXIOM_ fallback this invariant protects is
// meaningless there — a freshly generated adapter has no legacy deployment to
// be compatible with. Exempt the template rather than making it require a
// module its copies will never have.
const ALLOWED = new Set([
  'lib/environment-compat.js',
  'adapters/external-action/generic-adapter.js',
]);
