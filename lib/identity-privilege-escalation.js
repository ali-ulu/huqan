...
/**
 * Default observe (#2157). A widened card is always worth a human verdict, so
 * with nothing configured this returns an active `review` config sharing the
 * module's bounded ledger. A deployment that wants the old silent-always-allow
 * behaviour explicitly opts out (`privilegeEscalation.enabled: false` or the
 * `HUQAN_EXTERNAL_GUARD_PRIVILEGE_ESCALATION=off` env flag), and one that wants
 * escalation to be fatal sets it to `block`.
 *
...
function privilegeEscalationOptions(options = {}) {
  ...
  // Explicit opt-out restores the pre-detector memoryless behaviour (#2157).
  // It beats the default so a deployment can go back to the old posture.
  if (!configured && ['off', '0', 'false', 'disabled', 'no'].includes(flag)) return null;

  // Default observe (#2157): the session ledger is always tracked, and a card
  // that widens mid-session is surfaced as `review` (a real verdict, never the
  // silent allow of the unconfigured past). It is never an automatic permit:
  // the human either reviews the widening or the deployment hardens the flag
  // to `block`. `decision` is only ever `review` or `block`.
  let decision = 'review';
  if (configured && (supplied.enabled === true || supplied.enabled === undefined)) {
    if (supplied.decision === 'block') decision = 'block';
  } else if (flag === 'block') {
    decision = 'block';
  } else if (['1', 'true', 'review', 'enable', 'require', 'on', 'yes'].includes(flag)) {
    decision = 'review';
  }
