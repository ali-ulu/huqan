 ...
 *
 * ## Commit-ordering bound (#1135)
 *
 * The journal and the graph state are two separate files, so committing a
 * mutation is two writes and cannot be made atomic here. Graph's
 * _runMutationOnceJsonLocked() therefore persists graph state with save()
 * first and marks the journal 'completed' second, which fixes exactly which
 * failure is possible in the window between them:
 *
 *   - Crash after save(), before the journal write: the mutation is on disk
 *     but unrecorded, so a retry with the same operationId runs it again.
 *     Mutations that are not idempotent (upsertNode bumps weight by +0.1) are
 *     applied twice.
 *   - The reverse ordering would instead allow a 'completed' entry for state
 *     that never reached disk — a replay then returns success for a mutation
 *     that did not happen, which is silent data loss rather than duplication.
 *
 * A possible double-apply is accepted as the lesser failure; a false claim of
 * completion is not. Both sides of that window are pinned in
 * test/graph-json-commit-ordering.test.js, so reversing the two lines fails
 * the suite instead of quietly buying the worse failure. Callers that need
 * exactly-once semantics across a crash must make the mutation itself
 * idempotent — the journal alone does not provide it here. The SQLite backend
 * has no such window: state and journal commit in one transaction.
 */
...
/**
 * Journal sections are null-prototype maps (#1671).
 *
 * Operation identifiers, receipt ids and chain keys are caller-supplied
 * strings, and on a normal `{}` map three of them are not storable at all:
 * `map.__proto__ = entry` re-points the prototype instead of creating an own
 * property, so the entry is silently dropped and the *next* request with that
 * same operationId is not recognised as a replay. `constructor` and
 * `toString` fail the other way -- an inherited value is read back as if it
 * were a journal row. A null-prototype map has no inherited keys, so every
 * identifier round-trips as an ordinary own property.
 *
 * JSON.parse() already creates `__proto__` as an own data property, and
 * JSON.stringify() serialises a null-prototype object exactly like a plain
 * one, so persisted records keep own-property semantics across a round trip.
 */
function emptyJournalSection() {
  return Object.create(null);
}

function emptyMutationJournal() {
  return {
    operations: emptyJournalSection(),
    receipts: emptyJournalSection(),
    chainTips: emptyJournalSection(),
    receiptsById: emptyJournalSection(),
  };
}

/**
 * Copy a parsed section into a null-prototype map, keeping own enumerable
 * properties only. `Object.assign` reads the source's own keys and defines
 * them on a prototype-less target, so a `"__proto__"` key that JSON.parse
 * produced as an own property stays an own property here instead of being
 * re-interpreted as a prototype assignment.
 */
function toJournalSection(parsedSection) {
  const section = emptyJournalSection();
  for (const key of Object.keys(parsedSection)) {
    section[key] = parsedSection[key];
  }
  return section;
}
...
function readMutationJournal(journalPath, fileSystem = fs) {
  ...
  for (const section of JOURNAL_SECTIONS) {
    ...
    journal[section] = toJournalSection(parsed[section]);
  }
