...
const { defaultExternalActionStateRoot } = require('./external-action-receipt');
...
function registryPath(environment = process.env) {
  return path.join(defaultExternalActionStateRoot(environment), 'stores.json');
}
...
function assertStoreCreationAllowed({ dbPath, explicit, exists, environment = process.env }) {
  ...
  if (exists || explicit || isRebuildAllowed(environment) || isTestRunner()) {
