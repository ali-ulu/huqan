...
/**
 * Whether this path is a throwaway rather than a store the operator keeps.
 *
 * Both `resolveDbPath` implementations already admit `os.tmpdir()` as a root
 * for this exact reason -- "ephemeral stores (tests, sandboxed runs)
 * legitimately live there" -- and the cases that depend on it are real: the
 * packed-tarball smoke installs HUQAN into a temp directory several times in
 * one run and expects each install to work, and the gate install probe builds
 * a scratch store to verify a hook before writing it.
 *
 * Excluding them costs no protection. A store nobody meant to keep is exactly
 * what the temp directory is for; the stray this guard exists for sat in
 * %LOCALAPPDATA%, and the stores worth protecting are under the home directory.
 */
function isEphemeralStorePath(dbPath) {
  const resolved = path.resolve(dbPath);
...
function assertStoreCreationAllowed({ dbPath, explicit, exists, environment = process.env }) {
  ...
  if (isEphemeralStorePath(dbPath)) return;
