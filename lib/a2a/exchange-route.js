...
function createA2aExchangeBoundary(options = {}) {
  ...
  let replay;
  let tasks;
  let evaluateBoundedExchange;
  try {
    ({ evaluateBoundedExchange } = require('./bounded-exchange'));
    const { createA2aReplayStore } = require('./replay-store');
    const { createA2aTaskStore } = require('./task-store');
    authority = readReceiverAuthority(authorityFile);
    replay = createA2aReplayStore(replayDirectory);
  ...
  // Read the receiver clock exactly once per request from the authority, never
  // from the payload. Holding the parsed authority means an operator edit needs
  // a restart, which is the same lifetime the trusted-key set already has.
  ...
  async function handle(req, readBody) {
    ...
    try {
      ...
      // refusal.
      return refusal(403, 'verification_failed');
    }

    if (!result || result.decision !== 'allow') {
      return refusal(403, (result && result.reason) || 'verification_failed');
    }
