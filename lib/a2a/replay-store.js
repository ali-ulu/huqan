...
const { taskIdForReplayKey } = require('./task-store');

const SHA256 = /^[0-9a-f]{64}$/;

function createA2aReplayStore(directory) {
  ...
  function reserve(record) {
    ...
    const taskMarker = path.join(root, `${taskIdForReplayKey(record.replayKey)}.reserved-task`);
    const target = path.join(root, `${record.replayKey}.reserved`);
    let taskDescriptor;
    let descriptor;
    try {
      taskDescriptor = fs.openSync(taskMarker, 'wx', 0o600);
      fs.fsyncSync(taskDescriptor);
      ...
      if (taskDescriptor !== undefined) fs.closeSync(taskDescriptor);
    }
  }
...
module.exports = Object.freeze({ createA2aReplayStore });
