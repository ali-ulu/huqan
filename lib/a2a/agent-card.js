...
const SUPPORTED_PROTOCOL_VERSIONS = Object.freeze([PROTOCOL_VERSION, '0.1']);
...
/**
 * Where a caller looks up what happened to an exchange (P0-E).
 *
 * `states` is advertised because the interesting one is `unknown`, and a
 * consumer that has not been told it exists will treat it as an error and
 * retry -- which is precisely what the at-most-once design is protecting
 * against. Naming it in the card makes it part of the contract rather than a
 * surprise.
 */
const TASKS = Object.freeze({
  pathPrefix: '/api/a2a/tasks/',
  method: 'GET',
  states: Object.freeze(['completed', 'unknown']),
});

/**
 * The retry contract (P0-F).
 *
 * Every refusal from the exchange carries `safeToRetry`. It answers "can
 * resending this cause a second effect", not "might resending work" -- see
 * `lib/a2a/retry-classification.js`. When it is false the refusal also carries
 * a `taskId`, because a caller told not to retry needs somewhere to look
 * instead.
 *
 * Advertised because a consumer that does not know the flag exists will invent
 * its own retry policy, and the most natural invention -- retry on any failure
 * -- is the one this receiver cannot absorb.
 */
const RETRY = Object.freeze({
  responseField: 'safeToRetry',
  semantics: 'safe-to-retry-means-no-effect-possible',
  taskPointerOnUnsafe: true,
});

/**
 * Declared deadlines (P0-F).
 *
 * `requestTimeoutMs` is enforced on the socket, not merely advertised: a caller
 * that stalls mid-body is dropped before the evaluator is reached, so a
 * timed-out request provably never reserved and is safe to resend.
 */
...
/**
 * The deadline the exchange route enforces, taken from the card rather than
 * declared twice. `lib/a2a/negotiate-route.js` reads its path from `NEGOTIATION`
 * for the same reason: a document that advertises one number while the route
 * enforces another is worse than a document that advertises nothing.
 */
const REQUEST_TIMEOUT_MS = TIMEOUTS.requestTimeoutMs;
...
const UNSUPPORTED_SURFACES = Object.freeze([
  // `capability-negotiation` was here until P0-D shipped it, and
  // `task-lifecycle` until P0-E gave exchanges durable task records and a read
  // route. Removing the line is how a unit graduates, and the card's tests
  // assert this list exactly so that shipping a surface without updating it is
  // a failure rather than a quietly stale document.
  //
  // `idempotency-keys` deliberately did not. A caller-supplied key would have to
  // return a stored success for a retried request, and the one case where that
  // is unknowable is the case the replay marker exists for -- see
  // `lib/a2a/task-store.js`. The receiver-derived replay key already provides
  // at-most-once; a caller-supplied one would need to reconcile with it, and
  // that reconciliation is a decision on its own, not a field.
  'idempotency-keys',
  // P0-F shipped timeout and retry but deliberately not cancellation. There is
  // nothing here to cancel: the exchange is synchronous, and once the replay
  // key is reserved the exchange is accounted for and cannot be withdrawn --
  // that is the same at-most-once decision `idempotency-keys` is blocked on. A
  // cancel endpoint whose only possible answer is "too late" would be theatre.
  // Cancellation acquires a referent when there is an asynchronous or streaming
  // surface to cancel, which is P0-G.
  'cancellation',
  'streaming',
  'json-rpc',
]);
