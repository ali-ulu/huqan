...
function createAgentCardBoundary(options = {}) {
  ...
  //
  // Configured *and constructible*, via the exchange route's own construction
  // rather than a second check beside it. Two non-empty strings were never the
  // same claim as a working exchange: an unreadable A2A_REPLAY_DIR or an
  // environment where bounded-exchange cannot be required left the exchange
  // route unbuilt at 404 while the card kept advertising it (#1182).
  const dependencies = constructA2aBoundaryDependencies(resolveA2aBoundaryPaths(options));
  if (!dependencies) return null;

  let card;
  try {
    card = buildAgentCard(dependencies.authority);
  } catch (_) {
    return null;
  }
  if (!card) return null;
