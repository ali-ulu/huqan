...
function createA2aTaskStore(directory) {
   ...
   *
   * Each reservation writes a domain-separated task marker alongside its
   * replay marker. Looking up that marker never exposes or reconstructs the
   * replay key, and remains constant-time as reservations accumulate.
   */
  function reservationExists(taskId) {
    try {
      return fs.existsSync(path.join(root, `${taskId}.reserved-task`));
