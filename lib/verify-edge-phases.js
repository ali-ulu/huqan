...
// `<positive> <değil|değildir|not>` with trailing whitespace allowed. Scans the
// last token instead of `/^(.*?)\s+(neg)\s*$/`, which backtracks
// polynomially on long whitespace runs.
function splitTrailingNegation(predicate) {
  const trimmed = String(predicate).trimEnd();
  let start = trimmed.length;
  while (start > 0 && !/\s/.test(trimmed[start - 1])) start -= 1;
  if (start === 0) return null;
  if (!/^(de[gğĞ]il(?:dir)?|not)$/i.test(trimmed.slice(start))) return null;
  return { positive: trimmed.slice(0, start).trim() };
}
...
function verifyAgainstEdges(service, ctx) {
  ...
  const negMatch = splitTrailingNegation(predicate);
  if (negMatch) {
    const positive = negMatch.positive;
    if (positive) {
      ...
      if (posEdge) {
        return verdict(statement, opts, { status: 'contradicted', confidence: 0.85 }, [{
          kind: 'contradiction',
  ...
  if (directEdge) {
    ...
    const causalConflict = buildCausalPreventionConflict(subject, directEdge, statement, incomingPrevents); if (causalConflict) return verdict(statement, opts, causalConflict.data, causalConflict.evidence, { ...verifyContext, directEdge });
    ...
    if (directEdge.relation === 'PREVENTS' && !incomingPrevents) {
      ...
      return verdict(statement, opts, { status: 'contradicted', confidence: pConfidence }, [{
        kind: 'contradiction',
    ...
    if (incomingAbsolute) {
      ...
      return verdict(statement, opts, { status: 'verified', confidence }, [service.host.edgeEvidence(directEdge, 'direct_edge', confidence)], { ...verifyContext, directEdge });
    }
  }
  return null;
}
