...
function ensureState(state = {}, context = {}) {
  ...
  return {
    ...
    maxCycles: Number.isInteger(state.maxCycles)
      ? Math.max(1, Math.min(5, state.maxCycles))
      : base.maxCycles,
    maxHypotheses: Number.isInteger(state.maxHypotheses)
      ? Math.max(1, Math.min(MAX_HYPOTHESES, state.maxHypotheses))
      : base.maxHypotheses,
...
function advanceAfterVerification(kernel, currentState, report, context = {}) {
  ...
  const semanticSignal = signalFromVerification(report);
  const causalSimulation = simulateHypothesis(kernel, state, hypothesis, context);
  const signal = combineVerificationSignals(semanticSignal, causalSimulation.signal);
  const observation = {
    hypothesisKey: hypothesis.key,
    signal,
    semanticSignal,
    causalSignal: causalSimulation.signal,
    causalSimulation,
...
function loopEnabled(opts = {}, kernel = null) {
  if (opts.dreamExperimentLoop === false) return false;
