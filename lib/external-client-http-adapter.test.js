...
class Request extends EventEmitter {
  constructor({ method = 'POST', headers = {}, rawHeaders, chunks = [] } = {}) {
    ...
    this.chunks = chunks;
    this.destroyedByAdapter = false;
  }
  destroy() { this.destroyedByAdapter = true; }
  start(afterData) {
    queueMicrotask(() => {
...
function assertFailure(result, statusCode) {
  ...
  assert.deepEqual(Object.keys(result.headers), statusCode === 405 ? ['Content-Type', 'Cache-Control', 'Allow'] : ['Content-Type', 'Cache-Control']);
...
test('content type accepts only exact JSON forms and rejects duplicates before delegation', async () => {
  ...
  const duplicate = requestFor(jsonEnvelope(), {
...
test('declared and observed byte bounds are exact and malformed lengths fail before reading', async () => {
  for (const value of ['-1', '1.5', '01', 'NaN', [], Number.MAX_SAFE_INTEGER + 1]) {
    ...
    assertFailure(await adapterWith().handle(request), 400);
    assert.equal(request.listenerCount('data'), 0);
  }
  ...
  assertFailure(await adapterWith().handle(oversized), 413);
  assert.equal(oversized.listenerCount('data'), 0);
  const empty = jsonEnvelope({ padding: '' });
  const exact = jsonEnvelope({ padding: 'a'.repeat(EXTERNAL_CLIENT_HTTP_MAX_BODY_BYTES - Buffer.byteLength(empty)) });
  assert.equal(Buffer.byteLength(exact), EXTERNAL_CLIENT_HTTP_MAX_BODY_BYTES);
  assert.equal((await adapterWith().handle(requestFor(exact, { contentLength: true }))).statusCode, 201);
  const observed = requestFor(Buffer.concat([Buffer.from(exact), Buffer.from(' ')]));
  assertFailure(await adapterWith().handle(observed), 413);
  assert.equal(observed.destroyedByAdapter, true);
});
...
test('stream error, abort and premature close settle once, detach listeners and never delegate', async () => {
  for (const event of ['error', 'aborted', 'close']) {
    ...
    assert.equal(calls, 0);
    assert.equal(request.destroyedByAdapter, true);
    for (const name of ['data', 'end', 'error', 'aborted', 'close']) assert.equal(request.listenerCount(name), 0);
  }
});
test('read timeout returns 408, destroys the stream, detaches listeners and does not retry', async () => {
  const request = new Request({ headers: { 'content-type': 'application/json' } });
  let calls = 0;
...
test('module remains isolated, server route stays absent and npm dry-run excludes both internal files', () => {
  ...
  assert.equal(files.some((name) => name.includes('external-client-http-adapter')), false);
});
