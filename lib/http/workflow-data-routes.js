...
// #1287: tool_approvals has no workspace column of its own -- workspaceId
// lives inside context_json -- so it cannot be pushed into the SQL WHERE
// clause the way `tool` can. Filtering the *requested page* (limit, capped
// at 100) before applying the tool/workspace filter meant another tenant's
// pending approvals ahead of ours in `updated_at DESC` order could fill the
// whole page, leaving the caller's own queue looking empty with no error.
// Widen the fetched window well past any realistic per-workspace queue
// depth before filtering, so the caller's own approvals are essentially
// never pushed out by another tenant's. This is the same fetch-then-filter
// approach test/tool-approval-recovery-paging.test.js already uses for this
// exact table/column shortcoming.
const APPROVAL_WINDOW_CAP = 5000;
...
function createWorkflowDataRoutes({
  ...
  return async function handleWorkflowDataRoute(req, res, reqUrl) {
    ...
    if (ingestRunMatch) {
      ...
      const approval = formatApprovalRecord(runId && store.getToolApprovalById(runId));
    ...
    if (!approvalId) {
      ...
      const window = store.listUnresolvedToolApprovals(APPROVAL_WINDOW_CAP)
        .map(formatApprovalRecord)
        .filter(item => item && item.tool === 'http.ingest' && approvalWorkspace(item) === workspace.workspaceId);
      const approvals = window.slice(0, limit);
      // #1287: honest even at the (very unlikely) point where this
      // workspace alone has more than APPROVAL_WINDOW_CAP unresolved
      // approvals -- windowTruncated tells the caller their queue may be
      // larger than shown, rather than letting `total` silently under-report.
      const windowTruncated = window.length >= APPROVAL_WINDOW_CAP;
      writeJson(req, res, 200, {
        workflowId,
        ...workflowEnvelope({ ok: true, status: 'completed', data: { approvals, total: window.length, windowTruncated, workspaceId: workspace.workspaceId } }),
      }, NO_STORE);
      return true;
    }
