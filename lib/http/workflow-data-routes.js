...
function createWorkflowDataRoutes({
  ...
  if (![getApprovalStore, decideApproval, readReceipt, parseJsonRequest, writeJson, learnDocument, submitIngest].every(fn => typeof fn === 'function')) {
  ...
  return async function handleWorkflowDataRoute(req, res, reqUrl) {
    ...
    if (!approvalMatch && !receiptMatch && !ingestPreview && !ingestExecute && !learnReview) return false;
    ...
    if (ingestExecute) {
      ...
      writeJson(req, res, outcome.status, {
        ...
        ...workflowEnvelope({ ok: false, status: 'review_required', data: outcome.json }),
      }, NO_STORE);
      return true;
    }
