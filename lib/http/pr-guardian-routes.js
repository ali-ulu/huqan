...
function createPrGuardianRoutes(options = {}) {
  ...
  function fail(req, res, status, code, message) {
    writeNoStore(writeJson, req, res, status, { ok: false, status: 'failed', error: { code, message } });
  }
  ...
  const routeWebhook = createPrGuardianWebhookRoute({
    ...
    staticClient,
    service,
    write: (req, res, status, payload) => writeNoStore(writeJson, req, res, status, payload, NO_STORE),
    fail,
    logWebhookFailure,
  });
  ...
  async function route(req, res, reqUrl) {
    ...
    if (pathname === UI_PATHNAME) {
      // Do not claim the path when the console has no backend to drive (#1826).
      // Returning false lets it reach the generic 404, which is the same answer
      // the review routes already give, so the shell cannot advertise a
    ...
    if (pathname === REVIEWS_PATHNAME && req.method === 'GET') {
      ...
      writeNoStore(writeJson, req, res, 200, { ok: true, status: 'completed', data: { reviews, returned: reviews.length, limit } });
      return true;
    }
    ...
    if (pathname === REVIEWS_PATHNAME && req.method === 'POST') {
      ...
      try {
        ...
        writeNoStore(writeJson, req, res, result.decision === 'block' ? 409 : 202, result);
      } catch (error) {
    ...
    if (match) {
      ...
      if (decisionMatch) {
        ...
        writeNoStore(writeJson, req, res, result.status || (result.ok ? 200 : 400), result);
      } else {
    ...
    if (pathname === DRY_RUN_PATHNAME && req.method === 'POST') {
      ...
      try {
        ...
        writeNoStore(writeJson, req, res, 200, result);
      } catch (error) {
        fail(req, res, 400, error.code || 'PR_SNAPSHOT_INVALID', error.message || 'PR snapshot is invalid.');
      }
      return true;
