...
const { NO_STORE, readRawBody, parseBody, write, getHeader } = require('./pr-guardian-body');
  ...
  async function route(req, res, reqUrl) {
    ...
    if (pathname === WEBHOOK_PATHNAME) {
      ...
      try { rawBody = await readRawBody(req); } catch (error) {
        webhookFail(req, res, error.code === 'REQUEST_TOO_LARGE' ? 413 : 400, error.code || 'WEBHOOK_BODY_INVALID', 'Webhook body could not be read.');
        return true;
      }
      ...
      try {
        ...
        webhookFail(req, res, 400, error.code || 'PR_SNAPSHOT_INVALID', error.message || 'PR snapshot is invalid.');
      }
      return true;
    }
