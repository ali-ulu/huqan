...
const { createCommandPolicyBoundary } = require('./command-policy-route');
...
function createOptionalRouteBoundaries(options = {}) {
  const commandPolicy = createCommandPolicyBoundary(options.commandPolicy);
  ...
  const receiptCollector = receiptCollectorRoot
    ? createExternalActionReceiptCollectorRoute({
      ...
      trustedKeys: receiptCollectorOptions.trustedKeys || {},
      requireSignature: receiptCollectorOptions.requireSignature === true,
      sealKey: receiptCollectorOptions.sealKey || null,
    })
    : () => false;
