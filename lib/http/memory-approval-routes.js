 ...
 * and the operator token that authorizes the decision, presented in
 * `x-huqan-operator-token`. Configuring the API key alone does not create an
 * approval path.
 *
...
/**
 * Constant-time via the shared requestGuards helper, which hashes both operands
 * so the compared buffers are always the same size.
 *
 * This used to do its own `length === length &&` check in front of
 * timingSafeEqual, and said so — mirroring mcpServer.js rather than inventing a
 * second rule for the same secret. The rule it mirrored was the leaky one:
 * `&&` short-circuits, so a length mismatch skipped the comparison entirely and
 * returned measurably sooner, leaking the configured token's length. This route
 * is reachable over the network, so it was the most exposed of the three copies
 * (#1038). One shared helper is what keeps them from drifting again.
 */
function operatorAuthorized(configuredToken, presentedToken) {
  if (typeof configuredToken !== 'string' || typeof presentedToken !== 'string') return false;
...
function readOperatorToken(req) {
  const raw = req && req.headers ? req.headers[OPERATOR_TOKEN_HEADER] : '';
...
function createMemoryApprovalRoutes(options = {}) {
  ...
  async function route(req, res, reqUrl) {
    ...
    if (!operatorAuthorized(operatorToken, readOperatorToken(req))) {
      fail(req, res, 403, 'OPERATOR_AUTH_REQUIRED',
        `A separate operator capability is required. Present it in ${OPERATOR_TOKEN_HEADER}.`);
      return true;
    }

    // callTool compares the token in `params` against the one in `runtime`, so
    // both halves have to carry it. Supplying only the request side would fail
    // every call with OPERATOR_AUTH_REQUIRED after this route had already
    // authorized it.
    const runtime = {
    ...
    if (!decisionMatch) {
      ...
      const result = await callTool(kernel, {
        name: 'huqan.approvals', operatorToken, arguments: { limit, workspaceId },
      }, runtime);
    ...
    const result = await callTool(kernel, {
      name: 'huqan.approve',
      operatorToken,
      arguments: { approvalId, workspaceId, decision, reason: String(body.reason || '') },
    }, runtime);
    writeJson(req, res, result && result.ok === false ? 400 : 200, result, NO_STORE);
    return true;
  }
