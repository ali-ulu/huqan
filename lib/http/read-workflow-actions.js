...
const { runWebResearchWorkflow } = require('../web-research-workflow');
...
/**
 * The text a search query is allowed to match: what the node is called, not how
 * it is stored.
 *
 * Matching `JSON.stringify(node)` meant the query also ran against field names,
 * timestamps, workspaceId, numeric weights and JSON punctuation -- so `default`,
 * `provenance`, `created`, `2026` and even `{` matched every node in the
 * workspace, and nothing in the returned records showed where the match came
 * from.
 *
 * @param {object} node
 * @returns {string}
...
function searchableText(node) {
  ...
  return [node?.label, node?.name, node?.id, ...tags].filter(Boolean).join(' ');
}

function searchMemory(graph, input) {
  const workspaceId = text(input.workspaceId, 128);
  const query = normalizeAlias(text(input.query || input.claim || input.node, 300));
  if (!workspaceId || !query) return null;

  const nodes = graph.getNodes(workspaceId);
  const matches = [];
  for (const [id, node] of Object.entries(nodes)) {
    if (!normalizeAlias(`${id} ${searchableText(node)}`).includes(query)) continue;
    matches.push([id, node]);
    // One past the cap: enough to know the result was trimmed, and it stops the
    // scan there instead of serializing the rest of the workspace for results
    // that would be discarded anyway.
    if (matches.length > MAX_SEARCH_RESULTS) break;
  }

  const truncated = matches.length > MAX_SEARCH_RESULTS;
  const items = matches.slice(0, MAX_SEARCH_RESULTS).map(([id, node]) => ({
    id,
...
/**
 * The answer a surface gives when it deliberately does not enable plugin
 * capabilities -- the MCP surface, whose kernel is built with
 * `loadPlugins: false` (lib/mcp-approval-store.js).
 *
 * It exists so that decision is *stated*. `ensureCapabilities` used to default
 * to an anonymous no-op, which meant a caller that simply forgot it was
 * indistinguishable from one that had thought about it: the workflow would
 * quietly report capability_not_available for a capability that should have
 * been available, and nothing would point at the missing wiring (#2001).
 */
const NO_CAPABILITY_ENSURE = () => {};
...
async function runReadWorkflow({ workflowId, kernel, input = {}, ensureCapabilities }) {
  ...
  // silently downgrade a capability at the far end of the call.
  if (typeof ensureCapabilities !== 'function') {
    throw new TypeError('runReadWorkflow requires ensureCapabilities; pass NO_CAPABILITY_ENSURE to opt out deliberately');
  }
  const workspaceId = text(input.workspaceId, 128);
  if (!workspaceId) return invalid('workspaceId is required');

  if (workflowId === 'web-research') return runWebResearchWorkflow(input, { kernel });
  ...
  if (workflowId === 'memory-search') {
    const search = searchMemory(kernel.graph, input);
    if (!search) return invalid('workspaceId and query are required');
    // `total` counted `items` after the cap, so a workspace with 20 000 matches
    // reported total: 50 with no sign of trimming. `returned` says what the
    // number is, and `truncated` says whether more exist.
    return {
      statusCode: 200,
      body: completed({
        data: {
...
function createReadWorkflowHttpRouter(options) {
  ...
  if (typeof ensureCapabilities !== 'function') {
    throw new TypeError('createReadWorkflowHttpRouter requires ensureCapabilities; pass NO_CAPABILITY_ENSURE to opt out deliberately');
  }
  return async function handleReadWorkflow(req, res, reqUrl) {
    const match = reqUrl.pathname.match(/^\/api\/v2\/workflows\/(ask|verify|advocate|search|research)$/);
    ...
    const workflowId = match[1] === 'research' ? 'web-research' : match[1] === 'search' ? 'memory-search' : match[1];
    const validationError = validateWorkflowHttpRequest(workflowId, input);
    if (validationError) {
      const result = invalid(validationError);
...
module.exports = { createReadWorkflowHttpRouter, runReadWorkflow, searchMemory, NO_CAPABILITY_ENSURE };
