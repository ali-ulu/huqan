...
const PUBLIC_ROUTES = Object.freeze([
  ...
  Object.freeze({
    ...
    methods: Object.freeze(['GET']),
    why: 'Read-only command surface, separately constrained by the public command allowlist in requestGuards.',
  }),
  Object.freeze({
    id: 'graph-data-default-scope',
    match: Object.freeze({ pathname: '/graph-data' }),
    methods: Object.freeze(['GET']),
    scope: 'default-workspace-only',
    why: 'Demo visualization of the default workspace only; other workspaces require an API key.',
  }),
]);
...
function resolveRouteAuthPolicy(pathname, method = 'GET', context = {}) {
  ...
  // GitHub cannot present HUQAN's API key. When explicitly enabled, this route
  // is public only at the HTTP-policy layer and authenticates every POST using
  // GitHub's raw-body HMAC before parsing or persistence. When disabled it is
  // deliberately indistinguishable from an unknown route.
  if (normalizedPath === '/api/github-app/webhook') {
    if (context.githubAppBetaRouteEnabled !== true) {
      return { known: false, authRequired: false, ruleId: 'unknown', reason: 'unknown_route' };
    }
    return {
      known: true,
      authRequired: false,
      ruleId: 'github-app-beta-webhook',
      reason: normalizedMethod === 'POST' ? 'github_hmac_authenticated' : 'method_not_supported',
    };
  }
