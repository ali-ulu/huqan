 ...
 *          and stamped source/updated_at/evidence on all of them, so one
...
function applyTemporalEdgeMetadata(graph, options = {}) {
  ...
  for (const edge of graph._edges) {
    ...
    // A newly created edge already has a persistence timestamp from addEdge();
    // replace it with this operation's explicit learnedAt so the metadata survives
    // reload with the same temporal meaning as the in-memory record.
    if (created.has(key)) edge.created_at = timestamp;
...
function downgradeEdge(graph, spec = {}) {
  ...
  graph._mutationRollback?.recordEdge(live);
  const previous = {
    to: live.to,
