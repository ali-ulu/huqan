 ...
 * @param {string} [deps.operatorToken]        trusted CLI operator secret
 * @param {Function} [deps.createOperatorCapability]  scoped capability factory
...
function runQuickstart(deps = {}) {
  ...
  const runtime = { approvalStore, operatorSecret: operatorToken };
  ...
  const approvalArguments = { approvalId, decision: 'approved', workspaceId: 'default' };
  const approvalCapability = typeof createOperatorCapability === 'function'
    ? createOperatorCapability({ tool: 'huqan.approve', arguments: approvalArguments })
    : null;
  const approved = callTool(kernel, {
    name: 'huqan.approve',
    ...(approvalCapability ? { operatorCapability: approvalCapability } : {}),
...
function formatQuickstartResult(result, opts = {}) {
  ...
  lines.push('');
  // Do not point at `verify: smoking causes cancer` here (#1695). The claim
  // above lived in the throwaway store this run just deleted, so verifying it
  // against the reader's own graph answers `unknown (confidence: 0.00)` --
  // three lines under a receipt that said `verified (0.90)`, which reads as the
  // product contradicting itself. Suggest commands that work on an empty graph.
  lines.push('Your own graph is still empty: the run above used the throwaway store.');
  lines.push('Next: `huqan status` for what is in your graph, `huqan approvals` for pending');
  lines.push('decisions, or `huqan` for the interactive shell.');
  return lines.join('\n');
}
