 ...
 * CLI short-circuits *before* `axiom.learn` runs, so nothing is enqueued and
 ...
 * ---------------------
 * It does not weaken any gate. `axiom.learn` still returns `review`, an
 * approval row is still persisted, and the canonical write still happens only
 * through `axiom.approve`. The approval id is printed so the audit trail the
...
function runQuickstart(deps = {}) {
  ...
  const learn = callTool(kernel, { name: 'axiom.learn', arguments: { text: statement } }, runtime);
  const approvalId = learn?.approval?.id || null;
  if (!approvalId) {
    record('propose', false, learn?.error?.message || 'axiom.learn returned no approval id');
    return fail('QUICKSTART_NO_APPROVAL', 'axiom.learn did not queue an approval; nothing was written.');
  }
  record('propose', true, `axiom.learn -> ${learn?.gate?.decision || 'review'} (${learn?.gate?.reason || 'mutating_requires_review'}), approval ${approvalId}`);
  ...
  record('approve', true, `axiom.approve -> ${approved?.data?.decision || 'approved'} (actor ${QUICKSTART_ACTOR})`);
...
function formatQuickstartResult(result, opts = {}) {
  ...
  if (opts.storePath) {
    lines.push('');
    lines.push(`Demo store (throwaway, your own memory was not touched): ${opts.storePath}`);
  }
  lines.push('');
