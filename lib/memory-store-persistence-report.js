...
/**
 * Build the stable persistence failure envelope shared by MemoryStore and its
 * persistence adapters. Keeping the formatter here avoids making adapters
 * reach into MemoryStore private methods.
 * @param {string} operation
 * @param {Error} err
 * @returns {{ok:false,error:{code:string,operation:string,message:string}}}
 */
function persistenceError(operation, err) {
  return {
    ok: false,
    error: {
...
module.exports = { PERSISTENCE_DISABLED, saveResult, loadResult, persistenceError };
