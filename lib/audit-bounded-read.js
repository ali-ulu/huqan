...
function lastMemoryEventByAuditId(source, auditId) {
  const events = Array.isArray(source?._auditEvents) ? source._auditEvents : [];
...
function assertMemoryAgreement(source, durableEvent) {
  const memoryEvent = lastMemoryEventByAuditId(source, durableEvent.auditId);
...
function sqliteStatements(db) {
  ...
  return {
    ...
    exists: db.prepare('SELECT 1 AS present FROM audit_log WHERE audit_id = ? AND workspace_id = ?'),
  };
}
...
function* iterateSqlite(source, workspaceId, maxDetailsBytes) {
  ...
  while (row) {
    ...
    assertMemoryAgreement(source, durableEvent);
    yield durableEvent;
  ...
  for (const event of source._auditEvents || []) {
    if (getAuditEvents([event], { workspaceId }).length === 0) continue;
    if (!statements.exists.get(event.auditId, workspaceId)) {
      throw codedError(AUDIT_EVENT_SOURCE_DIVERGENCE_CODE,
        `in-memory audit event has no durable row for ${event.auditId}`, { auditId: event.auditId });
    }
  }
}
