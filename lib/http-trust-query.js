...
function readTrustFilters(reqUrl) {
  ...
  return {
    ...
    // Bounded audit paging (#729). The clamp itself lives in
    // lib/audit-query.clampAuditLimit, which owns the servable range; here we
    // only read and length-cap the raw parameters.
    limit: readEnum('limit'),
    cursor: sanitizeInput(params.get('cursor') || '', TRUST_FILTER_MAX_REF),
  };
}
...
/**
 * The reader-risk `intent` for a #2788 claim-read request: `{category}`
 * and/or `{riskScore}`, the two shapes a GET query string can carry (a
 * `blastRadius` intent is a computeBlastRadius() result object -- for a
 * library caller holding one already, not something reconstructed from a
 * query string). Absent fields are simply omitted, matching
 * resolveReaderRiskLevel()'s "no intent at all" -> `intent_absent` path when
 * neither is present.
 */
function readClaimReadIntent(reqUrl) {
  const params = reqUrl.searchParams;
  const intent = {};
  const category = sanitizeInput(params.get('category') || '', TRUST_FILTER_MAX_ENUM);
  if (category) intent.category = category;
  const rawScore = params.get('riskScore');
  if (rawScore !== null && rawScore !== '') {
    const riskScore = Number(rawScore);
    if (Number.isFinite(riskScore)) intent.riskScore = riskScore;
  }
  return intent;
}
