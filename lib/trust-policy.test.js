...
// #1297: os.tmpdir() is no longer a default-allowed policy root (it is
// world-writable shared storage on a multi-user system, not something the
// operator controls). Tests that need a scratch directory under it now grant
// that one directory explicitly through the operator-controlled
// TRUST_POLICY_ROOTS extension mechanism, the same way a real deployment
// would opt a non-default directory in.
...
test('loadTrustPolicy rejects a path under os.tmpdir() when it is not an explicitly allowed root (#1297)', () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'axiom-policy-untrusted-'));
  try {
    const policyPath = path.join(tempDir, 'policy.json');
    fs.writeFileSync(policyPath, JSON.stringify({ version: '9.9.9' }));
...
test('loadTrustPolicy rejects symlink targets outside the allowed roots (#383)', (t) => {
  ...
  try {
    ...
    withPolicyRoot(tempDir, () => {
      // tempDir (where the symlink lives) is now an allowed root, but
      // outsideDir (what it resolves to) is not -- so this still exercises
      // the realpath check, not the base-root grant.
      assert.throws(
