...
test('normalizeGitHubItem redacts caller supplied GitHub URL credentials and selectors', () => {
  const raw = 'https://user:secret@github.com/owner/repo/pull/123?token=secret#fragment';
  const normalized = normalizeGitHubItem(makeBaseItem({
    url: raw,
    sourceRef: raw,
...
test('ingestGitHubItem creates candidate provenance and imported audit', () => {
  ...
  assert.strictEqual(Object.hasOwn(result, 'mutationOperationId'), false);
  assert.strictEqual(Object.hasOwn(result.admission, 'mutationOperationId'), false);
...
test('ingestGitHubItem writes connector audits through the public graph contract (#2180)', () => {
  const kernel = new Kernel({ noLoad: true, useSQLite: false, ...makePaths('public-audit-boundary') });
  const originalPrivate = kernel._appendAuditEvent.bind(kernel);
  const originalPublic = kernel.graph.appendAuditEvent.bind(kernel.graph);
  let privateCalls = 0;
  let publicCalls = 0;

  kernel._appendAuditEvent = (...args) => {
    privateCalls++;
    return originalPrivate(...args);
  };
  kernel.graph.appendAuditEvent = (...args) => {
    publicCalls++;
    return originalPublic(...args);
  };
  ...
  assert.strictEqual(result.duplicate, false);
  assert.strictEqual(privateCalls, 0, 'connector must not reach through Kernel private audit methods');
  assert.ok(publicCalls > 0, 'connector audit must use Graph.appendAuditEvent');
  assert.strictEqual(
    kernel.graph.getAuditEvents({ eventType: AUDIT_EVENTS.IMPORTED, workspaceId: 'workspace-a' }).length,
    1,
  );
});
...
test('ingestGitHubItem accepts explicit canonical writes when requested', () => {
  ...
  assert.strictEqual(result.admission.canonical, true);
  assert.strictEqual(Object.hasOwn(result, 'mutationOperationId'), false);
  assert.strictEqual(Object.hasOwn(result.admission, 'mutationOperationId'), false);
