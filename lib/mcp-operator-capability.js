...
/**
 * Record a nonce as consumed, answering false when it was already spent.
 *
 * Three store shapes are accepted, and the order matters (#1674):
 *
 *   - An object with a `consume(nonce, exp, now)` method: the durable store in
 *     lib/mcp-capability-nonce-store.js, whose reservation is atomic across
 *     processes and survives a restart. This is the production shape.
 *   - A Map or Set: process-local, kept for in-process callers and tests. It
 *     forgets everything a restart takes with it, which is exactly the gap
 *     the durable store closes.
 *   - Anything else, including a store whose consume() throws: refused. An
 *     unusable nonce store must deny the capability, never wave it through.
 *
 * A missing store (undefined) still means "no replay protection requested" --
 * verifyMcpOperatorCapability() is also used to check a capability without
 * spending it.
 */
function consumeNonce(nonceStore, nonce, exp, now) {
  ...
  if (typeof nonceStore.consume === 'function') {
    try {
      return nonceStore.consume(nonce, exp, Number(now)) === true;
    } catch (_) {
      return false;
    }
  }
