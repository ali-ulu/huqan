...
function classifyChangedFile(file, context = {}) {
  ...
  if (isExactSensitiveFilePath(filePath) || isSecretLikeValue({ status: normalized.status, changeType: normalized.changeType, additions: normalized.additions, deletions: normalized.deletions }, SECRET_HINTS)) {
    return {
      ok: true,
      path: filePath,
      status: normalized.status,
