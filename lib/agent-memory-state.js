...
} = require('./default-persistence-path');
...
function normalizeMemoryPath(opts = {}, kernel) {
  ...
  const defaultPath = path.join(process.cwd(), 'agent.memory.json');
  if (isTestRunner() && isInsideRepo(defaultPath)) {
    return path.join(testRunPersistenceRoot(), 'agent.memory.json');
  }
  return defaultPath;
}
