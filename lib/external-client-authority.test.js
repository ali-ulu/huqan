...
test('exact authority allows once, reserves before return and exposes frozen secret-free evidence', async () => {
  ...
  assert.deepEqual(store.calls[0], {
    ...
    reservedAt: NOW,
    expiresAt: NOW + EXTERNAL_CLIENT_REPLAY_TTL_MS,
  });
...
test('exact key validity and freshness boundaries are accepted', async () => {
  const { publicKey, privateKey } = crypto.generateKeyPairSync('ed25519');
  const cases = [
    ['2026-08-02T11:00:00.000Z', () => Date.parse('2026-08-02T11:00:00.000Z')],
    ['2026-08-02T13:00:00.000Z', () => Date.parse('2026-08-02T13:00:00.000Z')],
    [new Date(NOW - EXTERNAL_CLIENT_MAX_PACKAGE_AGE_MS).toISOString(), () => NOW],
    [new Date(NOW + EXTERNAL_CLIENT_MAX_FUTURE_SKEW_MS).toISOString(), () => NOW],
  ];
  for (const [createdAt, clock] of cases) {
    const result = await enforceExternalClientAuthority(input(pkg(createdAt), privateKey),
      snapshotExternalClientAuthority(options(publicKey, { clock })));
...
test('replay result classification accepts only canonical success and bounds hostile shapes', async () => {
  ...
  const inherited = Object.create({ reserved: true });
  ...
  for (const result of [nonEnumerable, accessor, inherited, symbolExtended, extraField, ...hostileProxies]) {
    await rejectsCode(enforceExternalClientAuthority(input(pkg(), privateKey),
