...
const { createHuqanPackage } = require('./huqan-package-format');
const {
...
test('package object-count integrity errors are rejected (#1119)', () => {
  const pkg = makePackage();
  ...
  assert.throws(
    ...
    (error) => {
      assert.equal(error.code, EXTERNAL_CLIENT_PACKAGE_GATE_ERRORS.INVALID_PACKAGE);
      assert.ok(error.details.errors.some((entry) => (
        entry.code === 'PACKAGE_OBJECT_COUNT_MISMATCH'
          && /declares 1 but package embeds 0/.test(entry.message)
      )));
      return true;
    },
  );
...
test('revoked trusted signing keys fail closed before signature acceptance', () => {
  const pkg = makePackage();
  const { publicKey, privateKey } = makeKeys();
  const options = gateOptions(publicKey, {
    trustedKeys: {
      'trusted-key-1': trustedEntry(publicKey, { status: 'revoked' }),
    },
  });
...
test('expired trusted signing keys fail closed using the gate evaluation time', () => {
  const pkg = makePackage();
  const { publicKey, privateKey } = makeKeys();
  const options = gateOptions(publicKey, {
    now: Date.parse('2026-08-01T00:00:00.000Z'),
    trustedKeys: {
      'trusted-key-1': trustedEntry(publicKey, { expiresAt: '2026-07-31T23:59:59.000Z' }),
    },
  });
...
test('malformed trusted-key status metadata fails closed', () => {
  const pkg = makePackage();
  const { publicKey, privateKey } = makeKeys();
  const options = gateOptions(publicKey, {
    trustedKeys: {
      'trusted-key-1': trustedEntry(publicKey, { status: 'suspended' }),
    },
  });
...
// The three internal-only source types (`manual`, `decision`,
// `background_inference`) exist in the runtime set in provenance-ingest.js but
// deliberately not in ATP's SOURCE_TYPES, which is what this gate applies to
// packages arriving from outside. The asymmetry is the boundary: the kernel may
// record `background_inference` about its own bookkeeping, an external client
// may not assert it about itself. Widening SOURCE_TYPES to "agree" with the
// runtime set would open that boundary, so these two cases pin it (#2040).
function makeProvenanceRecord(sourceType) {
  return {
    provenanceId: 'prov.external.1',
    sourceRef: 'huqan://test/package',
    sourceTitle: 'External client submission',
    sourceType,
...
function packageCarryingSourceType(sourceType) {
  const pkg = makePackage();
  pkg.objects.provenanceRecords = [makeProvenanceRecord(sourceType)];
  pkg.manifest.objectCounts.provenanceRecords = 1;
  return pkg;
}

test('an external client may not claim an internal-only source type', () => {
  const { publicKey, privateKey } = makeKeys();

  for (const internalOnly of ['manual', 'decision', 'background_inference']) {
    assert.throws(
      () => enforceExternalClientPackage(
        makeInput(packageCarryingSourceType(internalOnly), privateKey),
        gateOptions(publicKey),
      ),
      (error) => {
        assert.equal(
          ...
          `sourceType '${internalOnly}' is internal-only and must not survive the external client gate`,
        );
        const fields = (error.details?.errors || []).map((entry) => entry.field || '');
        assert.ok(
          fields.some((field) => field.endsWith('.sourceType')),
          `rejection must name sourceType as the reason, got fields: ${JSON.stringify(fields)}`,
        );
        return true;
      },
    );
...
test('an external client may still assert an ordinary ATP source type', () => {
  const { publicKey, privateKey } = makeKeys();

  const result = enforceExternalClientPackage(
    makeInput(packageCarryingSourceType('document'), privateKey),
    gateOptions(publicKey),
  );
