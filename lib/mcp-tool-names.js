...
/**
 * The bare, namespace-free tool name behind a HUQAN MCP tool name, or null.
 *
 * `huqan.learn` and `axiom.learn` both answer `learn`. Anything else — an
 * unknown suffix, a bare name, an arbitrary string — answers null.
 *
 * This exists because the namespace is presentation, not identity: a policy or
 * gate keyed on bare names must reach the same decision whichever spelling it
 * is handed, or the product contradicts itself about its own tools. It is
 * deliberately strict about the suffix. `huqan.rm -rf /` is not a HUQAN tool,
 * so it answers null and its caller keeps the full string, which is what lets
 * the caller's unknown/destructive handling stay fail-closed instead of
 * silently unwrapping to a suffix nobody classified.
 */
function localMcpToolName(name) {
  if (typeof name !== 'string') return null;
  if (CANONICAL_SET.has(name)) return name.slice(CANONICAL_MCP_TOOL_PREFIX.length);
  if (LEGACY_SET.has(name)) return name.slice(LEGACY_MCP_TOOL_PREFIX.length);
  return null;
}
...
function withMcpToolDeprecationSurface(result, requestedName) {
  ...
  // The migration counter lives here rather than in canonicalMcpToolName, which
  // is a pure helper the request path calls eight or more times per call --
  // counting there would measure helper invocations, not migrations, and the
  // number would be unusable as the denominator #1824 needs. This wraps each
  // tools/call result exactly once.
  recordLegacyAliasUse('mcp_tool', deprecation.requestedName);
