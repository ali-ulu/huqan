...
function normalizeEntry(entry, context = {}) {
  ...
  // `deleted` no longer inherits `tombstoned` (#1257). A tombstone is a
  // reversible soft delete and the classifier has a path for it -- the graph
  // branch, which answers review or dry-run-only. But isDestructiveDelete
  // (`entry.deleted || ...`) is checked before hasGraphMutation, so inheriting
  // the flag routed every tombstone into the hard-delete branch and blocked it
  // unconditionally, with `CANONICAL_GRAPH_MUTATION_BLOCKED` as the reason.
  // That also made hasGraphMutation's own `entry.tombstoned` check dead code.
  const deleted = Boolean(raw.deleted ?? raw.deletedAt);
