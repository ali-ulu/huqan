...
function hasSecretLookingValue(value, keyPath = []) {
  if (keyLooksSecret(keyPath[keyPath.length - 1])) return true;
...
/**
 * Returns a deep copy of `value` with any secret-looking leaf (by key name
 * or by value shape — same detection rules as hasSecretLookingValue) replaced
 * with a fixed placeholder. Structure and non-secret fields are preserved so
 * the redacted copy stays useful for review/audit.
 */
function redactSecretValues(value, keyPath = []) {
  if (keyLooksSecret(keyPath[keyPath.length - 1])) return REDACTED_PLACEHOLDER;

  if (typeof value === 'string') return valueLooksSecret(value) ? REDACTED_PLACEHOLDER : value;

  if (Array.isArray(value)) {
    return value.map((item, index) => redactSecretValues(item, keyPath.concat(String(index))));
  }
  ...
  for (const [key, nested] of Object.entries(value)) {
    out[key] = redactSecretValues(nested, keyPath.concat(key));
  }
  return out;
}
