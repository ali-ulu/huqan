...
function verifyAgainstGraph(service, ctx) {
  ...
  const typeConflict = target && target !== subject
    ...
    : null;
  if (typeConflict) {
    return service.verifyResult(statement, opts, { status: 'contradicted', confidence: typeConflict.confidence || 0.9 }, typeConflict.evidence || [{
      kind: 'contradiction',
  ...
  if (subjCons.length > 0) {
    ...
    return service.verifyResult(statement, opts, { status: 'contradicted', confidence: 0.7 }, evidence, verifyContext);
  }
  ...
  if (target !== subject) {
    ...
    if (pathResult.path && !incomingAbsolute) {
      ...
      return service.verifyResult(statement, opts, { status: 'verified', confidence }, [service.host.pathEvidence(pathResult.path, 'path', confidence)], { ...verifyContext, pathSearch: pathResult });
    }
  }
    ...
    for (const edge of edges) {
      ...
      if (edgeNums) {
        ...
        if (mismatch) {
          ...
          if (hasTextOverlap) {
            return service.verifyResult(statement, opts, { status: 'contradicted', confidence: 0.75 }, [{
              kind: 'contradiction',
  ...
  for (const word of parts.slice(1)) {
    ...
    if (match) {
      ...
      if (contradictionSignals.length > 0) {
        ...
        return service.verifyResult(statement, opts, { status: 'contradicted', confidence: 0.75 }, evidence, { ...verifyContext, fuzzy });
      }
      if (incomingAbsolute || fuzzy.isWeak) continue;
      return service.verifyResult(statement, opts, { status: 'verified', confidence: 0.35 }, [service.host.edgeEvidence(match, 'partial_match', 0.35)], { ...verifyContext, fuzzy });
    }
  }

  return service.verifyResult(statement, opts, { status: 'unknown', confidence: 0 }, [], verifyContext);
}
