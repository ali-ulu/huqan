...
test('source has no system-time, JSON, memory-authority, route, mutation or receipt fallback', () => {
  // The store is split across external-client-replay-store*.js (#2183): follow
  // its own require chain so every part is held to the same rule.
  const files = ['external-client-replay-store.js'];
  for (let i = 0; i < files.length; i += 1) {
    const text = fs.readFileSync(path.join(__dirname, files[i]), 'utf8');
    for (const [, name] of text.matchAll(/require\(['"]\.\/(external-client-replay-store[\w-]*)['"]\)/g)) {
      if (!files.includes(`${name}.js`)) files.push(`${name}.js`);
    }
  }
  assert.ok(files.length > 1, 'the replay store parts are reachable from its entry file');
