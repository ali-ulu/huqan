...
const postIncidentReview = require('./post-incident-review');
...
const POST_ACTION_REASONS = Object.freeze({
  ...
  // Default observe (#2157): an anomaly detected without a human activation is
  // surfaced, never silently contained.
  ANOMALY_OBSERVED: 'post_action_anomaly_observed',
});
...
function evaluatePostActionBehavior(input = {}, options = {}) {
  ...
  if (!activation) {
    // Default observe (#2157): monitoring runs so anomalies are still detected
    // and surfaced, but nothing is quarantined or demoted without a human
    // activation. Observation is not containment.
    ...
    const receiptSummary = Object.freeze({
      ...
      decision: anomaly ? 'observe_quarantine_required' : 'observe',
      reason: anomaly ? POST_ACTION_REASONS.ANOMALY_OBSERVED : POST_ACTION_REASONS.OBSERVED,
...
function postActionMonitoringOptions(options = {}) {
  ...
  const envFlag = text(environment.HUQAN_EXTERNAL_GUARD_CONTINUOUS_MONITORING);
  // Default observe (#2157): monitoring runs so signals are collected and
  // anomalies surfaced. A deployment that wants the pre-Faz-E behaviour back
  // opts out here; containment also still needs a human activation to apply.
...
module.exports = {
  ...
  recordPostIncidentReview: postIncidentReview.recordPostIncidentReview,
  readPostIncidentReviews: postIncidentReview.readPostIncidentReviews,
};
