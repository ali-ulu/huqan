...
const { signReceiptBundle, verifyReceiptBundleSignature } = require('./signed-bundle');
const {
...
function verifyExportedBundle(bundle, opts = {}) {
  ...
  // Existing (unsigned) exports remain readable. Callers that need issuer
  // authenticity can opt into the stronger contract explicitly.
  const signatureAcceptable = signed ? signatureValid : opts.requireSignature !== true;
