...
// `seen` tracks objects/arrays on the current recursion path (not every
// object visited overall), so the same object referenced from two different
// branches -- a DAG, which is fine -- is not mistaken for a cycle. Only an
// object that contains itself, directly or transitively, trips this and
// throws instead of recursing forever (#446).
...
function buildCanonicalReceiptPayload(receipt, opts = {}) {
  ...
  assertRequiredReceiptFields(receipt);
...
function assertRequiredReceiptFields(receipt) {
  for (const field of REQUIRED_RECEIPT_FIELDS) {
    const value = receipt[field];
    if (value === null || value === undefined || String(value).trim() === '') {
      throw new TypeError(`buildCanonicalReceiptPayload requires receipt.${field} to be present and non-empty`);
    }
  }
}
