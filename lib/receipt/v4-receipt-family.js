...
const V4_RECEIPT_ERROR_CODES = Object.freeze({
  ...
  INVALID_RECEIPT_FIELDS: 'INVALID_RECEIPT_FIELDS',
...
/**
 * Whether every required receipt field carries a usable value.
 *
 * REQUIRED_RECEIPT_FIELDS is the same list buildCanonicalReceiptPayload
 * enforces on the write path; applying it on the read path closes the gap
 * where a receipt could be written elsewhere, or hand-edited, and still pass
 * chain validation with empty identity fields.
 *
 * @param {object} payload
 * @returns {boolean}
 */
function hasRequiredReceiptFieldValues(payload) {
...
function validateV4RecordShape(record) {
  ...
  // Field semantics, for both versions. The key-set check above proves every
  // allowlisted field is *present*; it says nothing about the values. A record
  // whose hash is internally consistent but whose receiptId is an empty string
  // used to validate, which is not a trust guarantee -- and V1 is not a legacy
  // curiosity, every chain from genesis carries V1 records.
  if (!hasRequiredReceiptFieldValues(payload)) {
    return { valid: false, code: V4_RECEIPT_ERROR_CODES.INVALID_RECEIPT_FIELDS };
  }
