...
// Shared with the other gates: the local copy only did trim+toLowerCase, so an
// accented 'delete' slipped past the hints here but not in code-change-gate.
const { normalizeText } = require('./text-utils');
const {
  isInsideSandbox,
  isPathTraversal,
  looksLikeSandboxPath,
} = require('./sandbox-path-containment');
...
// Ceiling rules live in their own module (#1113); MAX_TIMEOUT_MS is re-exported.
const { MAX_TIMEOUT_MS, resolveTimeoutCeiling, describeTimeoutCeiling } = require('./sandbox-timeout-policy');
...
function classifySandboxOperation(context, policy = null) {
  ...
  if (context.sourceTrust === 'untrusted') {
    ...
    findings.push(makeFinding({
      ...
      decision: 'quarantine',
      reason: SANDBOX_ISOLATION_REASONS.UNKNOWN_SOURCE_TRUST_QUARANTINE,
      risk: 'medium',
      detail: 'Unknown source trust requires review before sandbox execution.',
    }));
    decision = mergeDecision(decision, 'quarantine');
    if (riskLevel === 'low') riskLevel = 'medium';
    riskScore = Math.max(riskScore, 0.5);
  ...
  if (context.timeoutMs > timeoutCeiling) {
    findings.push(makeFinding({
      code: 'HIGH_TIMEOUT',
      decision: 'block',
  ...
  if (context.runner === 'unknown') {
    findings.push(makeFinding({
      code: 'UNKNOWN_RUNNER',
      decision: 'block',
      reason: SANDBOX_ISOLATION_REASONS.UNKNOWN_RUNNER_BLOCK,
      risk: 'critical',
      detail: `Unknown runner type ${JSON.stringify(context.runner)} cannot be sandboxed safely.`,
    }));
    decision = mergeDecision(decision, 'block');
    riskLevel = 'critical';
    riskScore = 1.0;
