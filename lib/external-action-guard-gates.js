...
const { evaluateImpactBudget } = require('./impact-budget-gate');
...
function finalize(envelope, partial, options) {
  ...
  // #2505 A: durable session budget enforcement. Armed only by explicit
  // deployment options (`impactBudget: { graph, policy }`); without both,
  // this block is a no-op. Runs after every other gate and only ever
  // tightens: the strictest verdict wins, scores commit on allow only, and
  // degraded inputs hold for review (block for high-risk/irreversible).
  const budget = evaluateImpactBudget({
    envelope,
    decision: result.decision,
    riskLevel: result.risk && result.risk.level,
    options,
  });
  if (budget) {
    const merged = mergeDecision(result.decision, budget.finding.decision);
    result = {
      ...result,
      allowed: merged === EXTERNAL_ACTION_DECISIONS.ALLOW,
      canExecute: merged === EXTERNAL_ACTION_DECISIONS.ALLOW,
      requiredReview: merged === EXTERNAL_ACTION_DECISIONS.REVIEW,
      decision: merged,
      reason: merged === result.decision ? result.reason : budget.finding.reason,
      risk: merged === EXTERNAL_ACTION_DECISIONS.BLOCK
        ? {
          level: RISK_LEVELS.CRITICAL,
          score: Math.max(Number.isFinite(result.risk && result.risk.score) ? result.risk.score : 0, 95),
        }
        : result.risk,
      findings: [...result.findings, budget.finding],
    };
  }
