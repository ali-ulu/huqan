...
function runImportPackage(storeApi, pkg, opts = {}) {
  // Import admission resolves event/link dependencies against the post-memory-loop
  // target state, so preserve its existing selective dangling-record conflict
  // behavior while the public package validator remains fail-closed by default.
  const validation = validateMemoryPackage(pkg, { checkReferences: false });
  if (!validation.ok) {
