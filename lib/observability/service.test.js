...
test('observability persists workspace-scoped runs and paginated events', () => {
  ...
  try {
    ...
    advance(25);
    service.recordRunFinish({ workspaceId: 'ws-a', runId: 'run-a', goal: 'private goal', objective: 'verify', runtime: 'agent-v3', status: 'completed', startedAt: 1_700_000_000_000, finishedAt: 1_700_000_000_050, stepCount: 1, successfulSteps: 1, usage: { tokens: 12, costMicros: 240 } });
...
test('queue claims are leased, workspace-scoped and redacted in reads', () => {
  ...
  try {
    ...
    assert.deepEqual(service.queueSummary({ workspaceId: 'ws' }).byStatus, { completed: 1 });
  } finally {
    db.close();
  }
});
