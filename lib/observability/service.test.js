...
test('injected notification adapters receive alert lifecycle events without widening telemetry failures', () => {
  const db = new Database(':memory:');
  let clock = 1_700_000_000_000;
  const notifications = [];
  ...
  try {
    service.createAlertRule({ workspaceId: 'ws', metric: 'error_count', operator: 'gt', threshold: 0, windowMs: 60_000, cooldownMs: 5_000 });
    service.recordRunFinish({ workspaceId: 'ws', runId: 'failed', status: 'failed', errorSteps: 1 });
    assert.equal(notifications.length, 1);
    assert.equal(notifications[0].type, 'alert_firing');
    assert.equal(notifications[0].workspaceId, 'ws');
    assert.equal(notifications[0].alert.status, 'firing');
    assert.equal(notifications[0].notificationId, notifications[0].alert.eventId);

    const alert = service.listAlerts({ workspaceId: 'ws' })[0];
    service.acknowledgeAlert({ workspaceId: 'ws', alertId: alert.alertId, reason: 'operator-reviewed' });
    service.resolveAlert({ workspaceId: 'ws', alertId: alert.alertId, reason: 'operator-resolved' });
...
test('notification adapter failures do not abort alert persistence or lifecycle transitions', () => {
  const db = new Database(':memory:');
  const service = createObservabilityService({
    db,
    notificationAdapter: { deliver() { throw new Error('transport unavailable'); } },
  });
  try {
    service.createAlertRule({ workspaceId: 'ws', metric: 'error_count', operator: 'gt', threshold: 0 });
    assert.doesNotThrow(() => service.recordRunFinish({ workspaceId: 'ws', runId: 'failed', status: 'failed', errorSteps: 1 }));
    const alert = service.listAlerts({ workspaceId: 'ws' })[0];
    assert.equal(alert.status, 'firing');
    assert.doesNotThrow(() => service.acknowledgeAlert({ workspaceId: 'ws', alertId: alert.alertId }));
    assert.equal(service.listAlerts({ workspaceId: 'ws' })[0].status, 'acknowledged');
  } finally {
    db.close();
  }
});
...
test('internal metrics expose bounded workspace-scoped health signals without recursive events', () => {
  ...
  try {
    ...
    assert.equal(Number.isInteger(metrics.database.calls), true);
    assert.equal(metrics.database.calls > 0, true);
    assert.equal(metrics.database.totalDurationMs >= 0, true);
    assert.equal(Number.isInteger(metrics.database.slowCalls), true);
...
test('event write failures preserve the storage error and do not retain a failed row', () => {
  const { db, service } = serviceWithClock();
  try {
    service.recordStep({ workspaceId: 'ws', eventId: 'duplicate-event', runId: 'run-1', tool: 'verify', status: 'done' });
    assert.throws(
      () => service.recordStep({ workspaceId: 'ws', eventId: 'duplicate-event', runId: 'run-2', tool: 'verify', status: 'done' }),
      error => /^SQLITE_CONSTRAINT/.test(String(error.code)),
    );
    const metrics = service.internalMetrics({ workspaceId: 'ws' });
    assert.deepEqual(metrics.eventWrites, { attempted: 2, succeeded: 1, failed: 1 });
    assert.deepEqual(service.listEvents({ workspaceId: 'ws' }).items.map(event => event.eventId), ['duplicate-event']);
  } finally {
    db.close();
  }
});
