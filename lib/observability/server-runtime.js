...
const { createWebhookNotificationAdapter } = require('./notification');
...
const OPTIONAL_CONFIG_SUFFIXES = new Set([
  ...
  'OBSERVABILITY_RATE_LIMIT_POLICY',
  'OBSERVABILITY_RETENTION_POLICY',
  'OBSERVABILITY_WEBHOOK_POLICY',
]);
  ...
  agent.run = async (goal, runOptions = {}) => {
    ...
    try {
      ...
      for (const step of steps) {
        try { service.recordStep({ workspaceId, runId, agentId, status: step.status, tool: step.tool, result: step.output, payload: { stepId: step.id, phase: 'workflow' } }); } catch (_) {}
      }
      ...
      try { service.recordRunFinish({ workspaceId, runId, agentId, runtime: 'workflow', goal, status: 'failed', startedAt, finishedAt: Date.now(), errorCode: error?.code || 'WORKFLOW_RUN_FAILED' }); } catch (_) {}
      throw error;
    }
  };
  return agent;
...
function createObservabilityServerRuntime({
  ...
  function getNotificationAdapter() {
    notificationAdapter ||= createWebhookNotificationAdapter({ policy: readConfig('OBSERVABILITY_WEBHOOK_POLICY') });
  ...
  function getRateLimiter() {
    rateLimiter ||= createObservabilityRateLimiter({ policy: readConfig('OBSERVABILITY_RATE_LIMIT_POLICY') });
    return rateLimiter;
  }

  function startRetentionIfEnabled() {
    try {
      retention ||= createObservabilityRetention({
        db: getStorage().db,
        policy: readConfig('OBSERVABILITY_RETENTION_POLICY'),
      });
      if (!retention.policy.enabled) return false;
      const cleanup = () => retention.policy.workspaceIds.forEach(workspaceId => retention.cleanup({ workspaceId }));
      cleanup();
      retentionTimer ||= setInterval(cleanup, retention.policy.intervalMs);
      retentionTimer.unref?.();
      return true;
    } catch (error) {
      console.error('[observability-retention] disabled:', error.code || error.message);
      return false;
    }
  }
  ...
  const handleRoute = createObservabilityHttpRouter({
    ...
    rateLimiter: Object.freeze({
      check: input => getRateLimiter().check(input),
      acquireStream: input => getRateLimiter().acquireStream(input),
    }),
  });

  function startWorkerIfEnabled() {
    startRetentionIfEnabled();
