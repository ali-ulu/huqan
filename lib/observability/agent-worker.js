...
function createAgentWorker({ service, createAgent, intervalMs = 1000, leaseMs = 120000, workerId = `agent-worker-${crypto.randomUUID()}` } = {}) {
  ...
  async function tick() {
    if (busy) return null;
    busy = true;
    service.recordWorkerHealth?.({ enabled: Boolean(timer), busy: true });
    let job = null;
    let agent = null;
    try {
      ...
      service.recordWorkerHealth?.({ enabled: Boolean(timer), busy: true, failed: true });
      if (!job) return null;
      ...
      busy = false;
      service.recordWorkerHealth?.({ enabled: Boolean(timer), busy: false });
    }
  }
  ...
  function start() {
    ...
    timer.unref?.();
    service.recordWorkerHealth?.({ enabled: true, busy });
    return true;
  }
  ...
  function stop() {
    ...
    clearInterval(timer);
    timer = null;
    service.recordWorkerHealth?.({ enabled: false, busy });
    return true;
  }
