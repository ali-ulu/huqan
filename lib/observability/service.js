...
const { decodePageCursor } = require('./pagination');
const { createObservabilityPageReaders } = require('./page-readers');
...
function createObservabilityService({ db, now = Date.now, costPer1kTokensMicros = null, notificationAdapter = null, logger = console } = {}) {
  ...
  const { pageAlertRules, pageAlerts, pageQueue } = createObservabilityPageReaders({ db, now });
