...
function createObservabilityService({ db, now = Date.now, costPer1kTokensMicros = null } = {}) {
  ...
  function evaluateAlerts(workspaceId, event) {
    const rows = statements.getEnabledRules.all(workspaceId, MAX_ALERT_LIMIT);
    const fired = [];
    ...
    for (const row of rows) {
      ...
      if (!breached) {
        if (previous && ['firing', 'acknowledged'].includes(previous.status)) {
          ...
          const changed = statements.resolveAlert.run(resolvedAt, workspaceId, previous.alert_id).changes > 0;
          if (changed) {
            insertEvent({
              workspaceId,
              eventType: 'alert_resolved',
              status: 'resolved',
              payload: { alertId: previous.alert_id, ruleId: rule.ruleId, metric: rule.metric },
            }, { evaluate: false });
          }
        }
        continue;
      ...
      const fingerprint = crypto.createHash('sha256')
        .update(`${workspaceId}\0${rule.ruleId}\0${rule.metric}`, 'utf8')
        .digest('hex');
  ...
  function acknowledgeAlert({ workspaceId, alertId } = {}) {
    ...
    const changed = statements.acknowledgeAlert.run(nowMs(now), workspace, id).changes > 0;
    ...
    return projectAlert(statements.getAlert.get(workspace, id));
  }
