...
function createObservabilityService({ db, now = Date.now, costPer1kTokensMicros = null } = {}) {
  ...
  function insertEvent(input, { evaluate = true } = {}) {
    ...
    publish(event);
    if (evaluate && eventType !== 'alert_firing') alertLifecycle.evaluateAlerts(workspaceId, event);
    return event;
  }
  ...
  const alertLifecycle = createAlertLifecycle({ statements, insertEvent, summary, now });
