...
function normalizeWorkspaceId(value) {
  ...
  if (!workspaceId || workspaceId.length > 128) {
    const error = new Error('workspaceId is required and must be at most 128 characters.');
...
function safePayload(payload) {
  ...
  for (const [key, value] of Object.entries(payload)) {
    ...
    if (/goal|prompt|input|output|text|content|secret|token|key|authorization/i.test(key)) continue;
...
function createObservabilityService({ db, now = Date.now, costPer1kTokensMicros = null } = {}) {
  ...
  const statements = {
    ...
    retryJob: db.prepare(`UPDATE agent_queue_jobs SET status = ?, available_at = ?, lease_until = NULL,
      ...
      WHERE job_id = ? AND status = 'running' AND worker_id = ?`),
    recoverLeases: db.prepare(`UPDATE agent_queue_jobs SET status = 'queued', lease_until = NULL,
  ...
  function publish(event) {
    for (const subscriber of subscribers) {
      try { subscriber(clone(event)); } catch (_) {}
    }
  }
  ...
  function listEvents({ workspaceId, limit, cursor, eventType, runId } = {}) {
  ...
  function listRuns({ workspaceId, limit, cursor, status } = {}) {
    const workspace = normalizeWorkspaceId(workspaceId);
    const pageSize = normalizeLimit(limit);
    const decoded = cursorDecode(cursor);
    const params = [workspace];
    ...
    const hasMore = rows.length > pageSize;
    const page = rows.slice(0, pageSize).map(projectRun);
  ...
  function evaluateAlerts(workspaceId, event) {
    ...
    for (const row of rows) {
      ...
      if (value === null || value === undefined || !compareMetric(value, rule.operator, rule.threshold)) continue;
      const previous = statements.getLatestAlert.get(workspaceId, rule.ruleId);
      const previousAt = previous ? Number(previous.fired_at) : 0;
      if (previous && previous.status === 'firing' && nowMs(now) - previousAt < rule.cooldownMs) continue;
  ...
  function recoverExpiredJobs() {
    const timestamp = nowMs(now);
    return statements.recoverLeases.run(timestamp, timestamp, timestamp).changes;
  }
  ...
  function retryJob({ jobId, workspaceId, workerId, delayMs = 1000, errorCode = 'WORKER_RETRY' } = {}) {
    ...
    if (!job || String(job.worker_id || '') !== worker || job.status !== 'running') return null;
  ...
  function queueSummary({ workspaceId } = {}) {
    ...
    return { workspaceId: workspace, byStatus, depth: Number((byStatus.queued || 0) + (byStatus.running || 0)) };
  }
