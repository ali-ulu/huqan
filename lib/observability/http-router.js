...
const { STREAM_HEADERS, openSseStream } = require('./sse-stream-lifecycle');
const {
  ...
  return async function handleObservabilityRoute(req, res, reqUrl) {
    ...
    if (suffix === '/stream') {
      ...
      try {
        ...
        openSseStream({ req, res, service, workspaceId: workspace.workspaceId, rate });
      } catch (error) {
        rate.release();
    ...
    if (!['/health', '/ready', '/metrics', '/events', '/runs', '/queue', '/alerts', '/alert-rules'].includes(suffix)
        && !/^\/alert-rules\/[^/]+$/.test(suffix)
        && !/^\/alerts\/[^/]+\/acknowledge$/.test(suffix)) return false;
    if (!denyIfUnauthorized(req, res, NO_STORE, { errorCode: 'UNAUTHORIZED' })) return true;
    if (!isValidObservabilityQuery(reqUrl)) {
      failure(writeJson, req, res, 400, 'OBSERVABILITY_QUERY_INVALID', 'Observability query is invalid.');
      return true;
    ...
    try {
      ...
      if (req.method === 'GET' && suffix === '/metrics') {
        send(writeJson, req, res, 200, { ok: true, data: { metrics: service.summary({ workspaceId, windowMs: reqUrl.searchParams.get('windowMs') }), queue: service.queueSummary({ workspaceId }), alerts: service.listAlerts({ workspaceId, limit: 20 }), internal: service.internalMetrics({ workspaceId }) } });
        return true;
      }
