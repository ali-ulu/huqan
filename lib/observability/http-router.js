...
function createObservabilityHttpRouter({ getService, parseJsonRequest, writeJson, denyIfUnauthorized, authorizeWorkspace, rateLimiter }) {
  ...
  return async function handleObservabilityRoute(req, res, reqUrl) {
    ...
    if (!['/metrics', '/internal-metrics', '/events', '/runs', '/queue', '/alerts', '/alert-rules'].includes(suffix)
