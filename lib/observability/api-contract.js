...
const schemas = {
  ...
  ResponseMetrics: objectSchema({ ok: { const: true }, data: objectSchema({ metrics: ref('Metrics'), queue: ref('QueueSummary'), alerts: { type: 'array', items: ref('Alert') } }) }),
...
const OBSERVABILITY_PATHS = {
  ...
  [`${OBSERVABILITY_API_PREFIX}/events`]: {
    get: operation({ operationId: 'observability-events', summary: 'Read redacted workspace events with cursor pagination.', method: 'GET', parameters: [workspaceParameter, limitParameter, cursorParameter, { name: 'eventType', in: 'query', schema: { type: 'string', enum: [...EVENT_TYPES] } }, { name: 'runId', in: 'query', schema: ID }], schema: ref('ResponseEventPage') }),
  },
  [`${OBSERVABILITY_API_PREFIX}/runs`]: {
    get: operation({ operationId: 'observability-runs', summary: 'Read redacted workspace runs with cursor pagination.', method: 'GET', parameters: [workspaceParameter, limitParameter, cursorParameter, { name: 'status', in: 'query', schema: { type: 'string', maxLength: 64 } }], schema: ref('ResponseRunPage') }),
  },
