...
function createAlertLifecycle({ statements, insertEvent, summary, notify, now = Date.now } = {}) {
  ...
  function deliverNotification(event, alert) {
    ...
    try {
      const pending = notify(input);
      if (pending && typeof pending.catch === 'function') pending.catch(error => {
        console.warn('[observability alert notification failed]', JSON.stringify({ notificationId: event.eventId, error: String(error?.message || error) }));
      });
    } catch (error) {
      console.warn('[observability alert notification failed]', JSON.stringify({ notificationId: event.eventId, error: String(error?.message || error) }));
    }
  ...
  function transitionAlert(row, status, reason) {
    ...
    const lifecycleEvent = insertEvent({
    ...
    deliverNotification(lifecycleEvent, alert);
    return alert;
  }
  ...
  function evaluateAlerts(workspaceId, event) {
    ...
    for (const row of rows) {
      ...
      deliverNotification(alertEvent, alert);
      fired.push(alert);
    }
    return fired;
  }
