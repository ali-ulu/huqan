...
test('agent worker terminally rejects a malformed claimed payload without creating an agent', async () => {
  const calls = [];
  const service = {
    claimNextJob: () => ({ jobId: 'job-bad', workspaceId: 'ws', goal: '', maxSteps: 2 }),
    finishJob: input => { calls.push(input); return { status: 'failed' }; },
    retryJob: () => { throw new Error('malformed payload must not be retried'); },
  };
  const worker = createAgentWorker({ service, createAgent: () => { throw new Error('agent must not be created'); } });
  const result = await worker.tick();
  assert.equal(result.status, 'failed');
  assert.equal(calls.length, 1);
  assert.equal(calls[0].errorCode, 'QUEUE_PAYLOAD_INVALID');
});
