...
test('observability dashboard exposes reconnect, stale and accessible live states', () => {
  ...
  assert.match(dashboard, /Last successful data/);
...
test('tool usage hides inconsistent totals instead of drawing a false ratio', () => {
  ...
  assert.match(dashboard, /Chart hidden/);
  assert.match(dashboard, /Calls were reported but the tool distribution is missing/);
});
