...
function validateReceipt(receipt, plan, reservation, claim, result) {
  ...
  if (!receipt || receipt.operationId !== claim.operationId || receipt.receiptId !== claim.receiptId || receipt.workspaceId !== plan.workspaceId || !printable(receipt.receiptHash,256)
    ...
    || payload.approvalId !== plan.approvalId || payload.reservationHash !== reservation.reservationHash || payload.candidatePlanHash !== plan.candidatePlanHash
    || payload.executionHash !== claim.executionHash || payload.resultHash !== sha256(result)) return failure('REVIEWED_GRAPH_RECEIPT_INVALID', 'durable graph receipt is invalid');
  return { ok:true };
}
...
function validateFinalization(value, plan, reservation, claim, receipt, result) {
  if (!exact(value, FINAL_KEYS) || value.version !== FINALIZATION_VERSION || value.operationId !== claim.operationId || value.receiptId !== claim.receiptId
    ...
    || value.resultHash !== sha256(result) || !canonicalTime(value.completedAt).ok || sha256(without(value,'finalizationHash')) !== value.finalizationHash) return failure('REVIEWED_GRAPH_FINALIZATION_INVALID', 'graph finalization is invalid');
  return { ok:true };
}
...
function executeReviewedExternalGraphMutation(plan, reservation, options = {}) {
  ...
  if (typeof graph?.runMutationOnce !== 'function' || typeof graph.getCommittedMutationReceiptByOperation !== 'function' || typeof graph.addNode !== 'function' || typeof graph.addEdge !== 'function') return failure('REVIEWED_GRAPH_BACKEND_REQUIRED', 'durable SQLite graph is required');
  ...
  if (record.status === 'approved') {
    ...
    checked = validateFinalization(record.context.reviewedExternalGraphFinalization,plan,reservation,claim,receipt,replay.result); if (!checked.ok) return checked;
  ...
  if (record.status === 'approved' && record.context?.reviewedExternalGraphFinalization) {
    checked = validateFinalization(record.context.reviewedExternalGraphFinalization,plan,reservation,claim,receipt,replay.result);
    return checked.ok ? {ok:true,replayed:true,result:replay.result,receipt,approval:record} : checked;
  }
  if (record.status !== 'executing' || record.decision !== 'approved' || Number(record.decided_at) !== 0 || Number(record.updated_at) !== claim.approvalUpdatedAt || !isDeepStrictEqual(record.context?.reviewedExternalGraphExecution,claim)) return failure('REVIEWED_GRAPH_FINALIZATION_STATE_CHANGED', 'approval changed before finalization');
