...
function recordBrowserHookOutcome(profile, payload, options = {}) {
  ...
  // #2139: surface what the monitor decided and whether a human still owes a
  // decision. The hook payload is attacker-influenced, so review-required is
  // only ever read from the monitor's summary and the guard's own quarantine
  // result -- never from anything the payload could have claimed.
...
/**
 * #2139: the browser review chain in one answer. Reads the outcome receipt
 * from the trail (only one whose own hash still verifies), the monitor's
 * verdict recorded on it, and the newest human review attached to it (#2137).
 * A trail with no verifying outcome for the id proves nothing, so the chain
 * answers `outcome: null` with no review-required claim -- absence is not a
 * verdict. The duplicate return of `recordBrowserHookOutcome` stays shape-
 * stable; callers that need the state of an already-recorded outcome use this.
 */
