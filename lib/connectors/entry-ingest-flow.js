...
/**
 * The roots file-backed ingest is allowed to read from.
 *
 * Same shape as `getCliReadRoots` in lib/cli-helpers.js: the working directory
 * and the temp directory are the baseline, and `HUQAN_INGEST_ALLOWED_ROOTS` (a
 * path-separator-delimited list) *extends* it. The deployment owns the
 * boundary either way -- the caller can narrow within it but never widen it.
 *
 * @param {object} [environment]
 * @returns {string[]} Absolute, resolved roots.
 */
function configuredIngestRoots(environment = process.env) {
  const extra = String(readCompatibleEnvironmentVariable('INGEST_ALLOWED_ROOTS', environment) || '')
    .split(path.delimiter)
    .map((entry) => entry.trim())
    .filter(Boolean)
    .map((entry) => path.resolve(entry));
  return [...new Set([path.resolve(process.cwd()), path.resolve(os.tmpdir()), ...extra])];
}
 ...
 *
 * The caller may narrow the scope, never widen it (#1293). `rootPath` used to
 * be taken from the caller verbatim and handed to listFilesWithinRoot as the
 * boundary to enforce -- so the party naming the target also named the fence it
 * would be checked against, and `rootPath: '/'` read anything on the machine.
 * The containment check in safe-file-walk was never wrong; the root it was
 * given was.
 *
 ...
 */
function requireRootedPath(input = {}, { label, rootCode, environment } = {}) {
  const targetPath = input.path || input.targetPath || '';
  if (!targetPath) {
    throw new Error(`${label} path is required`);
  }

  const requested = input.rootPath || input.workspaceRoot || input.allowedRoot || '';
  if (!requested) {
  ...
  const allowedRoots = configuredIngestRoots(environment);
  const resolved = path.resolve(String(requested));
  const containing = allowedRoots.find((root) => isWithin(root, resolved));
  if (!containing) {
    const err = new Error(
      `${label} rootPath is outside the configured ingest roots; `
      + 'set HUQAN_INGEST_ALLOWED_ROOTS to widen the boundary',
    );
    err.code = 'INGEST_ROOT_NOT_ALLOWED';
    throw err;
  }

  return { targetPath, rootPath: resolved };
}
