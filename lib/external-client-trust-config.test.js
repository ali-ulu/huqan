...
test('materializer source has no environment, filesystem, network, clock or runtime side effects', () => {
  // The materializer is split across external-client-trust-config*.js (#2196):
  // follow its own require chain so every part is held to the same rule.
  const files = ['external-client-trust-config.js'];
  for (let i = 0; i < files.length; i += 1) {
    const text = fs.readFileSync(path.join(__dirname, files[i]), 'utf8');
    for (const [, name] of text.matchAll(/require\(['"]\.\/(external-client-trust-config[\w-]*)['"]\)/g)) {
      if (!files.includes(`${name}.js`)) files.push(`${name}.js`);
    }
  }
  assert.ok(files.length > 1, 'the materializer parts are reachable from its entry file');
