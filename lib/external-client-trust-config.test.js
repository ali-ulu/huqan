...
test('materializer source has no environment, filesystem, network, clock or runtime side effects', () => {
  ...
  assert.doesNotMatch(source, /process\.env|require\(['"](?:node:)?fs['"]\)|require\(['"](?:node:)?https?['"]\)|Date\.now\s*\(|new\s+Date\s*\(\s*\)|globalThis|server\.js|snapshotExternalClientAuthority|replayStore|receipt|mutation/i);
