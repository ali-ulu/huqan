...
function runLearnUseCase(kernel, text, opts = {}, dependencies = {}) {
  return executeLearn.call(kernel, text, opts, dependencies);
}
...
function executeLearn(text, opts = {}, dependencies = {}) {
  ...
  if (this.strictProvenance && !hasRealProvenance(opts)) {
    this._appendAuditEvent({
      eventType: 'REJECT',
      targetType: 'learn',
      targetId: text,
  ...
  if (!parsed) return this.ok('learn', {
    learned: 0,
    skipped: 1,
    conflicts: [],
  ...
  for (const { subject, predicate } of parsed) {
    ...
    const negatedObject = isCopulaNegation ? `${object} [değil]` : object;
      ...
      if (relation === 'tür') {
        // Bir varlığın birden fazla uyumlu türü olabilir (ör. kedi -> hayvan
        // ve kedi -> memeli). Cosine similarity, uyumlu ama farklı etiketleri
        // yanlışlıkla alternatif/çatışma sayıyordu. Çatışma otoritesi yalnızca
        // workspace kapsamındaki type-lattice disjoint çiftleridir.
        for (const existing of existingTargets) {
          if (existing !== object && pairMatchesDisjoint(object, existing, workspaceId)) {
            conflicts.push({
              type: 'alternative',
              subject,
              relation: 'tür',
...
module.exports = { runLearnUseCase, hasRealProvenance };
