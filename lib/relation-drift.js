...
function detectRelationDrift(stored, incoming, opts = {}) {
  ...
  // A different predicate by itself is not evidence that either claim is
  // false. Require topical overlap as well; otherwise every new fact about a
  // known subject is misclassified as a contradiction (#1076).
  ...
  return {
    ...
    // Drift is worth surfacing for review, but it is not a refutation. Known
    // opposition, type, value and negation rules remain responsible for
    // producing contradicted verdicts.
    kind: 'risk',
