 ...
 * `runBeforeLearn` is a plugin *transform* event, not an enforcement point:
...
/**
 * The seam these paths fall back to when neither the caller nor the kernel
 * supplies one.
 *
 * Named once rather than reconstructed at each of the three call sites, so the
 * declaration below is the single place that changes when a kernel caller
 * starts carrying a receiver-owned claim — and so the three sites cannot drift
 * into declaring different things.
 */
function unenforcedFallbackSeam() {
  return createMutationAdmission({
    identityEvaluator: absent(
      'kernel callers carry no receiver-owned identity claim; the kernel seam '
      + 'admits on context shape alone until one of them does',
    ),
  });
}
 ...
 *   admission itself. Its contents are exactly what `runBeforeLearn` returned --
 ...
 */
function runBeforeLearn(kernel, text, opts = {}) {
  const payload = { text, opts: { ...opts } };
  if (kernel.plugins && typeof kernel.plugins.emitStrict === 'function') {
    return kernel.plugins.emitStrict('beforeLearn', payload);
  }
  if (kernel.plugins && typeof kernel.plugins.emit === 'function') {
    return kernel.plugins.emit('beforeLearn', payload);
  }
  return payload;
}

function admitLearn(kernel, text, opts = {}, admission = null) {
  const ev = runBeforeLearn(kernel, text, opts);
  const nextText = ev.text;
  const nextOpts = ev.opts || opts;

  const seam = admission || kernel._mutationAdmission || unenforcedFallbackSeam();
 ...
 */
function admitCandidateIngress(kernel, input = {}, opts = {}, admission = null, evaluateLearnAdmission = null) {
  const seam = admission || kernel._mutationAdmission || unenforcedFallbackSeam();
  ...
  }, () => routeCandidateClaim(kernel, input, opts, { evaluateLearnAdmission }));
