 ...
 *
 * ## The candidate family is still NOT fully routed
 *
 * Two of the family's three production entry points now pass this seam. The
 * third does not, and it does not go through the kernel at all:
 *
 *   `lib/external-client-mutation-receipt-owner.js` -> `graph.addCandidateClaim`
 *
 * It is open debt, tracked in
 * `docs/task-packs/p1c-admission-seam-change-surface.md`. Routing it is a
 * separate unit: it is an independent ingress rather than a kernel caller, so
 * there is nothing here for it to reuse. Until it is closed, "the candidate
 * family is routed" is not a claim this module supports.
 *
