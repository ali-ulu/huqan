 ...
 *
 * Two bounds on that, both load-bearing rather than decorative:
 *
 * 1. It is contingent on `lib/github-connector.js` staying `NOT_YET_WIRED`. The
 *    connector writes candidate claims directly *and* is `routeCandidateClaim`'s
 *    second, unadmitted caller. Wiring it reopens both this claim and the
 *    transitive one for `lib/conflict-detector.js`.
 * 2. Routed is not enforced. The seam still evaluates no identity checks; it is
