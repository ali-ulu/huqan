...
function verifyContentBindings(files) {
  for (const file of files) {
    const actualSizeBytes = Buffer.byteLength(file.content, 'utf8');
    if (file.sizeBytes !== actualSizeBytes || file.contentHash !== sha256Text(file.content)) {
...
function materializeReviewedExternalIngestBatch(plan, options = {}) {
  ...
  const contentBindings = verifyContentBindings(plan.files);
