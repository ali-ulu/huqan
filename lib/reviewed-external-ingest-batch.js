...
function executionPlanCore(plan) {
  return {
...
function verifyPlanIdentity(plan) {
  ...
  for (const [key, maxLength] of boundedFields) {
    ...
    if (!checked.ok || checked.value !== plan[key]) return failure('REVIEWED_BATCH_PLAN_IDENTITY_INVALID', `${key} is invalid`);
  }
...
function verifyPlanTiming(plan, options) {
  ...
  const now = canonicalTime(options.now instanceof Date ? options.now : (options.now || new Date().toISOString()), 'now', 'REVIEWED_BATCH_NOW_INVALID');
...
function verifyAndCopyFiles(plan) {
  ...
  for (const input of plan.files) {
    ...
    if (input.sizeBytes !== sizeBytes || input.contentHash !== sha256Text(input.content)) {
      return failure('REVIEWED_BATCH_CONTENT_MISMATCH', `reviewed content binding mismatch: ${filePath}`);
    }
...
function materializeReviewedExternalIngestBatch(plan, options = {}) {
  ...
  const expectedPlanHash = sha256(executionPlanCore(plan));
