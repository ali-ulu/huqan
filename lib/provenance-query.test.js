...
describe('Provenance Query', () => {
  ...
  it('an accepted candidate shadowing a canonical edge reads as canonical, not flagged forever (#2796)', () => {
    const graph = new Graph({
      memoryPath: path.join(tempDir, 'accepted-shadow.json'),
      useSQLite: false,
    });

    const provenance = makeProvenance({ provenanceId: 'prov-accepted-shadow', sourceRef: 'docs/accepted.md#claim' });

    graph.addNode('kedi', 'kedi hayvandir', provenance, { workspaceId: 'workspace-a' });
    // Shape lib/hypothesis-review.js and lib/conflict-candidate-review.js
    // both write on accept: status moves to 'accepted', recommendation stays
    // 'flag' as audit history, never cleared.
    graph.addCandidateClaim({
      candidateId: 'cand-accepted-shadow',
      workspaceId: 'workspace-a',
      claim: 'kedi hayvandir',
      proposedEdge: { from: 'kedi', relation: 'tür', to: 'hayvan', confidence: 0.4, workspaceId: 'workspace-a' },
      provenance,
      ...
      status: 'accepted',
      createdAt: '2026-06-02T00:00:00Z',
      conflict: { reason: 'shadowed canonical record', conflict: true },
    });
  ...
  it('a rejected candidate shadowing a canonical edge still reads as rejected (#2796 regression guard)', () => {
    const graph = new Graph({
      memoryPath: path.join(tempDir, 'rejected-shadow.json'),
      useSQLite: false,
    });

    const provenance = makeProvenance({ provenanceId: 'prov-rejected-shadow', sourceRef: 'docs/rejected.md#claim' });

    graph.addNode('kedi', 'kedi hayvandir', provenance, { workspaceId: 'workspace-a' });
    graph.addCandidateClaim({
      candidateId: 'cand-rejected-shadow',
      workspaceId: 'workspace-a',
      claim: 'kedi hayvandir',
      proposedEdge: { from: 'kedi', relation: 'tür', to: 'hayvan', confidence: 0.4, workspaceId: 'workspace-a' },
      provenance,
    ...
    assert.strictEqual(receipt.targetType, 'candidate_claim');
    assert.strictEqual(receipt.candidateClaim.candidateId, 'cand-rejected-shadow');
  });

  it('a still-pending flagged candidate keeps reading as flagged (#2796 regression guard)', () => {
    const graph = new Graph({
      memoryPath: path.join(tempDir, 'pending-flagged.json'),
      useSQLite: false,
    });

    const provenance = makeProvenance({ provenanceId: 'prov-pending-flagged', sourceRef: 'docs/pending.md#claim' });

    graph.addCandidateClaim({
      candidateId: 'cand-pending-flagged',
      workspaceId: 'workspace-a',
      claim: 'kedi hayvandir',
      proposedEdge: { from: 'kedi', relation: 'tür', to: 'hayvan', confidence: 0.4, workspaceId: 'workspace-a' },
      provenance,
      ...
      status: 'flagged',
      createdAt: '2026-06-02T00:00:00Z',
      conflict: { reason: 'still pending triage', conflict: true },
    });

    const receipt = buildTrustReceipt({ candidateId: 'cand-pending-flagged', workspaceId: 'workspace-a' }, { target: graph });

    assert.strictEqual(receipt.status, 'flagged');
  });

  it('an accepted candidate with no canonical edge to shadow reads as unknown, not stuck pending (#2796)', () => {
    const graph = new Graph({
      memoryPath: path.join(tempDir, 'accepted-no-canonical.json'),
      useSQLite: false,
    });

    const provenance = makeProvenance({ provenanceId: 'prov-accepted-hypothesis', sourceRef: 'docs/hypothesis.md#claim' });

    // Shape of an accepted hypothesis with no pre-existing edge to shadow --
    // lib/hypothesis-review.js's accept path never performs a canonical
    // write (#2794's design principle), so no addNode call here.
    graph.addCandidateClaim({
      candidateId: 'cand-accepted-hypothesis',
      workspaceId: 'workspace-a',
      claim: 'kedi hayvandir',
      proposedEdge: { from: 'kedi', relation: 'tür', to: 'hayvan', confidence: 0.4, workspaceId: 'workspace-a' },
      provenance,
    ...
    assert.strictEqual(receipt.status, 'unknown');
  });
  ...
  it('preserves cross-workspace source order without mutating target access time', () => {
    const graph = new Graph({
      memoryPath: path.join(tempDir, 'cross-workspace-baseline.json'),
      useSQLite: false,
    });
    const provenanceA = makeProvenance({ provenanceId: 'prov-cross-a', workspaceId: 'workspace-a' });
    const provenanceB = makeProvenance({ provenanceId: 'prov-cross-b', workspaceId: 'workspace-b' });

    graph.addNode('a', 'A', provenanceA, { workspaceId: 'workspace-a' });
    graph.addNode('a-target', 'A target', provenanceA, { workspaceId: 'workspace-a' });
    graph.addEdge('a', 'a-target', 'supports', { workspaceId: 'workspace-a', provenance: provenanceA });
    graph.addNode('b', 'B', provenanceB, { workspaceId: 'workspace-b' });
    graph.addNode('b-target', 'B target', provenanceB, { workspaceId: 'workspace-b' });
    graph.addEdge('b', 'b-target', 'supports', { workspaceId: 'workspace-b', provenance: provenanceB });
    graph.addCandidateClaim({
      candidateId: 'candidate-a',
      workspaceId: 'workspace-a',
      claim: 'A candidate',
      proposedEdge: { from: 'a', relation: 'supports', to: 'a-target', workspaceId: 'workspace-a' },
      provenance: provenanceA,
      status: 'pending',
      recommendation: 'accept',
      createdAt: '2026-06-02T00:00:00Z',
    ...
    graph.addCandidateClaim({
      ...
      workspaceId: 'workspace-b',
      claim: 'B candidate',
      proposedEdge: { from: 'b', relation: 'supports', to: 'b-target', workspaceId: 'workspace-b' },
      provenance: provenanceB,
      status: 'flagged',
      recommendation: 'flag',
      createdAt: '2026-06-02T00:01:00Z',
    ...
    graph._nodes['workspace-a::a'].created_at = '2026-06-02T01:00:00Z';
    graph._nodes['workspace-a::a-target'].created_at = '2026-06-02T01:01:00Z';
    graph._edges[0].created_at = '2026-06-02T01:02:00Z';
    graph._nodes['workspace-b::b'].created_at = '2026-06-02T02:00:00Z';
    graph._nodes['workspace-b::b-target'].created_at = '2026-06-02T02:01:00Z';
    graph._edges[1].created_at = '2026-06-02T02:02:00Z';

    graph._nodes['workspace-a::a'].lastAccessed = 101;
    const provenanceRecords = queryProvenance(graph, { crossWorkspace: true });
    assert.deepStrictEqual(
      provenanceRecords.map(item => [item.kind, item.targetId, item.workspaceId]),
      [
        ['candidate_claim', 'candidate-a', 'workspace-a'],
        ['candidate_claim', 'candidate-b', 'workspace-b'],
        ['node', 'a', 'workspace-a'],
        ['node', 'a-target', 'workspace-a'],
        ['edge', 'a|supports|a-target', 'workspace-a'],
        ['node', 'b', 'workspace-b'],
        ['node', 'b-target', 'workspace-b'],
        ['edge', 'b|supports|b-target', 'workspace-b'],
      ],
    );
    assert.strictEqual(graph._nodes['workspace-a::a'].lastAccessed, 101);

    const expectedCandidateOrder = graph.getCandidateClaims().map((item) => item.candidateId);
    const candidateRecords = queryCandidateClaims(graph, { crossWorkspace: true });
    assert.deepStrictEqual(candidateRecords.map((item) => item.candidateId), expectedCandidateOrder);
    ...
    try {
      graph._nodes['workspace-a::a-target'].lastAccessed = 101;
      const endpointResult = queryTrustGraph(graph, {
        targetId: 'a-target',
        workspaceId: 'workspace-a',
      });
      assert.strictEqual(endpointResult.canonical.targetType, 'node');
      assert.deepStrictEqual(getNodeCalls, [['a-target', 'workspace-a']]);
      assert.strictEqual(graph._nodes['workspace-a::a-target'].lastAccessed, 101);

      getNodeCalls.length = 0;
      const accessBeforeComposite = Object.fromEntries(
        Object.entries(graph._nodes).map(([key, node]) => [key, node.lastAccessed]),
      );
      const compositeResult = queryTrustGraph(graph, {
        targetId: 'a|supports|a-target',
        workspaceId: 'workspace-a',
      });
      assert.strictEqual(compositeResult.canonical.targetType, 'edge');
      assert.strictEqual(compositeResult.canonical.targetId, 'a|supports|a-target');
      assert.deepStrictEqual(getNodeCalls, [['a|supports|a-target', 'workspace-a']]);
      assert.deepStrictEqual(
        Object.fromEntries(Object.entries(graph._nodes).map(([key, node]) => [key, node.lastAccessed])),
        accessBeforeComposite,
      );
    } finally {
