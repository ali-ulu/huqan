...
function receiptFrom(details) {
  return isPlainObject(details.receipt) ? details.receipt : null;
}

function mapAuditEvent(event, workspaceId) {
  const details = isPlainObject(event.details) ? event.details : {};
  const receipt = receiptFrom(details);
...
function validateEvents(events, maxAuditEvents) {
  ...
  for (const event of events) {
    if (!isPlainObject(event) || !trimText(event.auditId) || !trimText(event.workspaceId)) {
