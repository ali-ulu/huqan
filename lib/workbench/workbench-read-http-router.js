...
const { createWorkbenchActivityRoute } = require('./activity-route');
const { createReceiptValidationCache } = require('../receipt/receipt-validation-cache');
...
function createWorkbenchReadHttpRouter(options = {}) {
  ...
  const receiptValidationCache = options.receiptValidationCache || createReceiptValidationCache();
  const handleWorkbenchActivity = createWorkbenchActivityRoute({ writeJson, denyIfUnauthorized });

  return function handleWorkbenchRead(req, res, reqUrl, graph) {
    if (handleWorkbenchActivity(req, res, reqUrl, graph)) return true;
    ...
    if (bundleRequest) {
      ...
      if (!workspaceParam.ok) {
        invalidMemoryRequest(writeJson, req, res, workspaceParam.code);
        return true;
      }
