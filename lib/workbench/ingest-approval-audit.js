...
function recordAuditEvidence(recordAudit, approval, receipt, result) {
  try {
    ...
    const auditRef = recorded && typeof recorded === 'object' && typeof recorded.auditId === 'string'
      ...
      : '';
    const trustReceiptId = recorded && typeof recorded === 'object' && typeof recorded.trustReceiptId === 'string'
      ? recorded.trustReceiptId.trim()
      : '';
    return auditRef
      ? { ok: true, auditRef, ...(trustReceiptId ? { trustReceiptId } : {}) }
      : { ok: false, reason: 'audit_reference_missing' };
  } catch (error) {
...
function auditOrGap(recordAudit, { approval, receipt, result, committed, message }) {
  ...
  if (audit.ok) return {
    ...
    ...(audit.trustReceiptId ? { trustReceiptId: audit.trustReceiptId } : {}),
    gap: null,
  };
  return {
    auditRef: '',
