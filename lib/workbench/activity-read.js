...
const BROWSER_SESSION_RECEIPT_KIND = 'browser_session_event_receipt';
...
function browserSessionSummary(metadata) {
  return {
    sessionId: trimText(metadata.sessionId),
    outcomeReceiptId: trimText(metadata.outcomeReceiptId),
    outcomeBinding: trimText(metadata.outcomeBinding),
    event: trimText(metadata.event),
    phase: trimText(metadata.phase),
    destination: destinationSummary(metadata),
    targetIdHash: trimText(metadata.targetIdHash),
    targetType: trimText(metadata.targetType),
    requestIdHash: trimText(metadata.requestIdHash),
    method: trimText(metadata.method),
    resourceType: trimText(metadata.resourceType),
    statusCode: Number.isFinite(metadata.statusCode) ? metadata.statusCode : null,
    consoleType: trimText(metadata.consoleType),
    argumentCount: Number.isFinite(metadata.argumentCount) ? metadata.argumentCount : null,
...
function receiptSummary(event) {
  ...
  const receiptKind = trimText(receipt.receiptKind);
  ...
  const isOutcome = receiptKind === OUTCOME_RECEIPT_KIND;
  return {
    ...
    ...(receiptKind.startsWith('external_action_') ? {
      receiptKind,
    ...
    ...(receiptKind === BROWSER_SESSION_RECEIPT_KIND ? {
      receiptKind,
      browserSession: browserSessionSummary(metadata),
    } : {}),
