...
function rejectIngestApproval({ store, approval, snapshot, recordAudit, toPublicApproval }) {
  ...
  return {
    status: 200,
    json: { ok: true, approval: toPublicApproval(rejected.approval), receipt, auditRef: audited.auditRef },
  };
}
...
async function executeApprovedIngest(deps) {
  ...
  return {
    status: 200,
    json: {
      ok: true, approval: toPublicApproval(finalized.approval), result, receipt, auditRef: audited.auditRef,
    },
  };
}
