...
function rejectIngestApproval({ store, approval, snapshot, recordAudit, toPublicApproval, oversightDecision }) {
...
async function executeApprovedIngest(deps) {
  ...
  const { oversightCase, decisionReason = '', oversightDecision: precomputedOversightDecision = null } = deps;
  ...
  if (oversightCase?.enabled && !oversightDecision.ok) {
    store.failToolApproval(approval.id, 'oversight_decision_failed');
  ...
  try {
    ...
    if (!execution.ok) {
      store.failToolApproval(approval.id, 'oversight_execution_blocked');
  ...
  if (audited.gap) return audited.gap;
  return { status: 200, json: { ok: true, approval: toPublicApproval(finalized.approval), result, receipt, auditRef: audited.auditRef, ...(oversightCase?.enabled ? { oversight: oversightSummary(oversightCase.result, oversightDecision.result, oversightExecution) } : {}) } };
}
...
async function decideIngestApproval(deps) {
  const { store, approvalId, decision, reason = '', toPublicApproval, humanOversight = null } = deps;
  ...
  if (decision !== 'rejected') {
    return executeApprovedIngest({ ...deps, approval, oversightCase, decisionReason: reason, oversightDecision });
  }
  ...
  return rejectIngestApproval({ ...deps, approval, snapshot, oversightDecision });
}
module.exports = {
  ACTION_OWNER,
