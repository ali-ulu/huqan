...
function rejectIngestApproval({ store, approval, snapshot, workspaceId = 'default', recordAudit, toPublicApproval, oversightDecision }) {
  ...
  return { status: 200, json: { ok: true, approval: toPublicApproval(rejected.approval), receipt, auditRef: audited.auditRef, ...(audited.trustReceiptId ? { trustReceiptId: audited.trustReceiptId } : {}), ...(oversightDecision?.enabled ? { oversight: oversightSummary(oversightDecision.case?.result, oversightDecision.result) } : {}) } };
}
async function executeApprovedIngest(deps) {
  ...
  }, Math.max(100, Math.min(5_000, Math.floor((Number(leaseMs) || 60_000) / 2))));
  heartbeat.unref?.();
  let result = null;
  let threw = false;
  ...
  if (audited.gap) return audited.gap;
  return { status: 200, json: { ok: true, approval: toPublicApproval(finalized.approval), result, receipt, auditRef: audited.auditRef, ...(audited.trustReceiptId ? { trustReceiptId: audited.trustReceiptId } : {}), ...(oversightCase?.enabled ? { oversight: oversightSummary(oversightCase.result, oversightDecision.result, oversightExecution) } : {}), ...(identityEvaluation?.enabled ? { identity: identityEvaluation.evidence } : {}) } };
}
