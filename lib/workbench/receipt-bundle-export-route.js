...
/**
 * Resolve the `workspaceId` query parameter into the exporter's vocabulary.
 *
 * Absent means `undefined` — the canonical default. Present-but-blank stays a
 * blank string so the exporter can reject it. Repeated parameters are rejected
 * here because a caller supplying two workspaces has not supplied the
 * canonical one.
 */
function readWorkspaceParam(searchParams) {
  if (!searchParams || typeof searchParams.getAll !== 'function') return { ok: true, workspaceId: undefined };
  const values = searchParams.getAll('workspaceId');
  if (values.length === 0) return { ok: true, workspaceId: undefined };
