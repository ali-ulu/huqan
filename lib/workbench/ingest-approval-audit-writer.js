...
function createIngestApprovalAuditWriter({ graph, admission, hashResult, ledger = null, identityContext = null }) {
  ...
  return function recordIngestApprovalAudit(approval, receipt, result = null) {
    ...
    const writeEvidence = () => {
      if (!ledger) return auditEvent();
      const appended = ledger.append({ operationId: evidenceOperationId, event: evidenceEvent, mutate: auditEvent });
      const trustReceiptId = String(appended?.receipt?.receiptId || '').trim();
      if (!trustReceiptId) return appended.result;
      return {
        ...appended.result,
        trustReceiptId,
      };
