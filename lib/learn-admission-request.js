...
function buildLearnAdmissionRequest({ text, opts = {}, provenance = null, workspaceId = 'default', contractVersion }) {
  ...
  const request = {
    ...
    // F1a: the caller's confidence claim, threaded for future calibration.
    // Read by nothing in the gate: admission risk above uses
    // provenance.confidence (the policy/system value) only, so recording a
    // declaration can never change allow/review/reject/quarantine.
    declaredConfidence:
      typeof provenance?.declaredConfidence === 'number' && Number.isFinite(provenance.declaredConfidence)
        ? provenance.declaredConfidence
        : null,
    createdAt,
    proposedMemory: {
      content: String(text || ''),
