...
function evaluateExternalAction(input, options = {}) {
  ...
  // this block is a no-op and the decision below is exactly today's. Runs
  // after every other gate and only ever tightens via the guard's own merge.
  const budget = evaluateImpactBudget({ envelope, decision, riskLevel, options });
  if (budget) {
    findings.push(budget.finding);
    decision = mergeDecision(decision, budget.finding.decision);
    if (decision !== EXTERNAL_ACTION_DECISIONS.ALLOW) reason = budget.finding.reason;
  }
