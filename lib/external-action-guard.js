...
function evaluateExternalAction(input, options = {}) {
  ...
  try {
    const autonomyOptions = graduatedAutonomyOptions(options);
    if (autonomyOptions) {
...
function recordExternalActionOutcome(input, admissionReceipt, outcome, options = {}) {
  ...
  const persisted = persistExternalActionReceipt(options.receiptWriter, receipt);
  return Object.freeze({ ok: true, receipt, receiptPersisted: persisted });
}
