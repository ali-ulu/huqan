 ...
 *
 * NOTE for anyone auditing this file: these are real filesystem writes, and
 * scripts/enforcement-coverage.js does NOT see them. That scanner binds a
 * capability to the imported module identifier, so `nodeFs.writeFileSync(...)`
 * is detected while `fs.writeFileSync(...)` through the injected handle is not
 * -- verified by changing this one call and re-running the scan. The injection
 * is here because the rollback test has to force a write failure, but it means
 * this module's write capability is absent from coverage-manifest.json. Read
 * the writes here, not the manifest.
 */
