...
const MCP_TOOL_CLASSIFICATIONS = Object.freeze({
  'axiom.ask': Object.freeze({ mutating: false, category: 'read', alphaDecision: 'allow', gates: ['AB1', 'AB11'] }),
  'axiom.verify': Object.freeze({ mutating: false, category: 'read', alphaDecision: 'allow', gates: ['AB1', 'AB11'] }),
  'axiom.plan': Object.freeze({ mutating: false, category: 'read', alphaDecision: 'allow', gates: ['AB1', 'AB11'] }),
  'axiom.policy': Object.freeze({ mutating: false, category: 'read', alphaDecision: 'allow', gates: ['AB1', 'AB11'] }),
  'axiom.approvals': Object.freeze({ mutating: false, category: 'read', alphaDecision: 'allow', gates: ['AB1', 'AB11'] }),
  'axiom.reason': Object.freeze({ mutating: false, category: 'read', alphaDecision: 'allow', gates: ['AB1', 'AB11'] }),
  'axiom.compare': Object.freeze({ mutating: false, category: 'read', alphaDecision: 'allow', gates: ['AB1', 'AB11'] }),
  'axiom.dream': Object.freeze({ mutating: false, category: 'read', alphaDecision: 'allow', gates: ['AB1', 'AB11'] }),
  'axiom.learn': Object.freeze({ mutating: true, category: 'write', alphaDecision: 'review', gates: ['AB1', 'AB2', 'AB4', 'AB11'] }),
  'axiom.agent': Object.freeze({ mutating: false, category: 'agent-loop', alphaDecision: 'dry_run_only', gates: ['AB1', 'AB2', 'AB8', 'AB9', 'AB11'] }),
});
...
/**
 * AB8 only has one real command-bearing surface today: axiom.agent's free-text
 ...
 * since that is the field axiom.agent actually declares.
 */
...
function buildAb11Input(args, metadata) {
  ...
  return {
    actorWorkspaceId,
    targetWorkspaceId,
    operation: isPlainObject(args) && typeof args.operation === 'string' ? args.operation : '',
...
function evaluateMcpGate(input, options = {}) {
  ...
  if (classification.gates.includes('AB8')) {
    try {
      ...
      warnings.push(`AB8 error: ${err.message}`);
    }
  }
  ...
  if (classification.gates.includes('AB9')) {
    try {
      ...
      warnings.push(`AB9 error: ${err.message}`);
    }
  }

  if (classification.gates.includes('AB11')) {
    try {
      const ab11Input = buildAb11Input(args, metadata);
      if (ab11Input) {
      ...
      warnings.push(`AB11 error: ${err.message}`);
    }
  }
