...
const { memoryWriteJustification } = require('./memory-blast-radius');
...
function evaluateLearnAdmission(deps, text, opts = {}, provenance = null, workspaceId = 'default') {
  ...
  const evaluated = evaluate(request, {
    ...
    // Why the write was admitted or not: its blast radius on the graph, measured
    // before the write, and the thresholds that applied (#2505). The gate reads
    // none of it; it is recorded on the receipt.
    metadata: { justification: memoryWriteJustification(kernel, text, workspaceId, request) },
  });
...
module.exports = { admissionReceiptDetails, evaluateLearnAdmission };
