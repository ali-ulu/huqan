...
async function runHuqanSdkCommand(kernel, payload, options = {}) {
  ...
  if (!capabilityName) {
    throw new Error(`Unknown HUQAN SDK command: ${resolved.command || normalizedCommand}`);
  }
...
function toLangChainTool(kernel, options = {}) {
  return {
    name: options.name || 'huqan',
    description: options.description || 'Use HUQAN to verify claims, find contradictions, and run reasoning capabilities.',
    async call(input) {
      return runHuqanSdkCommand(kernel, input, options);
    },
  };
}

function toVercelAiMiddleware(kernel, options = {}) {
  return async function huqanMiddleware(payload) {
...
function createHuqanClient(kernel, options = {}) {
...
// RFC-001 decision 7: a reader accepts both spellings. lib/sdk.js ships in
// package.json's `files`, so an external consumer may already import the
// AXIOM-era names. They keep resolving to the same functions and are no longer
// used anywhere in this repository.
module.exports.createAxiomClient = createHuqanClient;
module.exports.runAxiomSdkCommand = runHuqanSdkCommand;
