...
function snapshotPublicKey(value, keyId) {
  try {
    if (value instanceof crypto.KeyObject) {
      if (value.type !== 'public') throw new TypeError('public key is required');
      return value;
    }
    let privateKeyMaterial = false;
    try {
      crypto.createPrivateKey(value);
      privateKeyMaterial = true;
    } catch (_) {}
    if (privateKeyMaterial) throw new TypeError('private key material is forbidden');
    return crypto.createPublicKey(value);
  } catch (_) {
    failSdk(
      EXTERNAL_CLIENT_PACKAGE_GATE_ERRORS.TRUSTED_KEY_REQUIRED,
      'trusted key entry must include valid publicKey material',
      { keyId },
    );
  }
}

function snapshotTrustedKeys(value) {
  const trustedKeys = Object.create(null);
  if (!value || typeof value !== 'object' || Array.isArray(value)) {
    return Object.freeze(trustedKeys);
  }

  for (const [rawKeyId, rawEntry] of Object.entries(value)) {
    const keyId = cleanString(rawKeyId);
    if (!keyId || !rawEntry || typeof rawEntry !== 'object' || Array.isArray(rawEntry)) continue;
    if (Object.prototype.hasOwnProperty.call(trustedKeys, keyId)) {
      failSdk(
        EXTERNAL_CLIENT_PACKAGE_GATE_ERRORS.TRUSTED_KEY_REQUIRED,
        'trusted key IDs must be unique after normalization',
        { keyId },
      );
    }
    trustedKeys[keyId] = Object.freeze({
      publicKey: snapshotPublicKey(rawEntry.publicKey, keyId),
      workspaceId: cleanString(rawEntry.workspaceId),
      packageIds: snapshotStringList(rawEntry.packageIds),
      identitySubjects: snapshotStringList(rawEntry.identitySubjects),
      identityKinds: snapshotStringList(rawEntry.identityKinds),
    });
  }
...
function snapshotPackageAdmissionAuthority(kernel, options = {}) {
  const explicitHandler = typeof options.packageAdmissionHandler === 'function'
    ? options.packageAdmissionHandler
    : null;
  ...
  return Object.freeze({
    expectedWorkspaceId: cleanString(options.expectedWorkspaceId),
    expectedPackageId: cleanString(options.expectedPackageId),
    trustedKeys: snapshotTrustedKeys(options.trustedKeys),
