...
// Keep the candidate bounded for predictable scanning cost, then validate each
// possible identifier window with its checksum. Dots, slashes, and underscores
// are common formatting separators and must not create an egress bypass.
const DIGIT_RUN_PATTERN = /\d[\d\s._/-]{9,255}\d/g;
...
function piiTypeInDigitRun(match) {
  const digits = onlyDigits(match);
  for (let length = 19; length >= 13; length -= 1) {
    for (let start = 0; start + length <= digits.length; start += 1) {
      if (isValidLuhn(digits.slice(start, start + length))) return PII_TYPES.CREDIT_CARD;
    }
  }
  for (let start = 0; start + 11 <= digits.length; start += 1) {
    if (isValidTckn(digits.slice(start, start + 11))) return PII_TYPES.TCKN;
  }
  return null;
}
...
function findPiiInText(text) {
  ...
  const findings = [];
  // An IBAN-shaped token is handled by its own checksum above; do not classify
  // a numeric substring inside it as a card, whether its IBAN checksum passes
  // or fails.
  const digitScanText = str.replace(IBAN_PATTERN, (match) => match.replace(/\d/g, 'x'));
  ...
  for (const match of digitScanText.match(DIGIT_RUN_PATTERN) || []) {
    const type = piiTypeInDigitRun(match);
    if (type) findings.push({ type, match });
  }
...
function redactPiiInText(text) {
  ...
  out = out.replace(IBAN_PATTERN, (match) => {
    if (isValidIban(match)) return '[REDACTED_PII:iban]';
    const marker = `\u0000iban_${'x'.repeat(preservedInvalidIbans.length + 1)}\u0000`;
    preservedInvalidIbans.push([marker, match]);
    return marker;
  });
  out = out.replace(DIGIT_RUN_PATTERN, (match) => {
    const type = piiTypeInDigitRun(match);
    if (type) return `[REDACTED_PII:${type}]`;
    return match;
  });
  for (const [marker, match] of preservedInvalidIbans) out = out.replace(marker, match);
  return out;
}
