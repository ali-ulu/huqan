...
/**
 * Whether `term` appears in `haystack` as a whole word/phrase, not as a bare
 * substring of a longer word (#1284). A plain `haystack.includes(term)`
 * matched 'all' inside 'install', 'small', 'fall'; 'para' inside 'paragraf',
 * 'paralel'; 'tum' (normalizeText strips the Turkish diacritic off 'tüm')
 * inside 'tumor', 'tumevarim'.
 *
 * Checked against the characters immediately outside the match, not a `\b`
 * regex: some terms end in a non-word character ('100%'), where a trailing
 * `\b` would fail to anchor against a following space/end-of-string (both
 * sides of that boundary would be non-word, so `\b` never matches there).
 * `normalizeText` already lowercases and strips diacritics on both sides
 * before this runs, so plain ASCII a-z/0-9 is the whole alphabet that
 * matters here.
 */
...
function detectHighRiskDomain(text, opts = {}) {
  ...
  for (const [domain, tokens] of Object.entries(HIGH_RISK_DOMAINS)) {
    if (tokens.some(token => containsWholeTerm(norm, asText(token)))) {
      return signal(
...
function detectAbsoluteClaim(text, opts = {}) {
  const norm = asText(text);
  const found = ABSOLUTE_TERMS.find(term => containsWholeTerm(norm, asText(term)));
