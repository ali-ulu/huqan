...
function evaluateAgentActionFirewall(request = {}) {
  ...
  // Preserve the recursive detector's verdict across the narrow AB5 projection
  // (#2024). A boolean secret marker invokes AB5's existing mandatory block
  // without forwarding the original payload or recording its secret value.
  if (secretDetected) operationObject.secret = true;
