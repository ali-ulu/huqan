...
const RECEIPT_SCHEMA_VERSION = 'v5-github-app-beta-receipt-v1';
...
function receiptHash(receiptWithoutHash) {
  return crypto.createHash('sha256')
    .update(JSON.stringify(receiptWithoutHash), 'utf8')
    .digest('hex');
}
...
function buildReceipt(binding, action) {
  const projection = {
    ...
    repository: {
      ...
      fullName: binding.repositoryFullName,
    },
  ...
  return Object.freeze({ ...projection, receiptHash: receiptHash(projection) });
}
