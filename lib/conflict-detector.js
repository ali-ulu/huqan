...
const { buildBackgroundProvenance } = require('./background-provenance'); const { admissionReceiptDetails } = require('./admission-receipt-details');
...
function getJournalingKernel(kernelOrGraph) {
  for (const candidate of [kernelOrGraph, kernelOrGraph && kernelOrGraph.kernel]) {
    ...
    const hasAdmissionContext = candidate && typeof candidate.evaluateLearnAdmission === 'function';
...
function acceptCandidateClaimJournaled(kernel, candidate, conflict, built) {
  ...
  const admission = kernel.evaluateLearnAdmission(proposalText, admissionOpts, provenance, workspaceId);
