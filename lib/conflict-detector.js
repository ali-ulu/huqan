...
const { buildBackgroundProvenance } = require('./background-provenance'); const { admissionReceiptDetails, evaluateLearnAdmission } = require('./kernel-learn-admission');
...
// #217: journal only with a Kernel-shaped public admission context; bare Graph callers keep the legacy direct-write fallback.
function getJournalingKernel(kernelOrGraph) {
  for (const candidate of [kernelOrGraph, kernelOrGraph && kernelOrGraph.kernel]) {
    ...
    const hasAdmissionContext = candidate && typeof candidate.extractFacts === 'function' && typeof candidate.parsePredicate === 'function';
...
function acceptCandidateClaimJournaled(kernel, candidate, conflict, built) {
  ...
  const admission = evaluateLearnAdmission({ kernel, isLearnAdmissionBypass: () => false, contractVersion: kernel.contractVersion }, proposalText, admissionOpts, provenance, workspaceId);
