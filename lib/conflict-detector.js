...
const { admissionReceiptDetails, evaluateLearnAdmission } = require('./kernel-learn-admission');
...
// #217: candidate-claim acceptance can only be admission-gated and
// durably journaled when a real Kernel-shaped context (not a bare Graph) is
// available. Detect the public capabilities the admission calculation needs
// instead of reaching through a private Kernel method.
function getJournalingKernel(kernelOrGraph) {
  for (const candidate of [kernelOrGraph, kernelOrGraph && kernelOrGraph.kernel]) {
    const hasJournal = candidate && candidate.graph && typeof candidate.graph.runMutationOnce === 'function';
    const hasAdmissionContext = candidate
      && typeof candidate.extractFacts === 'function'
      && typeof candidate.parsePredicate === 'function';
    if (hasJournal && hasAdmissionContext) return candidate;
  }
  return null;
}
...
function appendAudit(kernelOrGraph, event, provenance, workspaceId) {
  ...
  const auditOpts = provenance ? { provenance, workspaceId } : { workspaceId };
  const write = () => graph.appendAuditEvent(payload, auditOpts);
  if (kernelOrGraph && kernelOrGraph.graph === graph) {
    try {
      return write();
    } catch (error) {
 ...
 */
function acceptCandidateClaimJournaled(kernel, candidate, conflict, built) {
  const workspaceId = candidate.workspaceId;
  const provenance = candidate.provenance && typeof candidate.provenance === 'object'
    ? candidate.provenance
    : buildBackgroundProvenance('candidate_claim', workspaceId, {}, {
        contractVersion: kernel.contractVersion,
        trustPolicyPath: kernel.trustPolicyPath,
      });
  ...
  const admission = evaluateLearnAdmission({
    kernel,
    isLearnAdmissionBypass: () => false,
    contractVersion: kernel.contractVersion,
  }, proposalText, admissionOpts, provenance, workspaceId);
...
function routeCandidateClaim(kernelOrGraph, claim, opts = {}) {
  ...
  if (conflict.recommendation === CONFLICT_RECOMMENDATIONS.ACCEPT) {
    ...
    const kernel = getJournalingKernel(kernelOrGraph);
    if (kernel) {
      return acceptCandidateClaimJournaled(kernel, candidate, conflict, built);
    }
