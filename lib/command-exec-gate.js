...
const COMMAND_EXEC_REASONS = Object.freeze({
  ...
  IRREVERSIBLE_GIT_OPERATION_REVIEW: 'IRREVERSIBLE_GIT_OPERATION_REVIEW',
});
...
/**
 * A force push overwrites history the remote already accepted, and nothing on
 * the remote keeps the overwritten commits reachable. That makes it the one git
 * operation whose damage survives the workspace boundary the rest of this gate
 * enforces -- `findOutOfWorkspaceTarget` cannot see it, because the target is
 * not a path.
 *
 * It reviews rather than blocks. Force-pushing your own branch is ordinary
 * work; force-pushing a shared one is not, and this gate cannot tell which
 * branch is which. `AGENTS.md` §4 encodes the same answer for this repository:
 * forbidden *unless explicitly approved*, which is a human decision, not a
 * refusal.
 *
 * Structural rather than textual for the reason #379 gives: flag order and
 * position vary (`git push --force origin main`, `git push origin main -f`,
 * `git -C dir push --force-with-lease`) and a single regex over the raw text
 * has to guess at all of them.
 */
const FORCE_PUSH_LONG_FLAGS = new Set(['--force', '--force-with-lease']);
const SHORT_FLAG_CLUSTER = /^-[a-z]*f[a-z]*$/i;

function findForcePushMatches(commandText) {
  const matches = [];
  for (const segment of splitCommandSegments(commandText)) {
    const tokens = stripCommandWrappers(tokenizeSegment(segment));
    const hasGit = tokens.some((token) => baseCommandName(token) === 'git');
    const hasPush = tokens.some((token) => token.toLowerCase() === 'push');
    if (!hasGit || !hasPush) continue;

    for (const token of tokens) {
      const flag = token.toLowerCase();
      const longFlag = flag.split('=')[0];
      if (FORCE_PUSH_LONG_FLAGS.has(longFlag)) {
        matches.push(longFlag.slice(2).replace(/-/g, '_'));
      } else if (!flag.startsWith('--') && SHORT_FLAG_CLUSTER.test(flag)) {
        matches.push('force');
      }
    }
  }
...
function evaluateCommandExec(input) {
  ...
  const forcePushMatches = findForcePushMatches(commandText);
  if (forcePushMatches.length > 0) {
    return buildResult(COMMAND_EXEC_DECISIONS.REVIEW, COMMAND_EXEC_REASONS.IRREVERSIBLE_GIT_OPERATION_REVIEW, {
      commandText,
      forcePushMatches,
    });
  }
...
function buildResult(decision, reason, details = {}) {
  return {
    ...
    forcePushMatches: details.forcePushMatches || [],
