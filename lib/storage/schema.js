...
const ADDITIVE_COLUMNS = Object.freeze([
  ...
  Object.freeze({
    ...
    column: 'workspace_id',
    definition: "TEXT NOT NULL DEFAULT 'default'",
    // Approval rows previously kept workspace identity only in JSON context,
    // which made every id/key/list/decision query cross-tenant. Existing rows
    // recover the workspace from the durable snapshot/context when present;
    // otherwise they retain the historical default scope.
    backfillSql: `UPDATE tool_approvals
      SET workspace_id = CASE
        WHEN json_valid(context_json)
          AND typeof(COALESCE(json_extract(context_json, '$.workspaceId'), json_extract(context_json, '$.snapshot.workspaceId'))) = 'text'
          AND length(trim(COALESCE(json_extract(context_json, '$.workspaceId'), json_extract(context_json, '$.snapshot.workspaceId')))) BETWEEN 1 AND 128
        THEN trim(COALESCE(json_extract(context_json, '$.workspaceId'), json_extract(context_json, '$.snapshot.workspaceId')))
        ELSE 'default'
      END`,
    reason: 'tool approval workspace isolation',
  }),
]);

// Monotonic storage schema version: each additive migration advances it by one.
// Doctor and operational tooling read this instead of inventing a parallel version source.
const STORAGE_SCHEMA_VERSION = ADDITIVE_COLUMNS.length;
...
const SCHEMA_INDEXES = Object.freeze([
  ...
  'CREATE INDEX IF NOT EXISTS idx_tool_approvals_workspace_status_updated ON tool_approvals(workspace_id, status, updated_at DESC)',
