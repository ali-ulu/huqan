...
function openProgressMarker(progressPath) {
  try {
    return fs.openSync(progressPath, 'wx');
  } catch (error) {
    if (error && error.code === 'EEXIST') {
      const interrupted = new Error('A previous restore was interrupted. Recover from the recorded safety backup before retrying.');
      interrupted.code = 'RESTORE_INTERRUPTED';
      interrupted.receipt = { status: 'unknown' };
      throw interrupted;
    }
    throw error;
  }
