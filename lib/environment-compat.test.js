...
const EXPECTED_SUFFIXES = Object.freeze([
  ...
  'CLI_READ_ROOTS', 'COLLECTOR_SEAL_KEY', 'COLLECTOR_SEAL_KEY_ID', 'DB_PATH', 'DEMO_MODE',
  'DISABLE_API_AUTH', 'DISABLE_AUTO_LISTEN',
  'EXTERNAL_CLIENT_ENDPOINT_ENABLED', 'EXPERIENCE_ENABLED', 'EXTERNAL_CLIENT_REPLAY_DB_PATH',
  // #1859: the collector decides which sending keys it trusts, and whether an
  // unverified batch is still storable, from deployment configuration.
  'EXTERNAL_CLIENT_TRUST_PROFILE_PATH', 'RECEIPT_COLLECTOR_ROOT', 'RECEIPT_REQUIRE_SIGNATURE',
  'RECEIPT_TRUSTED_KEYS', 'REGISTRY_DIR', 'GITHUB_APP_BETA_ENABLED',
  ...
  'GITHUB_APP_WEBHOOK_SECRET', 'HOST', 'HUMAN_APPROVAL_DISABLED', 'HEADERS_TIMEOUT_MS',
  // #1293: file-backed ingest no longer accepts whatever root the caller names,
  // so the deployment needs a way to widen the boundary deliberately.
  'INGEST_ALLOWED_ROOTS',
  'INGEST_APPROVAL_LEASE_MS',
  // #2018: the issuer seal signs with one configured key; the key id is derived
  // from it rather than configured, so sealing is one variable, not a pair.
  'ISSUER_SEAL_KEY',
  'KEEP_ALIVE_TIMEOUT_MS', 'MAX_CONCURRENT_REQUESTS', 'MAX_QUEUE_DEPTH', 'KERNEL_VERSION', 'LANG',
  // #1908: transparent LLM proxy reads its upstream, timeout and server key.
  'LLM_PROXY_API_KEY', 'LLM_PROXY_TIMEOUT_MS', 'LLM_PROXY_UPSTREAM',
  'MCP_CAPABILITY_NONCE_DIR', 'MCP_LEGACY_VERIFY_STATUS', 'MCP_OPERATOR_TOKEN', 'MEMORY_PATH',
  // The integrity siren's webhook is deployment configuration like any other,
  // so it reads through the compat shim rather than off process.env (#2591).
  'NOTIFY_WEBHOOK_SECRET', 'NOTIFY_WEBHOOK_URL',
  'OBSERVABILITY_AUTHZ_POLICY', 'PARANOID',
  'PLUGIN_PRODUCTION_ENFORCEMENT', 'PLUGIN_SIGNING_KEY',
  'PLUGIN_STRICT', 'REQUEST_TIMEOUT_MS', 'SUPPLY_CHAIN_ACTIVATION_POLICY', 'PORT', 'RUST_BIN', 'TRUST_POLICY_ROOTS', 'TRUST_PROXY',
...
test('container manifests preserve absent variables and use the compatibility bootstrap', () => {
  ...
  // The service is `huqan`; the AXIOM_ variables below stay in its environment
  // because the reader accepts both spellings, per RFC-001 decision 7.
  const service = compose.services.huqan;
...
test('runtime entrypoints reject conflicts before module startup', () => {
  ...
  for (const entrypoint of [
    ...
    'scripts/backup.js',
    'scripts/restore.js',
    'scripts/knowledge-graph-demo.js',
...
test('runtime source contains no direct branded process.env reads outside resolver', () => {
  ...
  const runtimeFiles = [
    ...
    'scripts/knowledge-graph-demo.js', 'scripts/seed-demo.js',
  ];
