const { toLegacyVerifyStatus } = require('./verify-status-vocabulary');
...
function normalizeCheck(result) {
  ...
  // Reader accepts both vocabularies (RFC-001 compatibility rule): a check
  // may arrive already projected into canonical English by the API boundary,
  // or straight from the kernel in the legacy internal spelling.
  const status = toLegacyVerifyStatus(data?.status);
