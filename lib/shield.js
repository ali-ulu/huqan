...
function evaluateLlmSor({
  ...
  // #1296: kernel.verify() above is called with llmTextStr in full (see PR #89 /
  // SHD-1, docs/audits/security-hardening-s1-checkpoint.md), so the whole
  // response is verified regardless of length. Only fullTextVerified/totalLength
  // describe that; a `verifiedPrefix: 300` key used to sit here too, left over
  // from before #89 fixed the prefix-only bug, and it contradicted
  // fullTextVerified by implying verification stopped at 300 chars.
  const partialVerification = totalLength > 300
    ? { totalLength, fullTextVerified: true }
    : null;
