...
/**
 * #2505 I: shape evaluated bypass signals as stop-proposal recommendations
 * for the behavioral bridge (`proposeBehavioralStop`). Pure mapping, no
 * approval runtime involved: `review` becomes a `require_review`
 * recommendation, `block-and-propose` a `block` recommendation, and `none`
 * maps to nothing. No deviation code or receipt is invented -- the bridge
 * already tolerates their absence -- and nothing is proposed here; the
 * caller decides whether to open a review case.
 */
function recommendationsForBypass(evaluation, scope = {}) {
  if (!evaluation || typeof evaluation !== 'object' || Array.isArray(evaluation)) {
    throw new TypeError('evaluation must be an evaluateBypassResponse result');
  }
  const workspaceId = typeof scope.workspaceId === 'string' && scope.workspaceId.trim()
    ? scope.workspaceId.trim()
    : null;
  if (!workspaceId) throw new TypeError('scope.workspaceId is required');
  const agentId = typeof scope.agentId === 'string' && scope.agentId.trim() ? scope.agentId.trim() : null;
  const responses = evaluation.responses && typeof evaluation.responses === 'object' ? evaluation.responses : {};
  const out = [];
  for (const [fingerprint, entry] of Object.entries(responses)) {
    if (!entry || typeof entry !== 'object') continue;
    const decision = entry.response === 'block-and-propose' ? 'block'
      : entry.response === 'review' ? 'require_review'
      : null;
    if (!decision) continue;
    out.push(Object.freeze({
      decision,
      scope: Object.freeze({ workspaceId, agentId }),
      reason: `bypass fingerprint ${fingerprint}: ${Array.isArray(entry.reasons) ? entry.reasons.join('; ') : 'recommended'}`,
    }));
  }
