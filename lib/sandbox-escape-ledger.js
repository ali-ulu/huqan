 ...
 * proceed, and `sandboxRunner.js` returns that verdict on `meta.ab6`.
 * `lib/sandbox-escape-producer.js` records block/quarantine verdicts when the
 * caller supplies a graph (#2505 G) — it lives outside `sandboxRunner.js`,
 * which is over the large-file threshold (#328) and may not grow. Every
 * current and future caller that runs through the producer persists `meta.ab6`
 * without remembering to: a verdict that exists only in a returned object for
 * the length of one call cannot support a claim about anything.
 *
