 ...
 * `runSandboxed` itself records block/quarantine verdicts when the caller
 * supplies `opts.graph` (#2505 G), so every current and future caller is a
 * producer without remembering to persist `meta` — a verdict that exists only
 * in a returned object for the length of one call cannot support a claim
 * about anything.
 *
