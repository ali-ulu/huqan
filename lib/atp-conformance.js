...
// Deliberately narrower than VALID_SOURCE_TYPES in provenance-ingest.js, which
// also accepts 'manual', 'decision' and 'background_inference'. Those three name
// writes that originate inside this repository, and this set is applied to
// packages arriving from outside — via huqan-package-format.js, reached from
// external-client-package-gate.js. Letting an external client label its own
// claim 'background_inference' would pass that claim off as the kernel's own
// bookkeeping, so the two sets must not be tidied into agreement. The boundary
// is pinned by 'an external client may not claim an internal-only source type'
// in external-client-package-gate.test.js (#2040).
