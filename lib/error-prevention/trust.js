...
function classifyFailureTrust(source, evidence = [], options = {}) {
  const normalized = normalizeSource(source);
  const hasEvidence = Array.isArray(evidence) && evidence.length > 0;
  const externallyVerified = options && options.verified === true;
  if (OBJECTIVE_FAILURE_SOURCES.has(normalized) && hasEvidence && externallyVerified) {
    return { source: normalized, verificationStatus: 'verified', trust: 'high' };
  }
  if (OBJECTIVE_FAILURE_SOURCES.has(normalized) && hasEvidence) {
