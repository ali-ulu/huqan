...
const CANONICAL_PRIORITY = Object.freeze({
  allow: 0,
  disabled: 1,
  review: 2,
  dry_run_only: 3,
...
function buildPreflightDecision(action, rules, policyVersion, options = {}) {
  ...
  const decision = mergeWithUpstreamVerdict(upstreamVerdict, preventionDecision);
  ...
  else if (decision !== preventionDecision) reasonCodes.push('STRICTER_UPSTREAM_VERDICT_PRESERVED');
  const payload = {
    ...
    matchedFailureIds: matches.map((rule) => rule.sourceFailureId), reasonCodes,
  };
  return {
    ...
    reasonCodes, matchedRules: matches,
