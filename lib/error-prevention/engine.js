...
class ErrorPrevention {
  ...
  _resolveRuleApproval(ruleMemoryId, rule, opts) {
    ...
    try {
      ...
      if (!approvalId || !['approved', 'rejected'].includes(status)) return { approvalId, status: 'pending' };
  ...
  proposeRule(failureMemoryId, input = {}) {
    ...
    const content = {
      ...
      status: 'proposed', enforcement, trigger,
      ...
      activationEligible: failure.verificationStatus === 'verified', workspaceId,
  ...
  preflight(input = {}, options = {}) {
    ...
    return listed.ok ? buildPreflightDecision(action, listed.rules, this.policyVersion, options) : listed;
  }
