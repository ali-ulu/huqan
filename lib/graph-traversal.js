...
/**
 * Detailed cycle wrapper exposing the bounded result and its stop reason.
 * `visited` is still mutated in place for callers that inspect it afterwards.
 */
function detectCycleWithMeta(graph, start, visited, pathArr, workspaceId = 'default', opts = {}) {
...
/**
 * Signature-compatible wrapper: same arguments and same cycle|null result as
 * before, now bounded. Use detectCycleWithMeta when callers need stop metadata.
 */
function detectCycle(graph, start, visited, pathArr, workspaceId = 'default', opts = {}) {
  const result = detectCycleWithMeta(graph, start, visited, pathArr, workspaceId, opts);
  return opts.returnMeta ? result : result.cycle;
}
