 ...
 */
// A double quote or `%` would end the quoted operand or expand inside it,
// turning the for-loop below into a second command; a line break would end
// the command line. Such a path is returned as is and, still holding its
// space, dropped by gateCommandCandidates. `cmd.exe` is named literally
// rather than read from ComSpec, which any parent process can set.
...
function unspaced(target) {
  ...
  const run = spawnSync('cmd.exe', ['/d', '/s', '/c', `for %I in ("${target}") do @echo %~sI`], { encoding: 'utf8' });
...
module.exports = Object.freeze({ pickGateCommand });
