...
test('owner is packaged only as the D8 server dependency and receipt module has no owner dependency', () => {
  ...
  assert.equal(packageJson.files.includes('lib/external-client-mutation-receipt-owner.js'), true);
...
test('aggregate JSON node budget rejects oversized sibling structures before mutation', (t) => {
  const { graph } = graphFixture(t);
  const pkg = packageFixture();
  pkg.metadata.padding = Array.from({ length: 10001 }, (_, index) => index);
...
test('npm package dry-run preserves receipt separation and packages the D8 owner dependency', () => {
  ...
  const result = spawnSync(
      ...
      : ['pack', '--dry-run', '--json', '--ignore-scripts'],
    {
      ...
      env: {
        ...process.env,
        npm_config_cache: path.join(os.tmpdir(), 'huqan-npm-pack-cache'),
      },
    },
  );
  ...
  assert.equal(Array.isArray(manifests), true);
  assert.equal(manifests.length, 1);
  const packedFiles = new Set(manifests[0].files.map((entry) => entry.path));

  assert.equal(packedFiles.has('kernel.js'), true);
  assert.equal(packedFiles.has('graph.js'), true);
  assert.equal(packedFiles.has('lib/receipt/v4-receipt-family.js'), true);
  assert.equal(packedFiles.has('lib/external-client-mutation-receipt-owner.js'), true);
  assert.equal(packedFiles.has('lib/external-client-structural-guards.js'), false);
  assert.equal(packedFiles.has('lib/external-client-mutation-intake.js'), false);
});

// --- mutation admission ----------------------------------------------------

test('admission: a refusal leaves no candidate row, no journal entry and no receipt', (t) => {
  const { graph } = graphFixture(t);
  const pkg = packageFixture();
  const context = contextFixture(pkg);
  ...
  expectCode(
    () => commitExternalClientCandidateClaim(pkg, context, { graph, admission }),
    'MUTATION_ADMISSION_REFUSED',
  );

  // The durable commit is the admitted effect, so a refusal is not advisory:
  // there is nothing to reconcile because nothing was attempted.
  assert.deepEqual(graph.getCandidateClaims({ workspaceId: context.workspaceId }), []);
});

test('admission: a refusal is not reported as an unknown outcome', (t) => {
  // The most misleading translation available here would be to let a refusal
  // fall into the OUTCOME_UNKNOWN catch, telling an operator that a write may
  // have half-happened and needs reconciliation when in fact it was denied.
  const { graph } = graphFixture(t);
  const pkg = packageFixture();
  const context = contextFixture(pkg);
  ...
  assert.equal(caught.code, 'MUTATION_ADMISSION_REFUSED');
  assert.equal(caught.admissionReason, 'identity.workspace_binding_failed');
  assert.notEqual(caught.code, EXTERNAL_CLIENT_MUTATION_RECEIPT_OWNER_ERRORS.OUTCOME_UNKNOWN);
  assert.equal(caught.details?.reconciliationRequired, undefined);
});

test('admission: the durable commit runs inside the admitted effect', (t) => {
  const { graph } = graphFixture(t);
  const pkg = packageFixture();
  const context = contextFixture(pkg);
  let seen = null;
  ...
  const admission = {
    admit: (adm, mutate) => {
      seen = adm;
      rowsBeforeMutate = graph.getCandidateClaims({ workspaceId: context.workspaceId }).length;
      return { admitted: true, result: mutate() };
    },
  };

  const result = commitExternalClientCandidateClaim(pkg, context, { graph, admission });

  assert.equal(rowsBeforeMutate, 0);
  assert.equal(result.ok, true);
  assert.equal(graph.getCandidateClaims({ workspaceId: context.workspaceId }).length, 1);

  assert.equal(seen.action, ADMISSION_ACTION);
  // Authoritative, not defaulted: the workspace the authority bound.
  assert.equal(seen.workspaceId, context.workspaceId);
});

test('admission: absence here means a missing claim shape, not a missing identity', (t) => {
  const { graph } = graphFixture(t);
  const pkg = packageFixture();
  const context = contextFixture(pkg);
  let seen = null;
  const admission = {
    admit: (adm, mutate) => { seen = adm; return { admitted: true, result: mutate() }; },
  };

  commitExternalClientCandidateClaim(pkg, context, { graph, admission });

  assert.equal(seen.identityClaim.kind, 'absent');
  // The distinguishing fact, asserted so the reason cannot drift back to the
  // kernel callers' wording: this ingress verifies identity before the seam.
  assert.match(ABSENCE_REASONS.identityClaim, /verifies a receiver-owned identity/);
  assert.match(ABSENCE_REASONS.identityClaim, /gate 3/);
  assert.doesNotMatch(ABSENCE_REASONS.identityClaim, /caller-supplied/);

  // And no invented claim shape leaked into the context. Passing a made-up
  // object would be accepted silently today -- admit() validates nothing beyond
  // "non-array object" -- and would drop this call site out of the enumeration
  // of places still lacking a claim.
  assert.equal(seen.identityClaim.subject, undefined);
  assert.equal(seen.identityClaim.trustedKeyId, undefined);
});

test('admission: an unusable injected seam is refused, and unknown dependency keys still are', (t) => {
  const { graph } = graphFixture(t);
  const pkg = packageFixture();
  const context = contextFixture(pkg);
  ...
  // Widening the exact-shape check to admit `admission` must not admit anything
  // else, and `graph` must still be mandatory.
  expectCode(
    () => commitExternalClientCandidateClaim(pkg, context, { graph, unexpected: 1 }),
    EXTERNAL_CLIENT_MUTATION_RECEIPT_OWNER_ERRORS.GRAPH_REQUIRED,
  );
  expectCode(
    () => commitExternalClientCandidateClaim(pkg, context, { admission: { admit: () => {} } }),
    EXTERNAL_CLIENT_MUTATION_RECEIPT_OWNER_ERRORS.GRAPH_REQUIRED,
  );
});
