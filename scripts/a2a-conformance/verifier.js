...
const { enforceExternalClientPackage } = require('../../lib/external-client-package-gate');
const { encodeJsonStableV1 } = require('../../lib/v5/cryptographic-profile-contract');
const { verifyCryptographicEvidence } = require('../../lib/v5/cryptographic-verification-adapter');
const { importPublicTrustReceipt } = require('../../lib/v5/public-trust-receipt');
...
const REQUEST_KEYS = Object.freeze([
  ...
  'constraints', 'observation', 'evidence', 'signature',
]);
...
function evaluateBoundedExchange(input) {
  try {
    if (utilTypes.isProxy(input)
      || !exactObject(input, ['request', 'authority', 'evaluationTime', 'replayReserve', 'effect'])) {
