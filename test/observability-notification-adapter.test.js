...
const SIGNING_MATERIAL = ['notification', 'fixture', 'value', '2026'].join('-');
const BASE_INPUT = {
...
test('normalizes only bounded alert notification fields and drops plaintext secrets', () => {
  ...
  assert.equal(JSON.stringify(normalized).includes(SIGNING_MATERIAL), false);
...
test('rejects non-HTTPS webhook configuration without exposing a secret', () => {
  for (const url of ['http://example.test/hook', 'https://user:pass@example.test/hook', 'https://example.test/hook?x=1', 'https://example.test/hook#fragment']) {
    assert.throws(() => createWebhookNotificationAdapter({ url, secret: SIGNING_MATERIAL, fetchImpl: async () => response(204) }), error => error.code === 'NOTIFICATION_INVALID_CONFIG');
  }
...
test('posts deterministic signed JSON and accepts only 2xx responses', async () => {
  ...
  assert.equal(calls[0].options.headers['X-Huqan-Notification-Signature'], signatureFor(SIGNING_MATERIAL, calls[0].options.body));
  assert.equal(calls[0].options.body.includes(SIGNING_MATERIAL), false);
...
test('notifySafely converts adapter exceptions to a non-throwing typed failure', async () => {
  const result = await notifySafely({ deliver: async () => { throw new Error(`secret=${SIGNING_MATERIAL}`); } }, BASE_INPUT);
  assert.deepEqual(result, { ok: false, code: 'NOTIFICATION_FAILED', attempts: 0, notificationId: 'notification-1' });
  assert.equal(JSON.stringify(result).includes(SIGNING_MATERIAL), false);
