...
const UNROUTED_SINK_CALLS = Object.freeze({
  ...
  'graph.js': { why: 'graph optimize maintenance audit; DELETE evidence is emitted by the Graph persistence owner until the family-independent admission seam covers maintenance operations', sinks: { appendAuditEvent: 1 } },
...
test('mutation admission: the debt ledger reflects the routing done so far', () => {
  ...
  // #1080 adds the reviewed Graph-owned DELETE evidence for optimize() while
  // the maintenance family has no admission seam of its own yet.
  assert.equal(unrouted, 23, 'unrouted sink calls');
  assert.equal(routed, 27, 'sink calls routed through admission (K2 + DEL callbacks)');
  assert.equal(unrouted + routed, 50, 'total sink calls, raised by K2 delegation, DEL audit, and optimize evidence');
});
