...
function executeSandboxed(source, bindings = {}, opts = {}) {
...
/**
 * #2505 G: the runner is the production producer of sandbox escape attempts.
 ...
 * `opts.graph` (absent means exactly today's behavior), bounded to escape
...
function runSandboxed(source, bindings = {}, opts = {}) {
  const result = executeSandboxed(source, bindings, opts);
  if (opts.graph) {
    try {
      const { recordSandboxVerdict } = require('./lib/sandbox-escape-ledger');
