...
| immutable source snapshot binding | `docs/v5/v5-immutable-source-snapshot-contract.md` | optional `receipt.sourceSnapshot`, exact key set `{ snapshotId, snapshotVersion, hash, algorithm }`, version pinned to `huqan.external-source-snapshot.v1`, algorithm fixed to `sha256` | valid source snapshot fixture and malformed source snapshot fixture | carry-or-reject at the runtime writer; secret-looking values rejected whole at write time | validator and writer both fail closed on malformed snapshots; route rejects secret-looking values | covered | snapshot provenance/authorization claims remain future gate | no |
