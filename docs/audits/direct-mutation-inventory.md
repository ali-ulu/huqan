...
| LLM memory plugin | `plugins/llm-memory-plugin.js:12-19` calls `learnFromLLM()` | Plugin hook / Kernel learn | none (Closed #218) | The redundant direct `kernel.graph.save()` was removed; `learnFromLLM()` already durably persists each outcome via `kernel.learn()`'s journal (#216). Hook rejection is still swallowed by the caller path. |
| Shield auto-learn | `lib/shield.js:89-109` calls `learnFromLLM()` | SDK/server shield / Kernel learn | none (Closed #218) | The redundant direct `graph.save()` was removed; `learnFromLLM()` already durably persists each outcome via `kernel.learn()`'s journal (#216). Learn errors are still shaped. |
...
6. ~~Plugin/shield direct Graph saves bypass the Kernel persistence facade and
   have no persistence audit/receipt.~~ **Closed (#218).** Both calls were
   removed: since #216, `kernel.learn()`/`learnFromLLM()` already durably
   journal and self-persist every outcome, so the extra manual
   `kernel.graph.save()` in `plugins/llm-memory-plugin.js` and `lib/shield.js`
   was redundant duplicate I/O, not a genuine persistence-facade bypass.
