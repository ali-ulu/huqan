...
| Backup/restore helper | `backupRestore.js:96-130` and `160-203` copy/replace persistence files and remove WAL/SHM files | Filesystem persistence helper | `direct-owner` | Multi-file restore is not transactional. The module emits no audit or receipt; copy and restore errors throw. |
