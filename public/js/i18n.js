...
function setupLocaleSelector() {
  ...
  selector.addEventListener('change', async (e) => {
    ...
    // Fetch before switching: a failed load used to leave currentLocale and
    // the stored preference on the locale whose messages never arrived, so
    // the next visit reopened a language the page could not actually render.
    const newMessages = await loadLocale(newLocale);
    if (!newMessages) {
      console.error(`[i18n] Failed to load locale ${newLocale}, staying on ${oldLocale}`);
...
/**
 * Only leaf elements carry `data-i18n`: applyTranslations assigns
 * textContent, which would delete the children of a wrapper. A composite
 * control (a nav button with its icon and sub-label) is annotated on its
 * inner text nodes instead, never on the button.
 */
if (typeof document !== 'undefined') {
  const start = () => { initI18n().catch(error => console.error('[i18n] init failed:', error)); };
